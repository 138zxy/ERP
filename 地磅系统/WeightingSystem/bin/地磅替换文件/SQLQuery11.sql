...
/****** Object:  View [dbo].[MetageStuffIn]    Script Date: 2020/6/8 18:27:52 ******/
SET ANSI_NULLS ON
GO
...
AS
    ...
    FROM    ( SELECT    ROW_NUMBER() OVER ( PARTITION BY father.StuffInID ORDER BY child.ParentID, child.OrderNum DESC ) AS rownumber ,
						...
						,Com.CompName
						,father.Volume
              FROM      (select * from dbo.StuffIn where buildtime>=(getdate()-30)) AS father
                        LEFT JOIN (select CarWeight,ParentID,pic4,pic3,SupplyNum,OutDate,OrderNum,InNum,Lifecycle from StuffIn where parentid is not null and buildtime>=(getdate()-30)) AS child 
                        ON child.ParentID = father.StuffInID
