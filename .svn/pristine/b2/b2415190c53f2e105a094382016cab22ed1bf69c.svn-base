    ...
    public class AlarmLogService : ServiceBase<AlarmLog>
    {
        ...
        public List<GPS_CarAlarmInfo> GetAlarams(string tid, IEnumerable<Car> allcars, IEnumerable<LastestGpsInfo> gpsinfos, IEnumerable<AlarmLog> allog)
        {
            ...
            else
            {
                ...
                double gpstime = Convert.ToDouble(ps.SysConfig.GetSysConfig("OilTime").ConfigValue);

                var Tid = gpsinfo.TerminalID;
                var cars = allcars.Where(e => e.TerminalID == tid && e.CarTypeID == "CT1");
                var car = cars != null && cars.Count() > 0 ? cars.First() : null;
                if (car == null)
                {
                }
                else
                {
                    if (gpsinfo.Receivetime != null && (DateTime.Compare(gpsinfo.Receivetime.Value.AddMinutes(gpstime), DateTime.Now) <= 0))
                    {
                        GPS_CarAlarmInfo caralarm = new GPS_CarAlarmInfo();
                        caralarm.alarmInfo = "超过" + gpstime + "分钟无GPS数据上传！";
                        caralarm.alarmTypeID = "002006";
                        caralarm.alarmTime = DateTime.Now;
                        list.Add(caralarm);
                    }
                    else
                    {
                        ...
                        var allogs = allog.Where(e => e.AlarmTime > DateTime.Now.AddMinutes(-gpstime) && e.CarID.ToString() == car.ID);
                        foreach (var log in allogs)
                        {
                            GPS_CarAlarmInfo caralarm = new GPS_CarAlarmInfo();
                            caralarm.alarmInfo = log.AlarmData;
