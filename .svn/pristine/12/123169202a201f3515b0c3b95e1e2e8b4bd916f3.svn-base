...
function produceTaskIndexInit(opts) {
    function bindIdentitySettings(container, contractItemId, taskId) {
        ...
        { contractId: contractItemId, taskId: taskId },
        false,
        function (response) {
            container.empty();
    ...
    function disableTechField(disable) {
        ...
        mygrid.setFormFieldDisabled('BetonTag', disable);
        //合同名称
    ...
    var mygrid = new MyGrid({
        ...
        , { label: '级配', name: 'CarpGrade', index: 'CarpGrade', edittype: 'select', width: 50, editoptions: { value: "I级配:I级配;II级配:II级配" }, hidden: true }
        ...
        , { label: '湿拌类别', name: 'CementType', index: 'CementType' }
        , { label: ' 湿拌配比', name: 'BetonFormulaName', index: 'BetonFormulaInfo.FormulaName', width: 80 }
        , { label: ' 湿拌配比ID', name: 'BetonFormula', index: 'BetonFormula', width: 80, hidden: true }
        ...
        , { label: '要求塌落度', name: 'DemandSlump', index: 'DemandSlump', width: 80, hidden: true }
        , { label: '要求检测人', name: 'DemandChecker', index: 'DemandChecker', width: 80, hidden: true }
        , { label: '要求检测时间', name: 'DemandCheckTime', index: 'DemandCheckTime', formatter: 'datetime', width: 120, hidden: true }
        , { label: '实测塌落度', name: 'RealSlump', index: 'RealSlump', width: 80, hidden: true }
        , { label: '实测检测人', name: 'RealChecker', index: 'RealChecker', width: 80, hidden: true }
        , { label: '实测检测时间', name: 'RealCheckTime', index: 'RealCheckTime', formatter: 'datetime', width: 120, hidden: true }
        , { label: "任务附件", name: "Attachments", formatter: attachmentFmt, sortable: false, search: false }
        , { label: '项目编号', name: 'TaskNo', index: 'TaskNo' }
        , { label: '子分部工程', name: 'ChildProject', index: 'ChildProject' }
   	]
            , autoLoad: true
            , functions: {
                ...
                handleRefresh: function (btn) {
                    mygrid.refreshGrid('1=1 AND DataType=1');
                },
                    ...
                    if (records.length == 1) {
                        mygrid.handleEdit({
                            ...
                            afterFormLoaded: function () {
                                ...
                                bindSelectData(contractIdField,
                                        ...
                                        { textField: 'ContractName',
                                            valueField: 'ID',
                                            condition: "CustomerID='" + customerId + "' and ContractStatus!='3' and AuditStatus='1' AND DataType=1  AND IsLimited = 0 "
                                        }, function () {
                                            if (!isEmpty(record.ContractID)) {
                                                ...
                                                bindSelectData(mygrid.getFormField("ContractItemsID"),
                                                    ...
                                                    { textField: 'ConStrength',
                                                        ...
                                                        orderBy: 'ConStrength',
                                                        ascending: true,
                                                        condition: "ContractID='" + contractId + "' AND ConStrength like 'M%'"
                                                    }, function () {
                            ...
                            beforeSubmit: function () {
                                ...
                                if (fileElement.val() == "") {
                                    showMessage("提示", "新建生产任务请至少上传一个附件！");
                                    return false;
                                }
                    ...
                    mygrid.handleAdd({
                        ...
                        afterFormLoaded: function () {
                            ...
                            contractNameField.bind('change', function (event) {
                                ...
                                if (contractId && contractName == selectedContractName) {
                                    ...
                                    bindIdentitySettings('#rightdiv3', contractId);
                                }
                        ...
                        , postCallBack: function (response) {
                            ...
                            if (response.Result) {
                                ...
                                var identities = [];
                                $('#rightdiv3 input:checked').each(function () {
                                ...
                                //上传附件
                                attachmentUpload3(response.Data);
                            }
                        }
                ...
                handleEdit: function (btn) {
                    ...
                    mygrid.handleEdit({
                        ...
                        afterFormLoaded: function () {
                            ...
                            var cid = mygrid.getFormField("ContractID").val(); //合同明细id
                        ...
                        beforeSubmit: function () {
                            ...
                            //检查是否重新上传附件
                            var isAddAtt = true;
                            var fileElement = $('#uploadFile2');
                            if (fileElement.val() == "") {
                                isAddAtt = false;
                            }
                            //检查编辑前是否已经有附件存在
                            var alreadyExitsAtt = true;
                            console.log($("a[href]", "#MyEditForm").length);
                            if ($("a[href]", "#MyEditForm").length < 1) {
                                alreadyExitsAtt = false;
                            }
                            console.log(isAddAtt || alreadyExitsAtt);
                            if (!(isAddAtt || alreadyExitsAtt)) {
                                showMessage("提示", "修改生产任务请至少上传一个附件！");
                            }
                ...
                handleCompleted: function () {
                    mygrid.refreshGrid("IsCompleted = 1 AND DataType=1");
                },
                //未完工任务单 add by xyl on 2012-3-26
                handleNotCompleted: function () {
                    mygrid.refreshGrid("IsCompleted = 0 AND DataType=1");
                },
                ...
                handleThree: function (btn) {
                    ...
                    condition = "NeedDate between '" + uom + " 00:00:00' and '" + today + " 23:59:59' AND DataType=1";
                ...
                handleToday: function (btn) {
                    ...
                    //从0点算
                    condition = "NeedDate between '" + today + " 00:00:00' and '" + today + " 23:59:59'  AND DataType=1";
                    //从交班时间开始算                    
                    ...
                    mygrid.showWindow({
                        ...
                        buttons: {
                            "关闭": function () {
                                ...
                                condition = "NeedDate between '" + BeginTime + "' and '" + EndTime + "' AND DataType=1";
                                mygrid.refreshGrid(condition);
        ...
        customerIdField.bind('change', function () {
            ...
            if (customerId) {
                bindSelectData(mygrid.getFormField("ContractID"),
                                        ...
                                        { textField: 'ContractName',
                                            valueField: 'ID',
                                            condition: "CustomerID='" + customerId + "' and ContractStatus!='3' and AuditStatus='1'  AND DataType=1 "
                                        }
                                    );
            ...
            if (isEmpty(pid)) {
                ...
                ajaxRequest(requestURL, postParams, false, function (response) {
                    if (response) {
                        ...
                        mygrid.getFormField("Distance").val(response.Data == null ? "" : rData.Distance);
    ...
    function attachmentUpload(objectId) {
        ...
        $.ajaxFileUpload({
            url: opts.uploadUrl + '?objectType=ProduceTask&objectId=' + objectId,
            secureuri: false,
    ...
    function attachmentUpload2(objectId) {
        var fileElement = $('#uploadFile2');
        if (fileElement.val() == "") return;
        $.ajaxFileUpload({
            url: opts.uploadUrl + '?objectType=ProduceTask&objectId=' + objectId,
    ...
    function attachmentUpload3(objectId) {
        var fileElement = $('#uploadFile3');
        if (fileElement.val() == "") return;
        $.ajaxFileUpload({
            url: opts.uploadUrl + '?objectType=ProduceTask&objectId=' + objectId,
