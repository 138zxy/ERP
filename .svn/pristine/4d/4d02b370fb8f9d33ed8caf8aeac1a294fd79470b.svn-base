function M_YingSFIndexInit(options) {
    var myJqGrid = new MyGrid({
		...
		, showPageBar: true
        , singleSelect: false
        , storeCondition: "(supplykind in ('Su1','Su2','Su4','Su5') and IsUsed =1)"
		, initArray: [
              { label: '厂商编号', name: 'ID', index: 'ID', width: 80 } 
        ...
        , functions: {
            ...
            handlePayIni: function (btn) {
                ...
                myJqGrid.handleEdit({
                    ...
                    afterFormLoaded: function () {
                        ...
                        $("#M_YingSFrec_FinanceMoney").val(Record.PaidOwing);
                        $("#M_YingSFrec_UnitID").val(Record.ID);
                    }
                });
            },
            print: function (btn) {
                ...
                var url = "/GridReport/DisplayReport.aspx?report=M_YingSF&supplyid=" + key + "&key=" + keys;
                window.open(url, "_blank");
            },
            PrintDesign: function (btn) {
                var url = "/GridReport/DesignReport.aspx?report=M_YingSF";
                window.open(url, "_blank");
            },
            PrintDirect: function (btn) {
                ...
                var url = "/GridReport/PrintDirect.aspx?report=M_YingSF&supplyid=" + key + "&key=" + keys;
                window.open(url, "_blank");
            }
        }
    });
    ...
    var myJqGridToPay = new MyGrid({
        ...
        initArray: [
            ...
            , { label: '材料', name: 'StuffInfo.StuffName', index: 'StuffInfo.StuffName', width: 100 }
            , { label: 'ID', name: 'ID', index: 'ID', hidden: true, width: 100 } 
            , { label: '确认数量(吨)', name: 'AllOkCount', index: 'AllOkCount', width: 80 }
            , { label: '结算金额', name: 'AllOkMoney', index: 'AllOkMoney', width: 60, align: 'right', formatter: 'currency' }
            , { label: '欠款额', name: 'PayOwing', index: 'PayOwing', width: 60, search: false, align: 'right', formatter: 'currency' }
            , { label: '本次付款额', name: 'PayMoney2', index: 'PayMoney2', width: 80, editable: true, search: false, align: 'right', formatter: 'currency' }
            , { label: '优惠额', name: 'PayFavourable2', index: 'PayFavourable2', width: 60, editable: true, align: 'right', formatter: 'currency' }
	...
	function (id, record, selBool) {
	    ...
	    myJqGridTo.refreshGrid("StockPactID='" + id + "' and PayOwing>0 and AuditStatus=1");
	});
	...
	myJqGridToPay.addListeners("afterSaveCell", function () {
	    ...
	    $("#M_BaleBalance_PayMoney").val(PayAll);
	    $("#M_BaleBalance_PayFavourable").val(PayPal);
	    $("#M_BaleBalance_PayOwing").val(overPay1);

	});
	$("#M_YingSFPayForm").dialog({
	    modal: true,
	    autoOpen: false,
	    width: 900,
	    Height: 500,
