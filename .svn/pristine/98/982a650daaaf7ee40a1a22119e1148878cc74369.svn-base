...
namespace WeightingSystem.Helpers
{
    ...
    class ERPDataProvider
    {
        ...
        string m_Select = @"SELECT StuffInID,CarNo,totalNum,CarWeight,InDate,OutDate,Operator,(SELECT TOP 1 stuffName FROM stuffInfo 
                            WHERE stuffInfo.StuffID = StuffIn.StuffID) AS StuffName,StuffID,siloID,(SELECT TOP 1 siloName FROM silo 
                            WHERE silo.siloId = StuffIn.siloId) AS siloName,SupplyID,(SELECT TOP 1 SupplyName FROM SupplyInfo 
                            ...
                            WHERE SupplyInfo.SupplyID = StuffIn.TransportID) as TransportName,Driver,InNum,Pic1,Pic2,Pic3,Pic4,DarkWeight,FastMetage,Spec,ParentID,
                            ...
                            ,MingWeight,CompanyID,SourceNumber";//,TransferName,KZL 
        ...
        {
            string sql = "SELECT SupplyName,SupplyName FROM dbo.SupplyInfo where IsUsed = 1 order by SupplyName ";
            return GetListFromSql(sql);
        }
        /// <summary>
        /// 获取材料出厂厂商
        ...
        {
            string m_Insert = @"INSERT INTO StuffIn(StuffInID,StuffID,Spec,SiloID,CarNo,SupplyID,GageUnit,TotalNum,CarWeight,WRate,InNum,InDate,Driver,AH,OutDate
                               ...
                               WeightName,MingWeight,RateMode,SupplyNum)
                            values( 
                            @StuffInID,
                            @StuffID,
                            @Spec,
                            ...
                            @ParentStuffInID,
                            @Proportion,
                            @OrderNum,@TransportID,@FootNum,@WeightName,@MingWeight,@RateMode,@SupplyNum); 
        ...
        private bool UpStuffIn(StuffIn record, int flag)
        {
            ...
            {
                sql = @"update StuffIn set StuffID = @StuffID,Spec = @Spec,SiloID = @SiloID,CarNo = @CarNo,SupplyID = @SupplyID,Lifecycle=1";
            }
        ...
        {
            string sql = m_Select + m_From + " WHERE InNum is null or InNum = 0 and lifecycle =0 ORDER BY indate desc,StuffINId DESC"; 
            IList<StuffIn> list;
            using (SqlDataReader sdr = SqlHelper.ExecuteReader(m_ERPConnString, System.Data.CommandType.Text, sql, null))
            {
        ...
        private IList<StuffSale> ReaderToStuffSaleList(SqlDataReader sdr)
        {
            ...
            while (sdr.Read())
            {
                ...
                record.StuffSaleID = sdr.IsDBNull(0) ? 0: sdr.GetInt32(0);
                ...
                //record.Modifier = sdr.IsDBNull(17) ? string.Empty : sdr.GetString(17);
        ...
        public IList<StuffIn> GetStuffInListDetail(string CarNo, string BaleId, string StuffId, string SupplyId,  string BeginTime, string EndTime)
        {
            string sql = @"SELECT StuffInID,CarNo,totalNum,CarWeight,InDate,OutDate,Operator,(SELECT TOP 1 stuffName FROM stuffInfo 
                            ...
                            WHERE SupplyInfo.SupplyID = StuffIn.TransportID) as TransportName,Driver,InNum,Pic1,Pic2,Pic3,Pic4,DarkWeight,FastMetage,Spec,ParentID,
                            (select top 1 FinalStuffType from StuffType inner join StuffInfo on StuffType.StuffTypeID = StuffInfo.StuffTypeID and StuffInfo.StuffID = StuffIn.StuffID) as FinalStuffType 
                            ,MingWeight,CompanyID,SourceNumber,CompName as CompanyName";//,TransferName,KZL 
            ...
            sql = sql + " from MetageStuffIn as StuffIn " + s_where;
            IList<StuffIn> list = new List<StuffIn>();
            using (SqlDataReader sdr = SqlHelper.ExecuteReader(m_ERPConnString, System.Data.CommandType.Text, sql, null))
            {
        ...
        public ShippingDocument GetShippingDocumentByCarId(string CarId)
        {
            ...
            sql += ", TotalWeight, ShippingDocument.CarWeight,ShippingDocument.Weight , CAST(ShippingDocument.Exchange AS decimal(18,2)) Exchange,cube,Driver,Tel,ShippingDocument.ImpGrade,DeliveryTime,Title,Car.Remark,WeightMan,ShipDocType,PlanCube,SlurryCount,ShippingCube";
            sql += ", LinkMan, Contract.Salesman ,isnull(CAST(Cons.Weight AS INT),0) as SlurryExchange";
        ...
        public int AddTzRelation(TZrelation tz)
        {
            string sql = @" insert into tzralation (SourceShipDocID ,SourceCube,Cube ,CarID,  IsAudit, IsCompleted ,
                                                    Builder, BuildTime, AH, TotalWeight, CarWeight, Weight ,Exchange,IsLock,ReturnType,TzRalationFlag)
                            values(@SourceShipDocID, @Cube,@Cube, @CarID, 0, 0 ,@Builder,getDate(),'Auto',@TotalWeight,@CarWeight,
                                   @Weight,@Exchange,0,@ReturnType,@TzRalationFlag )  select SCOPE_IDENTITY()";
            SqlParameter[] paras ={　
                                new SqlParameter("@SourceShipDocID", tz.SourceShipDocID),
                                new SqlParameter("@Cube",tz.Cube),
                                new SqlParameter("@CarID", tz.CarID),
        ...
        public bool UpdateShippingDocument2(ShippingDocument sd)
        {
            ...
            if (Convert.ToBoolean(Config.Ini.GetString(Config.Section.BaseSetting, "UseMetageCube", string.Empty)))
            {
                ...
                sql += "update shippingDocument set TotalWeight= " + sd.TotalWeight.ToString() + ", CarWeight = " + sd.CarWeight.ToString() + " ,Weight = " + sd.Weight + ",";
                sql += "Exchange=" + sd.Exchange + ",DeliveryTime='" + sd.DeliveryTime + "',Title='" + sd.Title + "',Cube =" + sd.Cube + ",ParCube=" + sd.Cube + ",CarID='" + sd.CarID
                    + "',SignInCube =" + sd.Cube + ",ProvidedTimes=" + sd.ProvidedTimes + ",ProvidedCube =  " + sd.ProvidedCube + ",Remark ='" + sd.Remark + "',WeightMan='" + sd.WeightMan + "',WeightName='" + sd.WeightName + "'";
                sql += " where shipDocId ='" + sd.ShipDocID + "' COMMIT TRANSACTION";

            }
            else
            {
                sql += "update shippingDocument set TotalWeight= " + sd.TotalWeight.ToString() + ", CarWeight = " + sd.CarWeight.ToString() + " ,Weight = " + sd.Weight + ",WeightMan='" + sd.WeightMan + "',WeightName='" + sd.WeightName + "',";
                sql += "Exchange=" + sd.Exchange + ",Cube =" + sd.Cube + "  where shipDocId ='" + sd.ShipDocID + "'";
            }
            return SqlHelper.ExecuteNonQuery(m_ERPConnString, System.Data.CommandType.Text, sql, null) > 0;
        }
       ...
       /// <returns></returns>
        ...
        public int getCarEmptyWeight(string CarID)
        {
            ...
            string sql = "select top 1 CarWeight from Car where CarID='" + CarID + "' order by CarID desc";
        ...
        {
            string sql = "select top 1 CarWeight from Car where CarNo='" + CarNo + "' order by LastBackTime desc";
            int? weight = Convert.ToInt32(SqlHelper.ExecuteScalar(m_ERPConnString, System.Data.CommandType.Text, sql, null));
            if (weight == null)
            {
                weight = 0;
