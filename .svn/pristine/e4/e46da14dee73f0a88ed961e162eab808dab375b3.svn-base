...
{
    public class ConsMixpropGHService: ServiceBase<ConsMixpropGH>
    {
        ControlSystemHelper controlSystem;
        internal ConsMixpropGHService(IUnitOfWork uow) : base(uow) {
        ...
        public void Audit(ConsMixpropGH consMixprop)
        {
            try
            {
                ConsMixpropGH cons = this.Get(consMixprop.ID);
        ...
        /// </summary>
        /// <param name="consMixprop"></param>
        public void BatchAudit(string[] ids,int auditStatus)
        {
            try
            {
                foreach (var id in ids)
                {
                    ConsMixpropGH cons = this.Get(id);
                    string auditor = AuthorizationService.CurrentUserID;
                    cons.AuditStatus = auditStatus;
                    //cons.AuditInfo = consMixprop.AuditInfo;
                    cons.Auditor = auditor;
                    cons.AuditTime = DateTime.Now;
                    base.Update(cons, null);
        ...
        public void UpdateAuditStatus(ConsMixpropGH entity)
        {
        ...
        public void UnAudit(string consMixID, int auditStatus)
        {
            try
            {
                ConsMixpropGH consmixpropGH = this.Get(consMixID);
                consmixpropGH.AuditStatus = auditStatus;
                consmixpropGH.Auditor = AuthorizationService.CurrentUserID;
                consmixpropGH.AuditTime = DateTime.Now;
                this.m_UnitOfWork.GetRepositoryBase<ConsMixpropGH>().Update(consmixpropGH, null);
        ...
        {
            return this.m_UnitOfWork.ConsMixpropGHRepository.CheckMesureScale(taskId, productLineId, concreteRZ, slurryRZ, checkConcrete, checkSlurry);
        }
        ...
        /// <returns></returns>
        public override IList<ConsMixpropGH> Find(JqGridRequest request, string condition, out int total)
        {
            IList<ConsMixpropGH> ConsMixpropList = base.Find(request, condition, out total);
            IList<ConsMixpropGH> ConsMixpropCount = base.All("ConsMixpropID in (select key_value from SynMonitor where key_name = 'ConsMixpropID' union select ConsMixPropID from ConsMixpropItemsGH where ConsMixpropItemsID in (select key_value from SynMonitor where key_name = 'ConsMixpropItemsID'))", "ID", true);
            foreach (ConsMixpropGH cm in ConsMixpropList)
            {
        ...
        public void SendModifiedPBToKZ(ConsMixpropGH entity, string[] DirtyDataArr)
        {
                ...
                try
                {
                    if (DirtyDataArr != null)
                    {
                        ...
                        foreach (string DirtyData in DirtyDataArr)
                        {
                            ...
                            ConsMixpropItemGHService consMixpropItemGHservice = new ConsMixpropItemGHService(this.m_UnitOfWork);
                            ConsMixpropItemGH item = consMixpropItemGHservice.Get(Convert.ToInt32(dirty[0]));
                            item.Amount = Convert.ToDecimal(dirty[1]);
                            consMixpropItemGHservice.Update(item, null);
                            ...
                            { 
                                ...
                                synTable.key_name = "ConsMixpropItemsID";
                                synTable.key_value = dirty[0];
                                synTable.key_type = "0";
                                synTable.action_time = DateTime.Now;
                                synTable.ProductLineID = item.ConsMixpropGH.ProductLineID;
                                if (op == null)
                    ...
                    IList<ConsMixpropItemGH> citems = IConItems.Query().Where(m => m.ConsMixpropID == entity.ID).ToList();
                    decimal totolWeight = 0;
                    foreach (ConsMixpropItemGH citem in citems)
                    {
                        totolWeight = totolWeight + citem.Amount;
                    }
                    ConsMixpropGH cons = this.Get(entity.ID);
                    ...
                    if (!entity.IsSlurry)
                    {
                        ...
                        if (config != null && bool.Parse(config.ConfigValue) && entity.ProduceTaskGH.CementType == Model.Enums.CementType.CommonCement)
                        {
                    ...
                    //发送配比
                    if (entity.SynStatus == 1)      
                    {
                        /*//直连:通知控制系统修改配比
                        by:Sky 2013/3/17
                        *****************************************************************************/
                        IList<ConsMixpropItemGH> cmItems = this.m_UnitOfWork.GetRepositoryBase<ConsMixpropItemGH>().All("ConsMixpropID='" + entity.ID + "'", "ID", true);
                        ResultInfo result = controlSystem.UpdateConsMixpropGH(entity, cmItems);
        ...
        public IList<string> UpdatePrimaryAndItems(ConsMixpropGH cm, NameValueCollection updateKeys) {
            ...
            using (var tx = this.m_UnitOfWork.BeginTransaction()) {
                try
                {
                    ...
                    foreach (KeyValuePair<string, decimal> kvp in needToUpdateSiloIdList) {
                        ConsMixpropItemGH cmi = this.m_UnitOfWork.ConsMixpropItemGHRepository.Query().Where(c => c.ConsMixpropID == _ConsMixpropID && c.SiloID == kvp.Key).FirstOrDefault();
                        cmi.Amount = kvp.Value;
                        this.m_UnitOfWork.ConsMixpropItemGHRepository.Update(cmi, null);
                    }
                    //检查容重与水泥、水的用量
                    IConsMixpropItemGHRepository IConItems = this.m_UnitOfWork.ConsMixpropItemGHRepository;
                    IList<ConsMixpropItemGH> citems = IConItems.Query().Where(m => m.ConsMixpropID == cm.ID).ToList();
                    decimal totolWeight = 0;
                    foreach (ConsMixpropItemGH citem in citems)
                    {
                        totolWeight = totolWeight + citem.Amount;
                    ...
                    if (!cm.IsSlurry)
                    {
                        ...
                        if (config != null && bool.Parse(config.ConfigValue) && cm.ProduceTaskGH.CementType == Model.Enums.CementType.CommonCement)
                        {
                            if (totolWeight > dFormulaRZMax || totolWeight < dFormulaRZMin)
                            {
                    ...
                    if (cm.SynStatus == 1)      //发送配比至搅拌楼（12版楼站）
                    {
                        ...
                        by:Sky 2013/3/17
                        *****************************************************************************/
                        IList<ConsMixpropItemGH> cmItems = this.m_UnitOfWork.GetRepositoryBase<ConsMixpropItemGH>().All("ConsMixpropID='" + cm.ID + "'", "ID", true);
                        ResultInfo result = controlSystem.UpdateConsMixpropGH(cm, cmItems);
                        if (!result.Result)       //配比没有发送成功
                        {
