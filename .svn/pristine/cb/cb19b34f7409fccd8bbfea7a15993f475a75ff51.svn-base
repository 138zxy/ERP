...
namespace ZLERP.Web.Controllers
{
    public class CarOilController : BaseController<CarOil, string>
    {
        ...
        public ActionResult GetOilPrice(string siloID)
        {
            decimal value = 0;
            var silo = this.service.GetGenericService<Silo>().Get(siloID);
            string TypeID = silo.StuffInfo.StuffTypeID;
            var now = DateTime.Now;
            var price = this.service.GetGenericService<CarOilPriceSetting>().Query().Where(t => t.OilType == TypeID && t.StartDate <= now).OrderByDescending(t => t.StartDate).FirstOrDefault();
            if (price != null)
            {
                value = Convert.ToDecimal(price.OilPrice);
            }
            return OperateResult(true, Lang.Msg_Operate_Success, value);
        }
        ...
        public ActionResult UpdateOilPrice(CarOilPrice CarOilPrice)
        {
            try
            {
                IList<CarOil> list = this.service.CarOil.Query().Where(p => p.AddDate >= CarOilPrice.BeginTime && p.AddDate <= CarOilPrice.EndTime
                    && p.StuffInfo.StuffTypeID == CarOilPrice.OilType
                    ).ToList();
                if (list.Count > 0)
                {
                    foreach (var a in list)
                    {
                        if (a.UnitPrice != CarOilPrice.Price)
                        {
        ...
        public ActionResult ConfirmOil0(decimal rate)
        {
            try
            {
                IList<CarOil> list = this.service.CarOil.Query().Where(p => p.Status == 0 ).ToList();
        ...
        public ActionResult ConfirmOil(decimal rate)
        {
            try
            {
                ...
                {
                    i.Status = 1;
                    this.service.CarOil.ConfirmOil(i);
                }
                if(list.Count==0)
                {
