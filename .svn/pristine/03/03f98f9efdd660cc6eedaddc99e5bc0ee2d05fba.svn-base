    ...
    public class StuffInService : ServiceBase<StuffIn>
    {
        ...
        public override void Delete(StuffIn entity)
        {
            try
            {
                ...
                {
                    throw new Exception("进货单[" + entity.ID + "]不是草稿状态，无法删除");
                }
                else
                {
                    base.Delete(entity);
            ...
            try
            {
                ...
                if (config != null)
                {
                    if (config.ConfigValue.ToString().ToUpper() == "TRUE")
                    {
                        ...
                        if (_si.Lifecycle==1||_si.Lifecycle==3)
                        {
                            nc.IsPositive = _si.Lifecycle == 1 ? true : false;
                ...
                if (entity.SiloID!=null)
                {
                    Silo silo = siloResp.Get(entity.SiloID);
                    entity.SiloName = silo.SiloName;
                }
        ...
        public override StuffIn Add(StuffIn entity)
        {
            using (IGenericTransaction tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
                {
                    ...
                    Silo silo = siloResp.Get(entity.SiloID);
                    entity.SiloName = silo.SiloName;
                    var list = this.m_UnitOfWork.GetRepositoryBase<StockPact>().Query().Where(a => a.SupplyID == entity.SupplyID).ToList();
                    //根据价格信息里设定的厂商数量跟净重来修改入库单的最终结算数量
                    if (list.Count > 0)
                    {
                        StockPact supl = list.First();
                        if (supl != null)
                        {
                            if (supl.FootFrom == 0)
                            {
                                entity.FootNum = entity.SupplyNum;
                                entity.FinalFootNum = entity.SupplyNum;
                            }
                            else
                            {
                                entity.FootNum = entity.InNum;
                                entity.FinalFootNum = entity.InNum;
                            }
                        }
                    }
                    else {
        ...
        public void Copy(string id)
        {
            ...
            {
                StuffIn obj = this.Get(id);
                StuffIn _o = (StuffIn)obj.Clone();
                _o.Lifecycle = 0;
                _o.ID = null;
                _o.M_BaleBalanceDels = null;
                _o.M_TranBalanceDels = null;
                _o.StockPactID = obj.StockPactID;
                Add(_o);
            }
            catch (Exception ex)
            {
        ...
        public void Comprice(string id)
        {
            using (IGenericTransaction tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
        ...
        public void StuffInChangeNum(string beginDate, string endDate, int FootFrom)
        {
            using (IGenericTransaction tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
                {
                    List<StuffIn> list = this.Find("OutDate >= '" + beginDate + "' and OutDate <='" + endDate+"'", 1, 3000, "OutDate", "DESC").ToList();
                    foreach (var item in list)
                    {
                        item.FinalFootNum = FootFrom == 0 ? item.InNum : item.SupplyNum;
                        item.FootNum = FootFrom == 0 ? item.InNum : item.SupplyNum;
                        base.Update(item, null);
                    }
                    tx.Commit();
