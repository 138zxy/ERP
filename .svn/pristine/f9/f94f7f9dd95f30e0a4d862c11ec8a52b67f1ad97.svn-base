function customerplanIndexInit(opts) {
    ...
    var myJqGrid = new MyGrid({
        ...
        dialogHeight: 650,
        showPageBar: true,
        storeCondition: condition + ' AND DataType=1 ',
        groupField: 'PlanDate',
        groupingView: {
        ...
        functions: {
            ...
            handleTomorrowPlan: function (btn) {
                var d = new Date();
                var tomorrow = opts.tomorrowDate; //d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + (d.getDate() + 1);
                var condition = "PlanDate >= '" + tomorrow + " 00:00:00' and PlanDate < '" + tomorrow + " 23:59:59'  and lifecycle<>-1 AND DataType=1 ";
                myJqGrid.refreshGrid(condition);
            },
            handleTodayPlan: function (btn) {
                var d = new Date();
                var today = d.format("Y-m-d");
                var condition = "PlanDate >= '" + today + " 00:00:00' and PlanDate < '" + today + " 23:59:59'  and lifecycle<>-1 AND DataType=1 ";
                myJqGrid.refreshGrid(condition);
            },
            handleAllPlan: function (btn) {
                myJqGrid.refreshGrid('1=1 AND DataType=1 ');
            },
            ...
            handleSZf: function (btn) {
                var condition = "lifecycle=-1 AND DataType=1 "; //已作废
                myJqGrid.refreshGrid(condition);
            }
        }
    });
...
function getConStrength(contractId) {
    ...
    if (gSysConfig.IsALLConStrength == "true") {
        swhere += " AND IsSH=1 AND ConStrengthCode IN (SELECT ConStrength FROM ContractItems WHERE ContractID='" + contractId + "')";
        }
