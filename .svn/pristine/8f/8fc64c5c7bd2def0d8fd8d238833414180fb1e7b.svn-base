...
namespace ZLERP.Web.Controllers.Attributes
{
    ...
    public class UrlAuthorizeAttribute : AuthorizeAttribute
    {
        ...
        public override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            {
                IList<SysFunc> userFuncs;

                //判断启用权限方案
                if (AuthorizationService.AuthScheme == "1")
                {
                    userFuncs = ps.User.GetUserFuncs(AuthorizationService.CurrentUserID);
                }
                else
                {
                    userFuncs = ps.User.GetUserFuncs2(AuthorizationService.CurrentUserID);
                }
