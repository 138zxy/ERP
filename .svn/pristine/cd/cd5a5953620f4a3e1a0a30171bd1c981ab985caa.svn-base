function taskAndFormulaInit(opts) {
    var TaskGrid = new MyGrid({
        renderTo: 'TaskGridDiv'
   		...
   		, { label: '客户编码', name: 'ContractGH.Customer.ID', index: 'ContractGH.Customer.ID', type: 'hidden', hidden: true }
        ...
        , { label: '客户名称', name: 'ContractGH.Customer.CustName', index: 'ContractGH.Customer.CustName', searchoptions: { sopt: ['cn']} }
        ...
        , { label: ' 强度', name: 'ConStrength', index: 'ConStrength', width: 60, align: 'center', searchoptions: { sopt: ['cn']} }
        , { label: ' 抗渗等级', name: 'ImpGrade', index: 'ImpGrade', width: 60, align: 'center' }
        , { label: ' 坍落度', name: 'Slump', index: 'Slump', width: 60, hidden: true }
        , { label: ' 用料时间', name: 'NeedDate', index: 'NeedDate', width: 120, formatter: 'datetime', searchoptions: { dataInit: function (elem) { $(elem).datetimepicker(); }, sopt: ['ge']} }
        , { label: ' 浇筑方式', name: 'CastMode', index: 'CastMode', width: 70 }
        , { label: ' 计划量', name: 'PlanCube', index: 'PlanCube', width: 70 }
        , { label: '已供车数', name: 'ProvidedTimes', index: 'ProvidedTimes', width: 60, align: 'right', sortable: false }
        , { label: '已供量', name: 'ProvidedCube', index: 'ProvidedCube', width: 60, align: 'right', sortable: false }
        ...
        , { label: ' 干混配比', name: 'BetonFormulaName', index: 'BetonFormula', width: 80 }
        , { label: ' 砂浆配比', name: 'SlurryFormula', index: 'SlurryFormula', hidden: true }
        , { label: ' 砂浆配比', name: 'SlurryFormulaName', index: 'SlurryFormula', width: 80, hidden: true }
        ...
        , { label: ' 施工单位', name: 'ConstructUnit', index: 'ConstructUnit' }
   		...
   		, { label: ' 干混类别', name: 'CementType', index: 'CementType' }
                        ...
                        if (confirm('批量下发配比存在风险，确定继续?')) {
                            TaskGrid.showWindow({
                                ...
                                afterLoaded: function () {
                                    if (csValue) {
                                        bindSelectData($("#BetonFormula1"),
                                                opts.listFormulaUrl,
                                                {
                                                    textField: 'FormulaGHName',
                                                    valueField: 'ID',
                                                    condition: "ConStrength like '%" + csValue + "%' and FormulaGHType='FType_GH' and Lifecycle > -1"
                                                }, function () { }
                                                );
                                        bindSelectData($("#SlurryFormula1"),
                                                opts.listFormulaUrl,
                                                {
                                                    textField: 'FormulaGHName',
                                                    valueField: 'ID',
                                                    condition: "((ConStrength like '%" + csValue + "%' and FormulaGHType='FType_GH') or FormulaGHType='FType_GH') and Lifecycle > -1"
                                                }, function () { }
                                                );
                                    }
                                },
                                buttons: {
                                    ...
                                    "确定": function () {
                                        var requestURL = "/ProduceTaskGH.mvc/MutiGenConsMix";
        ...
        if (formulaid.length > 0) {
            ...
            ajaxRequest(requestURL, postParams, true, function (response) {
                ...
                if (response.Result) {
                    ...
                    $("#BetonTag").val(response.Data[0].BetonTag);
                }
            });
        }
    });
    ...
    //lzl
    $("#SelectConStrength").bind('change', function () {
        //var csValue = $("#SelectConStrength").val();
        var csValue = $("#ConStrength").val();
        console.log(csValue);
    ...
    var ConsGrid = new MyGrid({
        renderTo: 'ConsGridDiv'
            ...
            , groupingView: { groupColumnShow: [false], groupText: ['<font style="color:blue;"><b>{0}</b></font>'] }
        //, groupingView: { groupColumnShow: [false], groupSummary: [true], groupText: ['<b>{0} - {1}</b>'] }
        //, editSaveUrl: "/ConsMixprop.mvc/Update"
		    , initArray: [
                ...
                //, { label: ' 站名', name: 'ProductLineName', index: 'ProductLineName', width: 150, align: 'center' }
                , { label: ' 搅拌线', name: 'ProductLineName', index: 'ProductLineName', sortable: false, width: 150 }
                ...
                , { label: ' 搅拌时间', name: 'MixingTime', index: 'MixingTime', align: 'center', width: 60, editable: true }
                , { label: ' 砂率', name: 'SCRate', index: 'SCRate', width: 50, align: 'right', hidden: true }
                , { label: ' 水灰比', name: 'WCRate', index: 'WCRate', width: 50, align: 'right', hidden: true }
            ...
            , functions: {
                ...
                handleBatchAudit: function (btn) {//批量审核
                    var taskrecord = TaskGrid.getSelectedRecord();
                    if (isEmpty(taskrecord.ID)) {
                        showMessage("提示：", "请选择一条任务单记录");
                        return false;
    ...
    window.handleAudit = function (id) {
        ...
        ConsGrid.showWindow({
            ...
            width: 400,
            height: 300,
            loadFrom: '/ConsMixpropGH.mvc/Index #PBAuditForm form',
            afterLoaded: function () {
                $("#ConsMixpropGH_ID").val(selectedRecord.ID);
                $("#ConsMixpropGH_FormulaName").val(selectedRecord.FormulaName);
                $("#ConsMixpropGH_AuditStatus").val(_auditstatus == 1 ? 0 : 1);
                $("#ConsMixpropGH_MixingTime").val(oldMixTime);
            },
            buttons:
                { "关闭": function () {
                    $(this).dialog("close");
                    ...
                    $.post(
                        "/ConsMixpropGH.mvc/Audit"
                        , { ID: id, AuditStatus: $("#ConsMixpropGH_AuditStatus").val(), AuditInfo: $("#ConsMixpropGH_AuditInfo").val(), MixingTime: $("#ConsMixpropGH_MixingTime").val() }
                        , function (data) {
                            if (!data.Result) {
                                showError("出错啦！", data.Message);
                                return false;
                            } else {
                                if (oldMixTime != $("#ConsMixpropGH_MixingTime").val()) {
                                    showAlert("重要提示", "审核配比时修改了搅拌时间（由 <font color=red>" + oldMixTime + "</font> 改成了 <font color=green>" + $("#ConsMixpropGH_MixingTime").val() + "</font> ），请重新发送配比至搅拌楼！");
                                }
    ...
    //批量审核配比
    window.handleBatchAudit = function () {
        var _auditstatus = "";
        var oldMixTime = "";
        var WinTitle = "配比批量审核";
        ...
        ConsGrid.showWindow({
            ...
            width: 200,
            height: 150,
            loadFrom: '/ConsMixpropGH.mvc/Index #PBAuditForm2 form',
            afterLoaded: function () {
                $("#ConsMixpropGH_AuditStatus").val(1);
            },
            buttons:
                { "关闭": function () {
                    $(this).dialog("close");
                       ...
                       ajaxRequest(
                            "/ConsMixpropGH.mvc/BacthAudit",
                            {
                                ids: keys, auditStatus: $("#ConsMixpropGH_AuditStatus").val()
                            },
                            true,
                            function (response) {
                                if (!response.Result) {
            ...
            , functions: {
                ...
                , handleModifySave: function (btn) {
                    ...
                    if (synstatus == 1) {
                        ...
                        ajaxRequest(
                            "/ConsMixpropGH.mvc/SendModifiedPBToKZ",
                            {
                                "ID": ID,
                                "SynStatus": 1,
    ...
    TaskGrid.addListeners('rowdblclick', function (id, record, selBool) {
        //是否已下发
        var value = $("input[name='plgroup']:checked").val();
        $("#legend01")[0].innerHTML = value;
        if (record.IsFormulaSend == "false") {
        ...
        TaskGrid.handleEdit({
            ...
            afterFormLoaded: function () {
                ...
                if (csValue) {
                    //
                    var cs = _csValue.substring(index + 1, index + 3);
                    var csstr = "";
                    //csstr = "(LEN(ConStrengthCode)=4 AND ConStrengthCode LIKE '%X%') OR LEN(ConStrengthCode)=3 AND ConStrengthCode LIKE '%C%'";
                    csstr = " ConStrengthCode like 'm%' ";
                    getCons(csstr);

                    bindSelectData(TaskGrid.getFormField("BetonFormula"), //加载页面
                                        opts.listFormulaUrl,
                                        { textField: 'FormulaGHName',
                                            valueField: 'ID',
                                            condition: "ConStrength like '%" + csValue + "%' and FormulaGHType='FType_GH' and Lifecycle > -1"
                        ...
                        if (ID != null) {
                            ...
                            function (response) {
                                if (response.Result) {
                                    ...
                                    FormulaItemsGrid.refreshGrid("FormulaGHID='" + formulaid + "'");
                                }
                            }
                        );
                        }
        ...
        if (dirtydatalen > 0) {
            ...
            showConfirm("确认信息",
                        ...
                        function () {//否按钮事件
                            ...
                            FormulaItemsGrid.refreshGrid("FormulaGHID='" + record.FormulaID + "'");
...
//        alert(parseFloat(preWeight) + parseFloat(clValue));
    });
    ...
    //加载砼强度列表
    function getCons(level) {        
        bindSelectData($('[name="SelectConStrength"]'),
