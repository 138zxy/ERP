...
namespace ZLERP.Business
{
    public  class RoleService : ServiceBase<Role>
    {
        ...
        /// <returns></returns>
        public IList<Organization> GetRoleOrganizations(string roleId)
        {
            var role = this.Get(roleId);
            if (role != null)
            {
                var Organizations = VerifyOrganizations(role.Organizations);
        ...
        /// <param name="userIds"></param>
        /// <param name="powers"></param>
        public void SaveRoleOrganizations(string[] roleIds, string[] powers)
        {
            var organizationRepo = this.m_UnitOfWork.GetRepositoryBase<Organization>();
            using (var tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
                {

                    var organizations = organizationRepo.Query()
                        .Where(org => powers.Contains(org.ID.ToString()))
                        .ToList();

                    foreach (string rId in roleIds)
                    {
                        var roleInfo = this.Get(rId);
                        if (roleInfo != null)
                        {
                            roleInfo.Organizations.Clear();
                            roleInfo.Organizations = organizations;
                            this.Update(roleInfo, null);
                            //刷新用户权限缓存
                            CacheHelper.RemoveCache(string.Format(CacheKeys.RoleOrganizationFormatString, rId));
                        }
                    }
                    tx.Commit();
                }
