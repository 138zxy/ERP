...
namespace ZLERP.Web.Controllers
{
    public class UserController : BaseController<User, string>
    {
        ...
        //将user同步到工作流
        public ActionResult ToFlow(string[] userIds)
        {
            this.service.User.ToFlow(userIds);
            return OperateResult(true, Lang.Msg_Operate_Success, "");
        }


        //发送流程至工作流
        public ActionResult AddWorkFlow(string id, string flowid, string stepid, string stepname, string title)
        {
            try
            {
                ...
                wf.InstanceID = id;
                wf.Title = title;
                User u = AuthorizationService.CurrentUserInfo;
                wf.SenderID = u.DigitSign;
                wf.SenderName = u.TrueName;
                wf.ReceiveID = u.DigitSign;
                wf.ReceiveName = u.TrueName;
                this.service.User.AddWorkFlow(wf);
            }
            catch (Exception ex)
            {
                return HandleExceptionResult(ex);
            }
            return OperateResult(true, "发送成功", "");
        }
        ...
        /// <param name="userId"></param>
        /// <returns></returns>
        public JsonResult GetUserFuncsAPP(string userId)
        {
            return Json(this.service.User.GetUserFuncsAPP(userId));
        }

        public ActionResult SaveUserPowersAPP(string[] userIds, string[] powers)
        {
            //UNDONE:同时修改多用户权限的问题
            this.service.User.SaveUserAPPPowers(userIds, powers);
            return OperateResult(true, Lang.Msg_Operate_Success, "");
        }
        [HttpPost]
        public JsonResult CopyPowerAPP(string userId, string copyFrom)
        {
            this.service.User.CopyAPPPower(userId, copyFrom);
            return OperateResult(true, Lang.Msg_Operate_Success, "");
        }
