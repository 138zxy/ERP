...
namespace ZLERP.Business
{
    public class TZRalationService : ServiceBase<TZRalation>
    {
        ...
        public override TZRalation Add(TZRalation entity)
        {
            ...
            entity.DataType =s.ShippingDocument.Get(entity.SourceShipDocID).DataType;
        ...
        /// <returns></returns>
        public ShippingDocument Forward(string sourceShipDocID, string targetTaskId, bool isOriginComplete, bool isTrashOrigin, string returnReason, out string message, string Cube, string ReduceConStrength, string FreightSubsidy, string[] _carids)
        {
            //正在生产的不能转发
            ...
            if (oldDoc != null && task != null)
            {
                ...
                //newDoc.ContractNo = task.Contract.ContractNo;//lzl 加个合同号
                ...
                newDoc.ExceptionInfo = string.Format("CODEADD由运输单:{0}整车转发生成", sourceShipDocID);
                newDoc.IsProduce = oldDoc.IsProduce;

                newDoc.ID = null;
                newDoc.TaskID = targetTaskId;
                newDoc.Driver = oldDoc.Driver;

                newDoc.OtherDemand = task.OtherDemand;
                newDoc.DataType = oldDoc.DataType;
                ...
                DispatchListService dispatchService = new DispatchListService(this.m_UnitOfWork);
                oldDoc.DispatchLists = dispatchService.Query().Where(p => p.ShipDocID == oldDoc.ID).ToList();
                //wdm: 原运输单的调度不清空
                //oldDoc.DispatchLists = dispatchService.Query()
                //    .Where(p => p.ShipDocID == oldDoc.ID).ToList();

                IList<DispatchList> oldDocDispatchLists = dispatchService.Query().Where(p => p.ShipDocID == oldDoc.ID).ToList();
                //newDoc.DispatchLists.Clear();
                //newDoc.DispatchLists = null;
                newDoc.TZRalations = null;

                //wdm : 因为 newDoc 还没有持久化到数据， 所以 newDoc.ID 为空
                //foreach (var d in oldDoc.DispatchLists)
                //{
                //    d.ShipDocID = newDoc.ID;
                ...
                if (oldDoc.IsEffective)
                    oldDoc.ExceptionInfo = (string.IsNullOrEmpty(oldDoc.ExceptionInfo) ? "" : oldDoc.ExceptionInfo + " ") + "CODEADD整车转发";
                else
                    oldDoc.ExceptionInfo = (string.IsNullOrEmpty(oldDoc.ExceptionInfo) ? "" : oldDoc.ExceptionInfo + " ") + "CODEADD整车转发作废";
                ...
                TZRalation tzRelation = new TZRalation
                {
                    ...
                    Cube = tagCube > 0 ? tagCube : oldDoc.ParCube,
                ...
                using (IGenericTransaction tx = this.m_UnitOfWork.BeginTransaction())
                {
                    try
                    {
                        ...
                        IRepositoryBase<DispatchList> dispatchListRepo = this.m_UnitOfWork.GetRepositoryBase<DispatchList>();
                        foreach (var d in oldDocDispatchLists)
                        {
                            d.ShipDocID = sourceShipDocID;
                            d.SynStatus = 1;
                            d.IsCompleted = true;
                            dispatchService.Update(d, null);
                            var newDispatchList = (DispatchList)(d.Clone());
                            ...
                            newDispatchList.DataType = newDoc.DataType;
        ...
        public override void Delete(TZRalation entity)
        {
            ...
            if (entity.ReturnType == "RT0")
            {
                ...
                if (!string.IsNullOrEmpty(sourceShippingDocument.ExceptionInfo) && sourceShippingDocument.ExceptionInfo.IndexOf("CODEADD整车转发") >= 0)
                    sourceShippingDocument.ExceptionInfo = sourceShippingDocument.ExceptionInfo.Remove(sourceShippingDocument.ExceptionInfo.IndexOf("CODEADD整车转发"));

                ShippingDocument targetShippingDocument = entity.TargetShippingDocument;
                targetShippingDocument.IsEffective = false;
                targetShippingDocument.ExceptionInfo = (string.IsNullOrEmpty(targetShippingDocument.ExceptionInfo) ? "" : targetShippingDocument.ExceptionInfo + " ") + "整车转发记录被删除";
            ...
            using (IGenericTransaction tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
                {
                    ...
                    foreach (var item in sourceShipDocIDArr)//合车：源运输单号用逗号拼接，如：001,002
                    {
                        if (item != "")
                        {
                            TZRalation tz = this.Get(Convert.ToInt32(item));
                    ...
                    if (unComplete != null)//本身有转退料，修改源
                    {
                        ...
                        cube += unComplete.Cube;//方量=原来的+目标方量
                        ...
                        if (unComplete.ActionType == Model.Enums.ActionType.Reject)//报废 改已完成
                    ...
                    for (int i = 0; i < sourceShipDocIDArr.Length; ++i)
                    {
                        if (!string.IsNullOrEmpty(sourceShipDocIDArr[i]) && !string.IsNullOrEmpty(sourceCubeArr[i]) && !string.IsNullOrEmpty(returnTypeArr[i]))
                        {
                            TZRalation tz = this.Query()
                                .Where(p => p.ID == Convert.ToInt32(sourceShipDocIDArr[i]) && p.IsLock != "3" && p.IsCompleted == false)
                                .FirstOrDefault();
                            if (tz == null)
                                continue;
                            ...
                            //tz.ActionType = actionType;
                            //tz.IsLock = "2";//合车锁定
                            //if (tz.ActionType == Model.Enums.ActionType.Reject)//报废 改已完成
                            //    tz.IsCompleted = true;
                            //else
                            //    tz.IsCompleted = false;
                            //tz.ReturnType = returnTypeArr[i];
