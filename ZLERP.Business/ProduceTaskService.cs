...
namespace ZLERP.Business
{
    public class ProduceTaskService : ServiceBase<ProduceTask>
    {
        ...
        public Project CreateProject(ProduceTask tempTask)
        {
            ...
            //}
            Contract con = new ContractService(this.m_UnitOfWork).Get(tempTask.ContractID);
            Customer cust = new CustomerService(this.m_UnitOfWork).Get(con.CustomerID);
            if (oldProject == null)
            {
                ...
                project.ConstructUnit = (tempTask.ConstructUnit == null || tempTask.ConstructUnit == "") ? con.CustName : tempTask.ConstructUnit;
                project.BuildUnit = tempTask.BuildUnit;
                project.LinkMan = tempTask.LinkMan;
                project.Tel = tempTask.Tel;
                project.Distance = tempTask.Distance;
                project.ContractID = tempTask.ContractID;
                return ps.Add(project);
            }
            else
            {
                oldProject.Tel = tempTask.Tel;
                oldProject.Distance = tempTask.Distance;
                oldProject.ProjectAddr = tempTask.ProjectAddr;
                oldProject.ConstructUnit = tempTask.ConstructUnit;
                ps.Update(oldProject, null);
                return oldProject;
            }
        ...
        /// <param name="isSlurry">是否为砂浆？</param>
        /// <returns>操作是否成功？</returns>
        public List<ConsMixprop> GenConsMix(string taskid, string formulaid, bool isSlurry, string[] plist, out string message)
        {
            using (IGenericTransaction tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var DispatchLists = this.m_UnitOfWork.GetRepositoryBase<DispatchList>().Query().Where(p => (p.TaskID == taskid && p.ProductLineID == item && p.IsRunning == true && p.IsCompleted == false)).ToList();
                        if (DispatchLists.Count == 0)
                        {
                            arraylist.Add(item);
                        }
                    }
                    if (plist.ToList().Count != arraylist.Count)
                    {
                        message = "正在生产的任务单不允许修改配比";
                    }
                    plist = (string[])arraylist.ToArray(typeof(string));
                    ...
                    taskObj.CarpRadii = formulaObj.CarpRadii;//骨料粒径 从理论配比插入到任务单
                    base.Update(taskObj, null);
                    foreach (string pl in plist)
                    {
                        ProductLine pline = productLines.FirstOrDefault(p => p.ID == pl);
                        ...
                        if (list.Count > 0)
                        {
                            ...
                            temp.SeasonType = formulaObj.SeasonType;
                            temp.SCRate = formulaObj.SCRate;
                            temp.ADMRate = formulaObj.ADMRate;
                            temp.FlexStrength = formulaObj.FlexStrength;
                            temp.IsSlurry = isSlurry;
                            ...
                            foreach (FormulaItem fi in formulaItems)
                            {
                                ...
                                if (sp.Silo != null && sp.Silo.StuffInfo != null)
                                {
                                    if (sp.Silo.StuffInfo.StuffType != null && (sp.Silo.StuffInfo.StuffType.ID == fi.StuffTypeID || ((num == formulaItems.Count) && !flag)))
                                    {
                                        ...
                                        item.ConsMixpropID = cm.ID;
                                        item.SiloID = sp.Silo.ID;
                                        decimal iamount = amount * (decimal)sp.Rate;//材料用量*比率
                                        item.Amount = iamount;
                                        int ordernum = sp.OrderNum;
                                        Type cmType = cm.GetType();
                                        if (null != cmType.GetProperty(string.Format("S{0}_wet", ordernum).ToString()))
                        ...
                        //判断自动审核的施工配比是否满足原材料审核条件：混凝土跟砂浆包含水、水泥、细骨料
                        if (base.GPSSwitch("MixAuditStuff") && base.GPSSwitch("IsAutoAuditForConsMixprop"))//施工配比要求审核原材料 && 自动审核 
                        {
                            foreach (var item in tmplist)
                            {
                                ConsMixprop cons = ICons.Get(item.ID);
                                List<ConsMixpropItem> list_WA = IConItems.Query().Where(a => a.ConsMixpropID == item.ID && a.Amount > 0
                                    && a.Silo.StuffInfo.StuffType.FinalStuffType == "WA").ToList();
                                List<ConsMixpropItem> list_CE = IConItems.Query().Where(a => a.ConsMixpropID == item.ID && a.Amount > 0
                                    && a.Silo.StuffInfo.StuffType.FinalStuffType == "CE").ToList();
                                List<ConsMixpropItem> list_FA = IConItems.Query().Where(a => a.ConsMixpropID == item.ID && a.Amount > 0
                                    && a.Silo.StuffInfo.StuffType.FinalStuffType == "FA").ToList();
                                //原材料类型判断 混凝土 "CA,CE,WA" //砂浆 "CE,WA"
                                if (list_WA.Count <= 0 || list_CE.Count <= 0 || list_FA.Count <= 0)
                                {
                                    cons.AuditStatus = 0;
                                    cons.AuditTime = null;
                                    cons.Auditor = "";
            ...
            if (task != null)
            {
                using (IGenericTransaction tx = this.m_UnitOfWork.BeginTransaction())
                {
                    ...
                    if (identities != null)
                    {
                        ...
                        task.BetonTag = task.ConStrength;

                        foreach (var a in list)
                        {
                            temp = temp + a.IdentityName + ",";
                            task.BetonTag = task.BetonTag + a.IdentityName;
                        }
                        if (temp != "")
                        {
                            temp = temp.Substring(0, temp.Length - 1);
                        }
                        task.IdentityValue = temp;
                    }
                    try
                    {
        ...
        public IList<ProduceTask> GetRangeTimeTasks(JqGridRequest request,string con)
        {
            string startDate;
            string endDate;
            ...
            SysConfigService svr = new SysConfigService(this.m_UnitOfWork);
            svr.GetTodayDateRange(out startDate, out endDate);
            string condition = "AuditStatus = 1 AND IsCompleted = 0 AND FormulaStatus > 0 AND NeedDate>='" + startDate + "' AND NeedDate < '" + endDate + "'" + con;
        ...
        public IList<ProduceTask> GetNotRangeTimeTasks(JqGridRequest request, string con)
        {
            string startDate;
            string endDate;
            ...
            SysConfigService svr = new SysConfigService(this.m_UnitOfWork);
            svr.GetTodayDateRange(out startDate, out endDate);
            string condition = "NeedDate>=(getdate()-365) and AuditStatus = 1 AND IsCompleted = 0 AND FormulaStatus > 0 AND (NeedDate<'" + startDate + "' OR NeedDate>='" + endDate + "')" + con;
                ...
                try
                {
                    ...
                    foreach (PropertyInfo pro in pros)
                    {
                        if (pro.Name == "ContractID" || pro.Name == "ContractItemsID" || pro.Name == "ConStrength" || pro.Name == "ProjectID" || pro.Name == "ProjectAddr"
                            ...
                            || pro.Name == "ConsPosType" || pro.Name == "ConsPos" || pro.Name == "CarpGrade" || pro.Name == "TaskNo" || pro.Name == "ChildProject" || pro.Name == "IsSlurry" || pro.Name == "BetonTag" || pro.Name == "CementType"
        ...
        public bool IsWarpConsMixprop(ConsMixprop cons, out string mesage)
        {
            mesage = "";


            if (cons.Formula.MaxLimitWeight != null)
            {
                if (cons.Weight > cons.Formula.MaxLimitWeight && cons.Formula.MaxLimitWeight!=0)
                {
                    mesage = "容重大于最大容重：" + cons.Formula.MaxLimitWeight;
                    return true;
                }
            }
            if (cons.Formula.MinLimitWeight != null)
            {
                if (cons.Weight < cons.Formula.MinLimitWeight)
                {
                    mesage = "容重小于最小容重：" + cons.Formula.MinLimitWeight;
                    return true;
                }
            }
            if (cons.ConsMixpropItems != null && cons.ConsMixpropItems.Count > 0)
            {
                decimal sum = 0;
                foreach (var item in cons.ConsMixpropItems)
                {
                    if (item.StuffTypeName.Contains("水泥") || item.StuffTypeName.Contains("矿粉"))
                    {
                        sum = sum + item.Amount;
                    }
                }
                if (cons.Formula.MaxSKLimitWeight != null)
                {
                    if (sum > cons.Formula.MaxSKLimitWeight && cons.Formula.MaxSKLimitWeight!=0)
                    {
                        mesage = "水泥矿粉容重大于最大容重：" + cons.Formula.MaxSKLimitWeight;
                        return true;
                    }
                }
                if (cons.Formula.MinSKLimitWeight != null)
                {
                    if (sum < cons.Formula.MinSKLimitWeight)
                    {
                        mesage = "水泥矿粉容重小于最小容重：" + cons.Formula.MinSKLimitWeight;
                        return true;
                    }
                }
            }
            ...
            //获取当前的施工配比
            var FormulaItems = cons.Formula.FormulaItems.Where(t => t.WarpAmount != 0).ToList();
            ...
            if (cons.ConsMixpropItems != null && cons.ConsMixpropItems.Count > 0)
            {
                ...
                foreach (var fitem in FormulaItems)
                {
                    ...
                    {
                        decimal acount = 0;
                        foreach (var citem in cons.ConsMixpropItems)
                        {
