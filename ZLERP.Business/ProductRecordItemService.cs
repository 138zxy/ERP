...
namespace ZLERP.Business
{
    public class ProductRecordItemService : ServiceBase<ProductRecordItem>
    {
        ...
        public decimal GetCostByStuff(string siloid, string stuffid, DateTime beginDate, DateTime endDate)
        {
            var cost = this.Query().Where(t => t.SiloID == siloid && t.StuffID == stuffid && t.BuildTime >= beginDate && t.BuildTime < endDate).Sum(t => t.ActualAmount);
            var cost1 = this.m_UnitOfWork.GetRepositoryBase<ManualProductItems>().Query().Where(t => t.SiloID == siloid && t.StuffID == stuffid && t.BuildTime >= beginDate && t.BuildTime < endDate).Sum(t => t.ActualAmount);
            var cost2 = this.m_UnitOfWork.GetRepositoryBase<HandleRecord>().Query().Where(t => t.SiloID == siloid && t.StuffID == stuffid && t.BuildTime >= beginDate && t.BuildTime < endDate).Sum(t => t.StuffAmount);

            return cost ?? 0 + cost1 ?? 0 + cost2 ?? 0;
        }
    }
}
