...
namespace ZLERP.Business
{
    public class ConsMixpropService: ServiceBase<ConsMixprop>
    {
                        ...
                        if (FormulaItems != null && FormulaItems.Count > 0)
                        {
                            ...
                            foreach (var fitem in FormulaItems)
                            {
                                if (fitem.WarpAmount != 0)
                                {
                                    ...
                                    {
                                        string mesage = "材料类型：" + fitem.StuffType.StuffTypeName + "的用量超过理论配比要求的允许范围";
        ...
        /// <returns></returns>
        public bool BatchEditConsMixpropItems(string consid, string taskid, string pid)
        {
            return this.m_UnitOfWork.ConsMixpropRepository.BatchEditConsMixpropItems(consid, taskid, pid);
        }
    }
}
