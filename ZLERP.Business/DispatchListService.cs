    ...
    public class DispatchListService : ServiceBase<DispatchList>
    {
        ...
        public SchedulerViewModel SaveScheduler(SchedulerViewModel entity)
        {
            ...
            using (var tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
                {
                    ...
                    document.TotalProduceCube = dispatch.BetonCount + dispatch.SlurryCount;
                    document.SendBetonCount = dispatch.BetonCount;
                    document.SendSlurryCount = dispatch.SlurryCount;
                    ...
                    task.OtherDemand = document.OtherDemand;
                    task.Remark = document.Remark;
        ...
        public ShippingDocument SaveShippingDocument(ShippingDocument document)
        {
            try
            {
                    ...
                    if (config != null)
                    {
                        switch (config.ConfigValue)
                        {
                            case "0":
                                ...
                                break;
                            case "1":
                                document.ConStrength = String.IsNullOrEmpty(task.BetonTag) ? task.ConStrength : task.BetonTag + "砂浆";//砼强度打印“任务单砼强度+砂浆”，如“C30砂浆”
                                break;
                            case "2":
                                document.ConStrength = String.IsNullOrEmpty(task.BetonTag) ? task.ConStrength : task.BetonTag;//砼强度仅打印“任务单砼强度”，如“C30P6”
                                break;
                        }
                    }
                ...
                {
                    document.ConStrength = String.IsNullOrEmpty(task.BetonTag) ? task.ConStrength : task.BetonTag;
                }
                ...
                document.Distance = task.Distance; 
                ...
                //湿拌的
                if (task.DataType == 1)
                    document.DataType = task.DataType;
                    ...
                    if (document.RemainCube > 0)
                    {
                        ...
                        if (obj == null)
                        {
                            ShippingDocument prevDoc = docRepository.Query()
                                ...
                                .OrderByDescending(d => d.ProduceDate).ToList()
                                .FirstOrDefault();
        ...
        private DispatchList DeleteFromOneProductLine(string id, string pid)
        {
            using (var tx = this.m_UnitOfWork.BeginTransaction())
            {
                try
                {
                    ...
                    newShipDoc.ExceptionInfo = "CODEADD该票由[" + document.ProductLineName + ":" + document.ID + "]转线生成";
                    //新增记录.
                    ...
                    if (contractStatus.Equals(Consts.ContractStatusBeginning))
                    {
                        switch (cubeLimit)
                        {
                            ...
                            case Consts.LimitMatCube:
                                ...
                                {
                                    _matcube = this.m_UnitOfWork.GetRepositoryBase<ShippingDocument>().Query()
