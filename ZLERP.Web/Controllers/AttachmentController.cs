...
{
    public class AttachmentController : BaseController<Attachment, int?>
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        public ActionResult UploadAttachment(string objectType, string uploadPath)
        {
             string objectId=AuthorizationService.CurrentUserID;
            //附件类型
                ...
                try
                {
                    ...
                    if (message.Count == 0)//如果没有异常信息则执行文件上传
                    {
                        ...
                        for (int i = 0; i < Request.Files.Count; i++)
                        {
                            ...
                            //判断是否存在，若存在则先删除再新增
                            List<Attachment> list = this.service.Attachment.All().Where(a => a.ObjectType == "AttachmentManage" && fileName==a.Title).ToList();
                            if (list != null&&list.Count>0)
                            {
                                foreach (var item in list)
                                {
                                    string FilePath = Server.MapPath(item.FileUrl);//转换物理路径
                                    ...
                                    this.service.Attachment.Update(item);
                                    //this.service.Attachment.Delete(item.ID);
                                }
                            }
                            else {
        ...
        [HttpPost]
        public  ActionResult DeleteAttachment(string ids)
        {
            try
            {
                List<Attachment> list2 = this.service.Attachment.All().Where(a => a.ObjectType == "AttachmentManage").ToList();   
                List<Attachment> list = this.service.Attachment.All().Where(a => a.ObjectType == "AttachmentManage" && ids.Contains(a.ID.ToString())).ToList();                 
                foreach (var item in list)
                {
                    string FilePath = Server.MapPath(item.FileUrl);//转换物理路径
                    if (System.IO.File.Exists(FilePath))//判断文件是否存在
