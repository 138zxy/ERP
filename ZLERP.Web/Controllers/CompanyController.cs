...
namespace ZLERP.Web.Controllers
{
    public class CompanyController : BaseController<Company, int>
    {
        ...
        {
            return Json(this.service.Company.All());
        }
        ...
        public override ActionResult Add(Company Company)
        {
            SysConfig SysConfig = base.service.SysConfig.GetSysConfig("IsOpenNC");
            if (SysConfig != null)
            {
                if (bool.Parse(SysConfig.ConfigValue) && (Company.Org_Purchaseorg == "" || Company.Org_Purchaseorg == null))
                {
                    return OperateResult(false, "您设置了NC必填，请填写NC采购组织！", null);
                }
            }
