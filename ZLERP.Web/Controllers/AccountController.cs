...
namespace ZLERP.Web.Controllers
{
    public class AccountController : BaseController<User,string>
    {
        ...
        public ActionResult IntoWorkFlow()
        {
            if (String.IsNullOrEmpty(AuthorizationService.CurrentUserInfo.DigitSign))
            {
                return OperateResult(false, "当前账号未开通工作流", "");
            }
            else {
                //this.service.User.IntoWorkFlow(AuthorizationService.CurrentUserInfo.DigitSign);
                string url = string.Concat(ConfigurationManager.AppSettings["workflowpath_IntoWorkFlow"], "?account=", AuthorizationService.CurrentUserInfo.DigitSign);
                return Redirect(url);
            }
        }
            ...
            if (userInfo != null)
            {
                ...
                if (userInfo.Password == AuthorizationService.EncryptPassword(user.Password))
                {
                    FormsAuthentication.SetAuthCookie(string.Format("{0},{1},{2}", userInfo.ID, userInfo.TrueName, user.YearAccountID == null ? "默认" : user.YearAccountID),
                        user.RememberMe);
                    //工作流登陆
                    Session["UserID"] = userInfo.DigitSign;
                    Response.Cookies.Add(new HttpCookie("UserID", userInfo.DigitSign) { Expires = DateTime.Now.AddDays(7) });
