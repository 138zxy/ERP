                 ...
                 $.post(
                    ...
                    function (resp) {
                        if (resp && resp.Result) {
                            ...
                            data['Accepter'] = resultData.Contract.Salesman; //业务员
                            $.post(
...
//            data.BetonTag = data.FormulaName; 
//        }
        //标记加上砂浆字样
      
//        if (data.BetonCount > 0 && data.SlurryCount > 0) {
//            data.BetonTag = data.BetonTag + "砂浆";
//        }
      
        if (data.ShipDocType == "1" || data.ShipDocType == "水票") {
            //水票
            data.ShipDocType = "水票";
            data.ProvidedTimes = data.ProvidedCube = data.ParCube = data.RealSlump = "";
            data.BetonTag = data.ConStrength;  //如果打印水票，可能会修改砼强度
