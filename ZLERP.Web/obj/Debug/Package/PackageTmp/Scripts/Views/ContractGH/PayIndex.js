...
function payIndexInit(options) {
    var ContractGrid = new MyGrid({
        ...
        //, singleSelect: true
            , initArray: [
                 { label: "系统编号", name: "ID", index: "ID", align: "center", width: 70 }
                , { label: "合同编号", name: "ContractNo", index: "ContractNo" }
                , { label: "工程名称", name: "ContractName", index: "ContractName" }
                ...
                , { label: "应收金额", name: "TotalComeMoney", index: "TotalComeMoney", width: 80, align: "right", hidden: false }
                , { label: "累计结算金额", name: "TotalJSMoney", index: "TotalJSMoney", width: 80, align: "right", hidden: true }
                , { label: "已收金额", name: "PayMoney", index: "PayMoney", width: 80, align: "right" }
                , { label: "未收余额", name: "Balance", index: "Balance", width: 60, align: "right", hidden: false }
                , { label: "累计开票金额", name: "TotalInvoiceMoney", index: "TotalInvoiceMoney", width: 60, align: "right", hidden: false }
                , { label: "授信额度", name: "CreditMoney", index: "CreditMoney", width: 60, align: "right", hidden: true }
                , { label: "上次结算日期", name: "FootDate", index: "FootDate", width: 70, formatter: "date", searchoptions: { dataInit: function (elem) { $(elem).datepicker(); }, sopt: ["ge"] }, hidden: true }
        ...
        //  , { label: "付款日期", name: "PaymentDate", index: "PaymentDate", formatter: "date", searchoptions: { dataInit: function (elem) { $(elem).datepicker(); }, sopt: ["ge"]} }               
            ...
            , functions: {
                ...
                handlePayReport: function (btn) {
                    if (ContractGrid.isSelectedOnlyOne()) {
                        var selectedRecord = ContractGrid.getSelectedRecord();
                            window.open("/Reports/Sales/R310425.aspx?uid=" + selectedRecord.ContractNo, 'top');
                        } else {
                            window.open("/Reports/Sales/R310425.aspx", 'top');
                        }
                },
                ...
                handlePay: function (btn) {
                    ...
                    ContractGrid.handleAdd({
                        ...
                        afterFormLoaded: function () {
                            ...
                            $("#ContractPay_ContractName").val(selectrecord.ContractName);
                            $("#ContractPay_ContractNo").val(selectrecord.ContractNo);
                ...
                handleAddInvoice: function (btn) {
                    ...
                    contractInvoiceGrid.handleAdd({
                        btn: btn,
                        width: 300,
                        height: 300,
                        loadFrom: "divInvoiceForm",
                        reloadGrid: false,
                        afterFormLoaded: function () {
                            contractInvoiceGrid.setFormFieldValue("ContractInvoice.ContractID", selectrecord.ID);
                        }
                        , postCallBack: function (response) {
                            contractInvoiceGrid.refreshGrid();
                            ContractGrid.refreshGrid();
                        }
                    });
                }
    ...
    var jstzGrid = new MyGrid({
        renderTo: "jstzGrid"
            ...
            , autoWidth: true
            , buttons: buttons1
            , height: gGridWithTitleHeight / 3 - 30
            , storeURL: "/ContractJSTZGH.mvc/Find"
            ...
            , dialogHeight: 300
            , singleSelect: true
            , editSaveUrl: "/ContractJSTZGH.mvc/Update"
            , initArray: [
                { label: "调整编号", name: "ID", index: "ID", hidden: true }
                , { label: "合同编号", name: "ContractID", index: "ContractID", hidden: true }
                , { label: "工程名称", name: "ContractName", index: "ContractName", hidden: true }
    ...
    var payGrid = new MyGrid({
        renderTo: "payGrid"
            ...
            , storeURL: "/ContractPayGH.mvc/Find"
            , sortByField: "PayDate"
            , sortOrder: "ASC"
            , primaryKey: "ID"
            , setGridPageSize: 50
        ...
        //, singleSelect: true
            ...
            , groupField: 'act'
            , editSaveUrl: "/ContractPayGH.mvc/Update"
            , initArray: [
                ...
                , { label: "创建人", name: "Builder", index: "Builder", width: 80, align: "center" }
                 , { label: "对方结算人", name: "OtherAccountMan", index: "OtherAccountMan", width: 80, align: "center", editable: true }
                  , { label: "联系电话", name: "OtherTel", index: "OtherTel", width: 80, align: "center", editable: true }
            ]
            , functions: {
                handleReload: function (btn) {
                    payGrid.reloadGrid();
    ...
    var ContractItemGrid = new MyGrid({
        renderTo: 'ContractItemGrid'
            ...
            , buttons: buttons1
            , height: gGridWithTitleHeight / 3
            , storeURL: '/ShippingDocumentGH.mvc/Find'
            ...
            , groupField: 'act'
            , initArray: [
                { label: '是否有效', name: 'IsEffective', index: 'IsEffective', width: 50, align: 'center', formatter: booleanFmt, unformat: booleanUnFmt, formatterStyle: { '0': '无效', '1': '有效' }, stype: 'select', searchoptions: { value: booleanSearchValues() }, hidden: true }
                ...
                , { label: '砼单价', name: 'price', index: 'price', width: 50, align: 'center' }
                , { label: '泵送单价', name: 'PumpPrice', index: 'PumpPrice', width: 60, align: 'center' }
                , { label: '金额', name: 'money', index: 'money', width: 70, align: 'center', summaryType: 'sum', summaryTpl: '{0}' }
                , { label: '运输单编号', name: 'ID', index: 'ID', width: 80 }
                , { label: '工程名称', name: 'ProjectName', index: 'ProjectName', width: 140 }
                , { label: '砼强度', name: 'ConStrength', index: 'ConStrength', width: 50, align: 'center' }
                , { label: '浇筑方式', name: 'CastMode', index: 'CastMode', width: 60, align: "center" }
                , { label: '出站时间', name: 'DeliveryTime', index: 'DeliveryTime', width: 120, formatter: 'datetime', hidden: false }
                , { label: '到场时间', name: 'ArriveTime', index: 'ArriveTime', width: 120, formatter: 'datetime', hidden: true }
            ]
            , functions: {
                handleReload: function (btn) {
    ...
    var productGrid = new MyGrid({
        renderTo: "productGrid"
            ...
            , buttons: buttons1
            , height: gGridWithTitleHeight / 3 - 30
            , storeURL: "/ContractJSGH.mvc/Find"
            ...
            , dialogHeight: 400
            , singleSelect: true
            , editSaveUrl: "/ContractJSGH.mvc/Update"
            , initArray: [
                { label: "编号", name: "ID", index: "ID", hidden: true }
                , { label: '操作', name: 'myac', width: 70, fixed: true, sortable: false, resize: false, formatter: 'actions',
                    formatoptions: { keys: true, editbutton: false }
                ...
                , { label: "合同编号", name: "ContractID", index: "ContractID", hidden: true }
                , { label: "工程名称", name: "ContractName", index: "ContractName", hidden: true, align: "center" }
    ...
    //发票网格
    var contractInvoiceGrid = new MyGrid({
        renderTo: "divContractInvoice"
        , title: "销售发票"
        , autoWidth: true
        //,buttons: buttons0
        //,buttonRenderTo: "buttonToolbar"
        , height: gGridWithTitleHeight / 3 - 60
        , dialogWidth: 200
        , dialogHeight: 700
        , storeURL: options.ContractInvoiceStoreUrl
        , storeCondition: ""
        , sortByField: "ID"
        , sortOrder: 'DESC'
        , primaryKey: "ID"
        ...
        , showPageBar: true
        , singleSelect: false
        , editSaveUrl: options.ContractInvoiceUpdateUrl
        , initArray: [
              { label: '操作', name: 'act', index: 'act', width: 60, sortable: false, align: "center" }
            , { label: "发票系统编号", name: "ID", index: "ID", width: 65 }
            , { label: "发票编号", name: "ContractInvoiceNo", index: "ContractInvoiceNo", align: "center", width: 80, hidden: false, editable: true }
            , { label: "合同系统编号", name: "ContractID", index: "ContractID", width: 80, editable: false, hidden: true }
            , { label: "发票金额", name: "InvoiceMoney", index: "InvoiceMoney", width: 80, editable: true }
            , { label: "税额", name: "TaxMoney", index: "TaxMoney", width: 80, editable: true }
            , { label: "发票日期", name: "InvoiceDate", index: "InvoiceDate", formatter: 'date', editable: true, editrules: { date: true }, width: 70 }
            , { label: "经办人", name: "Manager", index: "Manager", width: 80, editable: true }
        //, { label: "审核人", name: "Auditor", index: "Auditor", hidden: true }
        //, { label: "审核时间", name: "AuditTime", index: "AuditTime", formatter: "datetime", hidden: true }
        //, { label: "审核状态", name: "AuditStatus", index: "AuditStatus", formatter: dicCodeFmt, unformat: dicCodeUnFmt, formatoptions: { parentId: "AuditStatus" }, stype: "select", searchoptions: { value: dicToolbarSearchValues("AuditStatus") }, width: 100, align: "center" }
            , { label: "建立时间", name: "BuildTime", index: "BuildTime", formatter: "date", editable: false, width: 80, editrules: { date: true} }
            , { label: "建立人", name: "Builder", index: "Builder", editable: false }
            , { label: "修改时间", name: "ModifyTime", index: "ModifyTime", formatter: "date", editable: false, width: 80, editrules: { date: true} }
            , { label: "修改人", name: "Modifier", index: "Modifier", editable: false }
        ]
        , autoLoad: true
        , functions: {
            handleReload: function (btn) {
                contractInvoiceGrid.reloadGrid();
            },
            handleRefresh: function (btn) {
                contractInvoiceGrid.refreshGrid("1=1");
            }
        }
    });
    ...
    ContractGrid.addListeners("rowclick", function (id, record) {
        ...
        contractInvoiceGrid.refreshGrid("ContractID='" + id + "'");

        ContractItemGrid.getJqGrid().setCaption("应收明细：&nbsp;<font color='#2E6E9E'><strong>-></strong></font>&nbsp;" + record.CustName);
        ContractItemGrid.refreshGrid("ContractID='" + id + "' and IsEffective = 1");
    });
    //加载完成后事件
    ...
    contractInvoiceGrid.addListeners("afterSubmitCell", function () {
        ContractGrid.refreshGrid();
    ...
    contractInvoiceGrid.addListeners("gridComplete", function () {

        var ids = contractInvoiceGrid.getJqGrid().jqGrid('getDataIDs');
        for (var i = 0; i < ids.length; i++) {
            ce = "<div onmouseout=\"jQuery(this).removeClass('ui-state-hover');\" onmouseover=\"jQuery(this).addClass('ui-state-hover');\" onclick=\"handleDeletePay(" + ids[i] + ");\" class='ui-pg-div ui-inline-del' style='float:left;margin-left:5px;' title='删除所选记录'><span class='ui-icon ui-icon-trash'></span></div>";
            contractInvoiceGrid.getJqGrid().jqGrid('setRowData', ids[i], { act: ce });
