...
function produceTaskIndexInit(opts) {
    ...
    var mygrid = new MyGrid({
        ...
        , { label: '客户编码', name: 'CustomerID', jsonmap: 'ContractGH.Customer.ID', index: 'ContractGH.Customer.ID', type: 'hidden', hidden: true }
        , { label: '客户名称', name: 'CustName', jsonmap: 'ContractGH.Customer.CustName', index: 'ContractGH.Customer.CustName', searchoptions: { sopt: ['cn']} }
        , { label: '合同编码', name: 'ContractID', index: 'ContractGH.ID', type: 'hidden', hidden: true }
        , { label: '施工部位', name: 'ConsPos', index: 'ConsPos', width: 80, searchoptions: { sopt: ['cn']} }
        , { label: '坍落度', name: 'Slump', index: 'Slump', align: 'center', width: 50, hidden: true }
        , { label: '浇筑方式(干混)', name: 'CastMode', index: 'CastMode', width: 110 }
        , { label: '强度', name: 'ConStrength', index: 'ConStrength', align: 'center', width: 50, stype: 'select', searchoptions: { dataUrl: opts.conStrengthSelectUrl} }
        ...
        , { label: '标记', name: 'BetonTag', index: 'BetonTag' }
        , { label: '特殊要求', name: 'SpecialDemand', index: 'SpecialDemand', sortable: false }
        , { label: '品种', name: 'CementBreed', index: 'CementBreed' }
        ...
        , { label: ' 干混配比', name: 'BetonFormulaName', index: 'BetonFormulaInfo.FormulaName', width: 80 }
                            ...
                            afterFormLoaded: function () {
                                ...
                                bindSelectData(contractIdField,
                                        ...
                                        { textField: 'ContractName',
                                            valueField: 'ID',
                                            condition: "CustomerID='" + customerId + "' and ContractStatus!='3' and AuditStatus='1' AND IsLimited = 0 "
                                        }, function () {
                                ...
                                consPosTypeChange();
                                linkManChange();
                                mygrid.getFormField("PlanCube").val(""); //计划量和计划时间都设空
...
//                    alert(btn.data.Url);
    ...
    var taskHisGridInitArray = [
                ...
                , { label: '计划量', name: 'PlanCube', index: 'PlanCube', sortable: false, width: 60 }
                , { label: '计划车数', name: 'NeedTimes', index: 'NeedTimes', sortable: false, width: 60 }
                , { label: '泵车类型', name: 'PumpType', index: 'PumpType', sortable: false }
                , { label: '用料时间', name: 'NeedDate', index: 'NeedDate', sortable: false, formatter: 'date', width: 80, align: 'center', searchoptions: { dataInit: function (elem) { $(elem).datepicker(); }, sopt: ['ge']} }
                , { label: '强度', name: 'ConStrength', index: 'ConStrength', sortable: false, width: 50, align: 'center' }
                , { label: '坍落度', name: 'Slump', index: 'Slump', sortable: false, width: 60, hidden: true }
                , { label: '浇筑方式(干混)', name: 'CastMode', index: 'CastMode', sortable: false, width: 110 }
