...
function tzrelationInit(opts) {
    ...
    var myJqGrid = new MyGrid({
        renderTo: 'MyJqGrid'
            , autoWidth: true
            , buttons: buttons0
            , height: 740 //gGridHeight * 2 / 3
		    , storeURL: opts.storeUrl
		    , sortByField: " CASE WHEN ISNULL(ActionTime,'')='' THEN BuildTime ELSE ActionTime END DESC, BuildTime"
            , sortOrder: 'DESC'
		    , primaryKey: 'ID'
		    , setGridPageSize: 30
            , storeCondition: '1=1 AND DataType=1' //(TZValue == null || TZValue == '') ? '1=1' : ("ID='" + TZValue + "'")
            , dialogWidth: 600
            , singleSelect: true
            , dialogHeight: 600
		    , showPageBar: true
        ...
        //, groupingView: { groupColumnShow: [true], groupText: ['<b>{0}(<font color=red>{1}</font>)</b>'], groupOrder: ['desc'], groupSummary: [false], groupColumnShow: true, showSummaryOnHide: true, groupCollapse: false, minusicon: 'ui-icon-circle-minus', plusicon: 'ui-icon-circle-plus' }
		    , initArray: [
                ...
                , { label: '源运输单号', name: 'SourceShipDocID', index: 'SourceShipDocID', width: 180 }
                , { label: '源工程名称', name: 'SourceProjectName', index: 'SourceProjectName', search: false }
                , { label: '源运输车号', name: 'SourceCarID', index: 'SourceCarID', width: 80, search: false }
                , { label: '源剩退方量', name: 'SourceCube', index: 'SourceCube', width: 80, align: 'center' }
                , { label: '源砼强度', name: 'SourceConStrength', index: 'SourceConStrength', width: 80, search: false }
                ...
                , { label: '降标耗损', name: 'ReduceConStrength', index: 'ReduceConStrength', width: 80, align: 'center' }
                , { label: '运费补助', name: 'FreightSubsidy', index: 'FreightSubsidy', width: 80, align: 'center' }
                , { label: '是否需要统计', name: 'IsStatistics', index: 'IsStatistics', width: 60, formatter: booleanFmt, unformat: booleanUnFmt, hidden: true }
            ...
            , functions: {
               ...
               , handlePrint: function (btn) {
                ...
                //财务结算，修改降标耗损跟运费补助
                , hanldeFreightSubsidy: function (btn) {
    ...
    myJqGrid.addListeners('gridComplete', function () {
        ...
        for (var i = 0; i < ids.length; i++) {
            ...
            if (record.IsStatistics != null && record.IsStatistics == "true") {
                var $id = $(document.getElementById(ids[i]));
                $id.removeAttr("style");
                $id.removeClass("ui-widget-content");
                document.getElementById(ids[i]).style.backgroundColor = "#a4c2f4"; //设置行背景颜色
            }
        }
    });
    ...
    function bindCarChangeEvent(container) {
        $('#CarID', container).bind('change', function () {
            var carId = $(this).val(); 
            ajaxRequest(opts.getLastDocUrl, { carId: carId, dataType: 1 }, false, function (response) {
    ...
    //根据目标运输单显示相关信息
    TargetTaskChange = function () {
        var TargetTaskField = myJqGrid.getFormField("TargetTaskId");
        TargetTaskField.unbind('change');
        TargetTaskField.bind('change', function (event) {
            var TargetTaskId = $(this).val();
            document.getElementById('hiddenTargetTaskId').value = TargetTaskId;
            var carid = $('#CarID', '#forwardForm').val();
            if (!isEmpty(carid)) {
                var requestURL = '/TZRalation.mvc/GetTagCube';
                var postParams = { carid: carid, targetTaskId: TargetTaskId };
                var rData;
                ajaxRequest(requestURL, postParams, false, function (response) {
                    if (response.Result) {
                        rData = response.Data;
                        $("#Cube", '#forwardForm').val(rData == null ? "0" : rData.Cube);
                        $("#hiddenPrice", '#forwardForm').val(rData == null ? "0" : rData.ReduceConStrength);
                        var price = parseFloat(rData == null ? "0" : rData.ReduceConStrength);
                        var Cube = parseFloat(rData == null ? "0" : rData.Cube);
                        $("#ReduceConStrength", '#forwardForm').val(price * Cube);
                        $("#FreightSubsidy", '#forwardForm').val(rData == null ? "0" : rData.FreightSubsidy);
                    }
                });
            }
        });
    ...
    //根据目标车辆显示相关信息
    TargetCarIDChange = function () {
        $('#CarID', '#forwardForm').unbind('change');
        $('#CarID', '#forwardForm').bind('change', function (event) {
            var carId = $(this).val();
            var TargetTaskId = $('#hiddenTargetTaskId', '#forwardForm').val();
            ajaxRequest(opts.getLastDocUrl, { carId: carId ,dataType:1}, false, function (response) {
                $('table.info', '#forwardForm').remove(); //上车发货信息 移除
                //窗体高度、宽度
                $('#MyFormDiv').height(218);
                ...
                if (response.Result) {//AJAX成功
                    ...
                    $('#tmplShipDoc').tmpl(response.Data).insertAfter('table.mvcform', '#forwardForm'); //上车发货信息 插入下面
                    myJqGrid.setFormFieldValue('Driver', response.Data.Driver); //设置 当班司机
                    myJqGrid.setFormFieldValue('SourceShipDocID', response.Data.ID)
                }
                else {
                    showError('错误', isEmpty(response.Message) ? '未找到该车的发货记录' : response.Message);
                    $('#CarID', '#forwardForm').val('');
                }
            });
            console.log(TargetTaskId);
            if (!isEmpty(TargetTaskId)) {//如果已选运输单，再选择车号，则显示相关数据
                var requestURL = '/TZRalation.mvc/GetTagCube';
                var postParams = { carid: carId, targetTaskId: TargetTaskId };
                var rData;
                ajaxRequest(requestURL, postParams, false, function (response) {
    ...
    $('#Cube', '#forwardForm').blur(function () {
        var Cube = parseFloat($('#Cube', '#forwardForm').val());
        var price = parseFloat($('#hiddenPrice', '#forwardForm').val());
        price = isNaN(price) ? 0 : price;
        Cube = isNaN(Cube) ? 0 : Cube;
        $("#ReduceConStrength", '#forwardForm').val(price * Cube);
    });
