...
function contractIndexInit(options) {
    ...
    var ContractGrid = new MyGrid({
		...
		},
        {
            ...
            width: 60
                ...
                { label: '补方方量', name: 'BuCube', index: 'BuCube', width: 75, align: 'right', search: false },
                { label: '补方单价', name: 'BuPrice', index: 'BuPrice', width: 75, align: 'right', search: false },
                { label: '超时单价', name: 'OverTimePrice', index: 'OverTimePrice', width: 75, align: 'right', search: false },
		 {
		    label: "基准价",
		    name: "BasePrice",
		    index: "BasePrice",
        ...
        functions: {
            ...
            handleIsUseProject: function (btn) {
                if (!ProjectGrid.isSelectedOnlyOne()) {
                    showMessage("提示", "请选择一条工程记录进行操作");
                    return false;
                }
                var data = ProjectGrid.getSelectedRecord();
                var Lifecycle = 1;
                var UpIsUsed = false;
                var mes = "";
                if (data.IsUsed == false || data.IsUsed == "false") {
                ...
                showConfirm("确认信息", "您确定要<font color=green><b>" + mes + "</b></font>吗？", function () {
                    setIsUseProject(btn, ProjectGrid, UpIsUsed);
                    $(this).dialog("close");
                });
                //setIsUseProject(btn, ProjectGrid, true);
            },
            ...
            ,
            handleUpdatePrice: function (btn) {
                //var data = ContractGrid.getSelectedRecord();
                //var auditValue = data.AuditStatus;
                //if (auditValue == 1) {
                //    showMessage('提示', '已审核的数据不能修改');
    ...
    //工地置为未启用状态
    function setIsUseProject(b, grid, IsUsed) {
        if (!grid.isSelectedOnlyOne()) {
            showMessage('提示', '请选择一条记录进行操作！');
            return;
        }
        var selectedRecord = grid.getSelectedRecord();
        ajaxRequest(b.data.Url, {
            ID: selectedRecord.ID, IsUsed: IsUsed
        },
		true,
		function () {
		    grid.refreshGrid();
