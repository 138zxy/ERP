...
function dispatchListIndexInit(options) {
    ...
    function DrawTaskInMap(data, map) {
        ...
        ajaxRequest('ProjectGH.mvc/Get', { id: data.ProjectID }, false, function (response) {
            if (response) {
    ...
    //设置字段必填颜色提示
    $('#DispatchListGH_ProductLineID', '#scdjFormDiv').addClass('text requiredval valid');
    $('#ShippingDocumentGH_ProjectName', '#scdjFormDiv').addClass('text requiredval valid');
    $('#DispatchListGH_CompanyID', '#scdjFormDiv').addClass('text requiredval valid');
    $('#ShippingDocumentGH_CarID', '#scdjFormDiv').addClass('text requiredval valid');
    ...
    var scrwGrid = new MyGrid({
        ...
        //  , groupField: 'IsFormulaSend'
		    , initArray: [
            { label: '操作', name: 'act', index: 'act', width: 90, sortable: false, align: "center", width: 50, search: false }
            , { label: '配比?', name: 'IsFormulaSend', index: 'IsFormulaSend', sortable: false, align: 'center', width: 30, formatter: booleanFmt, unformat: booleanUnFmt, search: false, hidden: true }
   		    , { label: '置顶', name: 'TopShow', index: 'TopShow', width: 30, searchoptions: { sopt: ['cn'] }, formatter: booleanFmt, unformat: booleanUnFmt }
            ...
            , { name: 'ProjectID', index: 'ProjectID', hidden: true }
   		    ...
   		    , { label: '强度', name: 'ConStrength', index: 'ConStrength', width: 50, align: 'center' }
   		    , { label: '浇筑方式(干混)', name: 'CastMode', index: 'CastMode', width: 110 }
            ...
            , { label: '用料时间', name: 'NeedDate', index: 'NeedDate', width: 120, formatter: 'datetime', search: false }
            ...
            , { label: '含砂浆', name: 'IsSlurry', index: 'IsSlurry', width: 50, align: 'center', formatter: booleanFmt, unformat: booleanUnFmt, search: false, hidden: true }
   		    ...
   		    , { label: '坍落度', name: 'Slump', index: 'Slump', width: 70, search: false, hidden: true }
   		    ...
   		    , { label: '砂浆配比', name: 'SlurryFormulaName', index: 'SlurryFormulaName', search: false, hidden: true }
   		    , { label: '干混类别', name: 'CementType', index: 'CementType', width: 80, search: false }
   		    , { label: '干混配比', name: 'BetonFormulaName', index: 'BetonFormulaName', search: false }
            ...
            , functions: {
                ...
                , handleAddPreView: function (btn) {//保存预览
                    SaveDispatchPrint(btn, 'preview');
                    //scrwGrid.refreshGrid();
                }
                , handleAddPrint: function (btn) {//保存打印
                    SaveDispatchPrint(btn, 'printGH');
                    //scrwGrid.refreshGrid();
                }
                , printShippingDocument: function (btn) {//打印运输单票据
                    scrwGrid.handleAdd({
                        ...
                        postCallBack: function (response) {
                            if (response.Result) {
                                ...
                                if (!isEmpty(shipTemp)) {
                                    setShipDocTemplate("gh"+shipTemp);
                                }
                ...
                , printShippingDocument1: function (btn) {//打印其他票据
                    scrwGrid.handleAdd({
                ...
                , handleToday: function (btn) {//今日任务单
                    //  var d = new Date();
                ...
                , handleLastDay: function (btn) {//
                    ...
                    scrwGrid.setGridParam({ url: 'ProduceTaskGH.mvc/Find', postData: { condition: condition2} });

                    //scrwGrid.refreshGrid(condition2);
                }
                , handleAllUnCompleted: function (btn) {//所有未完成任务单
                    showUpdateTime = true;
		        ...
		        , handleIsOpen: function (btn) {
		            ...
		            scrwGrid.setGridParam({ url: 'ProduceTaskGH.mvc/Find', postData: { condition: condition} });
		        }
		        ...
		        //批量休息
        , handleProduceStop: function (btn) {
            scrwGrid.showWindow({
                title: "车队选择",
                width: 300,
                height: 150,
                loadFrom: 'RentCarNameForm',
                buttons: {
                    "确定": function () {
                        var RentCarNameID = $("#RentCarNameID").val();
                        $.ajax({
                            url: "/Car.mvc/RestCarByRentCarName",
                            dataType: "json",
                            type: "POST",
                            data: {
                                RentCarName: RentCarNameID, dataType: 2
                            },
		        ...
		        //批量开工
        , handleProduceOpen: function (btn) {
            scrwGrid.showWindow({
                title: "车队选择",
                width: 300,
                height: 150,
                ...
                buttons: {
                    "确定": function () {
                        var RentCarNameID = $("#RentCarNameID").val();
                        $.ajax({
                            url: "/Car.mvc/AllowShipCarByRentCarName",
                            dataType: "json",
                            type: "POST",
                            data: {
                                RentCarName: RentCarNameID,dataType:2
    ...
    scrwGrid.addListeners("gridComplete", function () {
        ...
        var records = scrwGrid.getAllRecords();
        var rid, istop;
        for (var i = 0; i < records.length; i++) {
            //for (var i = 0; i < ids.length; i++) {
            rid = records[i].ID;
            istop = records[i].TopShow;
            //var cl = ids[i];
            if (showUpdateTime) {
                ...
                //ce1 = "<div onmouseout=\"jQuery(this).removeClass('ui-state-hover');\" onmouseover=\"jQuery(this).addClass('ui-state-hover');\" onclick=\"handleUpdateTime('" + ids[i] + "');\" class='ui-pg-div ui-inline-del' style='width:16px;margin:auto;' title='置顶操作'><span class='ui-icon ui-icon-clock'></span></div>";
                ce = "<div onmouseout=\"jQuery(this).removeClass('ui-state-hover');\" onmouseover=\"jQuery(this).addClass('ui-state-hover');\" onclick=\"handleUpdateTime('" + rid + "');\" class='ui-pg-div ui-inline-del' style='float:left;margin-left:5px;' title='更新时间'><span class='ui-icon ui-icon-clock'></span></div>";
                de = "";
            }
            else {
                ...
                //ce1 = "<div onmouseout=\"jQuery(this).removeClass('ui-state-hover');\" onmouseover=\"jQuery(this).addClass('ui-state-hover');\" onclick=\"handleCompleted('" + ids[i] + "');\" class='ui-pg-div ui-inline-del' style='width:16px;margin:auto;' title='置顶操作'><span class='ui-icon ui-icon ui-icon-stop'></span></div>";
                ce = "<div onmouseout=\"jQuery(this).removeClass('ui-state-hover');\" onmouseover=\"jQuery(this).addClass('ui-state-hover');\" onclick=\"handleCompleted('" + rid + "');\" class='ui-pg-div ui-inline-del' style='float:left;margin-left:5px;' title='置为完成'><span class='ui-icon ui-icon-stop'></span></div>";
                de = "<div onmouseout=\"jQuery(this).removeClass('ui-state-hover');\" onmouseover=\"jQuery(this).addClass('ui-state-hover');\" onclick=\"handleTopShow('" + rid + "','" + istop + "');\" class='' style='float:left;margin-left:5px;' title='置顶操作'><span class='ui-icon arrowthick-1-n'></span>";
            }
            scrwGrid.getJqGrid().jqGrid('setRowData', rid, { act: ce + de });
        }
        if (rwid) {
    ...
    var shippDocGrid = new MyGrid({
        ...
        //, buttons: buttons0
            ...
            , storeCondition: " SHIPDOCTYPE in ('0','成品仓')"
        ...
        //, groupField: 'ProductLineName'
		    , initArray: [
                ...
                , { label: '其他量2', name: 'XuCube', index: 'XuCube', width: 40, align: 'right', search: false }
            ...
            , functions: {
                ...
                handleRefresh: function (btn) {
                    shippDocGrid.refreshGrid(" SHIPDOCTYPE in ('0','成品仓')");
                }
                //作废
    ...
    //转退料列表
    var TZGrid = new MyGrid({
        renderTo: 'insteadProductGrid'
            , title: '成品仓列表'
            , autoWidth: true
            , height: 120
		    , storeURL: '/FinishedGoodsWarehouseCount.mvc/Find'
		    , sortByField: 'ProductLineID'
            , sortOrder: 'ASC'
		    , setGridPageSize: 30
            , dialogWidth: 600
            , singleSelect: true
		    ...
		    , showPageBar: false
		    , initArray: [
                { label: '成品仓编号', name: 'ID', index: 'ID', width: 60, hidden: true  }
                , { label: '生产线', name: 'ProductLineID', index: 'ProductLineID', width: 80, sortable: false, align: "center" }
                , { label: '成品仓编号', name: 'SiloNo', index: 'SiloNo', width: 80, sortable: false, align: "center" }
                , { label: '成品仓名称', name: 'SiloName', index: 'SiloName', width: 80, sortable: false, align: "center" }
                , { label: '成品仓库存（吨）', name: 'StoreCount', index: 'StoreCount', formatter: Kg2TFmt, unformat: T2KgFmt, align: 'right', width: 100 }     
		    ]
		    , autoLoad: true
            , functions: {
                handleReload: function (btn) {
                    TZGrid.reloadGrid('1=1');
                },
                handleRefresh: function (btn) {
                    TZGrid.refreshGrid('1=1');
                }
            }
    });
    ...
    function SaveDispatchPrint(btn, printType) {
        ...
        var TaskID = scrwGrid.getFormFieldValue('ShippingDocumentGH.TaskID');
        if (TaskID.length == 0) {
            showError('提示', '请选择今日任务单！');
            return;
        }
        if (castmode != '非泵') {
            var pumpName = $("#ShippingDocumentGH_PumpName").val();
//            if (pumpName == "") {
//                showError('提示', '泵名称不能为空！');
//                return;
//            }
//            var pumpDriver = $("#ShippingDocumentGH_PumpDriver").val();
//            if (pumpDriver == "") {
//                showError('提示', '泵司机不能为空！');
//                return;
//            }
        }
        ...
        scrwGrid.handleFormSubmit({
            formId: 'scdj-form'
                ...
                , beforeSubmit: function () {
                    var pvalue = scrwGrid.getFormFieldValue('DispatchListGH.ProductLineID');
                    if (isEmpty(pvalue)) {
                        $('#DispatchListGH_ProductLineID', '#scdjFormDiv').addClass('input-validation-error'); //设置字段框红色提示                     
                        showError('提示', '请选择搅拌机组编号！');
                        return false;
                    }
                    var carId = scrwGrid.getFormFieldValue('ShippingDocumentGH.CarID');

                    if (isEmpty(carId)) {
                        $('#ShippingDocumentGH_CarID', '#scdjFormDiv').addClass('input-validation-error'); //设置字段框红色提示
                    ...
                    if (parseInt(tqmin) != 0) {
                        ...
                        if (record.Exception.replace(/(^\s+)|(\s+$)/g, "") == "" && record.OpenTime.replace(/(^\s+)|(\s+$)/g, "") == "" && needtime.setMinutes(needtime.getMinutes() + parseInt(tqmin), needtime.getSeconds(), 0) < now) {
                            scrwGrid.handleEdit({
                                ...
                                height: 220,
                                afterFormLoaded: function () {
                                    $("#ProduceTaskGH_ID").val(record.ID);
                                },
                                postUrl: '/ProduceTaskGH.mvc/Update',
                                postCallBack: function (response) {
                                }
                            });
                            scrwGrid.setFormId('scdj-form');
                    ...
                    var hntfs = scrwGrid.getFormFieldValue('DispatchListGH.BetonCount'); // 混凝土量
                    var sjfs = scrwGrid.getFormFieldValue('DispatchListGH.SlurryCount'); // 砂浆方量
                    var qtfs = scrwGrid.getFormFieldValue('ShippingDocumentGH.OtherCube'); //其他方量
                    var syfs = scrwGrid.getFormFieldValue('ShippingDocumentGH.RemainCube'); //剩余方量
                ...
                , postCallBack: function (response) {
                    if (response.Result) {
                        ...
                        scdjGrid2.reloadGrid();
                        if (!isEmpty(printType)) {
                            if (printType == 'printGH' && response.Data != null) {
                                //var shipurl = "/Reports/Produce/ShippingDocumentGH.aspx?id=" + response.Data + "&type=GH";
                                var shipurl = "/Reports/Produce/ReportPrint.aspx?id=" + response.Data + "&type=ShippingDocumentGH&isclose=true";
                                window.open(shipurl);
                            } else {
                                //printShippingDocGH(printType, response.Data);
                                var url = "/GridReport/PrintDirect.aspx?report=ShippingDocumentGH&ID=" + response.Data;
                                window.open(url, "_blank");
                            }
                        }
                        refreshCarStatus();
                        ...
                        if (response.Data == 0) {               //存在未处理的自动转退料记录，但是没有当前用户没有权限处理
                            ...
                            scrwGrid.handleAdd({
                                ...
                                height: 220,
                                afterFormLoaded: function () {
                                    $('#TZRalationGH_ReturnType').children('option[value=RT0]').remove();
                                    $("#TZRalationGH_CarID").val(recordTz.CarID);
                                    $("#TZRalationGH_Cube").val(recordTz.Cube);
                                    $("#TZRalationGH_ID").val(recordTz.ID);
                                    $("#TZRalationGH_Driver").val(recordTz.DriverUser);
                    ...
                    scrwGrid.resetForm();
                    scdjGrid2.reloadGrid();
                    shippDocGrid.refreshGrid(" SHIPDOCTYPE in ('0','成品仓') and TaskID='" + TaskID + "'", 'ID', 'DESC');
                    $("#parcube_red").html(0);
                }
        });
    }
    ...
    var scdjGrid2 = new MyGrid({
        ...
        //            , singleSelect: true
        //            , groupingView: {
        //                groupField: ['CompanyName', 'ProductLineName'],
        //                groupColumnShow: [true, true], //是否显示分组列
        //                groupText: ['<b>{0}(<font color=red>{1}</font>)</b>'], //表头显示数据(每组中包含的数据量)
        //                groupCollapse: [false, false], //加载数据时是否只显示分组的组信息
        //                groupSummary: [true, true], //是否显示汇总  如果为true需要在colModel中进行配置summaryType:'max',summaryTpl:'<b>Max: {0}</b>'
        //                groupOrder: ['asc', 'asc'], //分组后组的排列顺序
        //                minusicon: 'ui-icon-circle-minus', plusicon: 'ui-icon-circle-plus'
        //            }
        //            //,groupingView: { groupSummary: [true], groupText: ['笔数({1})'] }
        //            ,groupField: ['ProductLineName'],groupColumnShow: [true, true], groupText: ['<b>{0}(<font color=red>{1}</font>)</b>'], groupOrder: ['asc', 'asc'], groupSummary: [false, false], minusicon: 'ui-icon-circle-minus', plusicon: 'ui-icon-circle-plus' }
		    , title: '生产调度列表'
            , width: containerWidth * 0.5
        //, autoWidth: true
            , buttons: buttons1
            ...
            , initArray: [
                  { label: '发送状态', name: 'SendStatus', index: 'SendStatus', formatter: sendStatusFmt, unformat: booleanUnFmt, width: 80, align: 'center', sortable: false }
                , { label: '生产线', name: 'ProductLineName', index: 'ProductLineName', sortable: false, width: 45 }, { label: '车号', name: 'CarID', index: 'CarID', width: 50, align: 'center', sortable: false }
                ...
                , { label: '生产量', name: 'ProduceCube', index: 'ProduceCube', width: 50, align: 'center', sortable: false }
                , { label: '累计车数', name: 'ProvidedTimes', index: 'ProvidedTimes', width: 50, align: 'center', sortable: false }
                , { label: '已供量', name: 'ProvidedCube', index: 'ProvidedCube', width: 50, align: 'center', sortable: false }
                , { label: '工程信息', name: 'ProjectName', index: 'ProjectName', width: 200, editable: true, formatter: renderProjectName, sortable: false }
                , { label: '出票量', name: 'ParCube', index: 'ParCube', width: 50, sortable: false, align: 'center', sortable: false }
                , { label: '计划量', name: 'PlanCube', index: 'PlanCube', width: 50, sortable: false, align: 'center', sortable: false }
                ...
                , { label: '强度', name: 'ConStrength', index: 'ConStrength', editable: true, hidden: true }
                , { label: '浇筑方式(干混)', name: 'CastMode', index: 'CastMode', editable: true, hidden: true }
                , { label: '施工部位', name: 'ConsPos', index: 'ConsPos', editable: true, hidden: true }
                , { label: '干混量', name: 'BetonCount', index: 'BetonCount', editable: true, editrules: { required: true }, sortable: false, width: 50 }
                , { label: '砂浆量', name: 'SlurryCount', index: 'SlurryCount', editable: true, editrules: { required: true }, sortable: false, width: 50, hidden: true }
                , { label: '剩余量', name: 'RemainCube', index: 'RemainCube', editable: true, editrules: { required: true }, sortable: false, width: 50 }
                , { label: '其他量', name: 'OtherCube', index: 'OtherCube', editable: true, editrules: { required: true }, sortable: false, width: 50 }
                , { label: '其他量2', name: 'XuCube', index: 'XuCube', editable: true, editrules: { required: true }, sortable: false, width: 70 }
                ...
                , { label: '公司名称', name: 'CompanyName', index: 'CompanyName' }
            ...
            , functions: {
                handleRefresh: function (btn) {
                    scdjGrid2.refreshGrid();
                },
                handleDelete: function (btn) {
                    if (scdjGrid2.isSelectedOnlyOne('请选择一条记录!')) {
                        var record = scdjGrid2.getSelectedRecord();
                        scdjGrid2.deleteRecord({
                            deleteUrl: btn.data.Url
                        , postCallBack: function (response) {
                            if (response.Result) {
                                shippDocGrid.refreshGrid(" SHIPDOCTYPE in ('0','成品仓') and  TaskID='" + record.TaskID + "' and " + getLatestDocscondition, 'ID', 'DESC');
                                scrwGrid.refreshGrid();
                                refreshCarStatus();
                            }
                        }
                ...
                , handleCompleted: function (btn) {
                    if (scdjGrid2.isSelectedOnlyOne('请选择一条记录!')) {
                        showConfirm("确认操作", "确认要置为完成?", function () {
                            var dispatchid = scdjGrid2.getSelectedKey();
                ...
                , handleSampling: function (btn) {
                    if (scdjGrid2.isSelectedOnlyOne('请选择一个车号!')) {
                        var dispatchid = scdjGrid2.getSelectedKey();
                    ...
                    if (scdjGrid2.isSelectedOnlyOne('请选择一条记录!')) {
                        ...
                        scdjGrid2.showWindow({
                            ...
                            , afterLoaded: function () {
                                ...
                                $("#exception-ProvidedTimes").val(scdjGrid2.getSelectedRecord().ProvidedTimes);
                                $("#exception-ProvidedCube").val(scdjGrid2.getSelectedRecord().ProvidedCube);
                                //修改已供车次，方量
                                $("#exception-providedbtn").bind('click', record, function (btn) {

                                    showConfirm("确认操作", "是否执行操作？", function () {
                                        ajaxRequest(options.changeProvidedUrl, { ID: scdjGrid2.getSelectedKey(),
                    ...
                    function (response) {
                        if (response.Result) {
                            ...
                            showConfirm('提示', '需要打印吗？', function () {
                                ...
                                var url = "/GridReport/PrintDirect.aspx?report=ShippingDocumentGH&ID=" + shipDocID;
                                window.open(url, "_blank");
                            });
                            refreshCarStatus();
                            dispatchrecord = scdjGrid2.getRecordByKeyValue(id);
                            rwid = dispatchrecord.TaskID;
                            scrwGrid.reloadGrid();
                            shippDocGrid.refreshGrid(" SHIPDOCTYPE in ('0','成品仓') and TaskID='" + rwid + "' and " + getLatestDocscondition, 'ID', 'DESC');
                        }
                        scrwGrid.resetForm();
                        scdjGrid2.refreshGrid();
    ...
    function refreshCarStatus() {
        console.log("refreshCarStatus");
        if ($('.carlist').length == 0) {
            clearInterval(gTimer_Car2);
            return;
        }
        var cid = scrwGrid.getFormFieldValue('DispatchListGH.CompanyID');

        ajaxRequest(options.getCarStatusUrl, { id: cid }, false, function (response) {
            //加判断条件:防止response为null时，报错
            if (response == null || response == 'undefined') {
                return;
            }
            var AllowShipCarGH = response.AllowShipCar;
            var ShippingCarGH = response.ShippingCar;
            var RestCarGH = response.RestCar;
            if (AllowShipCarGH == null || AllowShipCarGH == 'undefined') {

            }
            else {
                //可调度车辆
                $('#AllowShipCarGH').empty();
                var allowShipHtml = '';
                for (var i = 0; i < AllowShipCarGH.length; i++) {
                    var car = AllowShipCarGH[i];
                    allowShipHtml += '<li data-itemid="' + car.ID + '"><div>' + car.ID + '</div></li>';
                }
                $('#AllowShipCarGH').append(allowShipHtml);

                //-----------------
                //                allowShipHtml += '<li title="' + car.ID + '" data-itemid="' + car.ID + '"><div>'

                //                if (car.PassNum != null && car.IsGG == true) {
                //                    allowShipHtml += '有 √';
                //                }
                //                else if (car.PassNum != null && car.IsGG == false) {
                //                    allowShipHtml += '有';
                //                }
                //                else if (car.PassNum == null && car.IsGG == true) {
                //                    allowShipHtml += ' √';
                //                }
                //                else {
                //                    allowShipHtml += car.MaxCube + '/' + car.GSCube;
                //                }
                //                allowShipHtml += '</div></li>'
                //---------------

            }

            if (ShippingCarGH == null || ShippingCarGH == 'undefined') {
            }
            else {
                //出货车辆
                $('#ShippingCarGH').empty();
                var shippingHtml = '';
                for (var i = 0; i < ShippingCarGH.length; i++) {
                    var car = ShippingCarGH[i];
                    shippingHtml += '<li data-itemid="' + car.ID + '"><div>' + car.ID + '</div></li>';
                }
                $('#ShippingCarGH').append(shippingHtml);
            }


            if (RestCarGH == null || RestCarGH == 'undefined') {
            }
            else {
                //休息车辆
                $('#RestCarGH').empty();
                var restHtml = '';
                for (var i = 0; i < RestCarGH.length; i++) {
                    var car = RestCarGH[i];
                    restHtml += '<li data-itemid="' + car.ID + '"><div>' + car.ID + '</div></li>';
                }
                $('#RestCarGH').append(restHtml);
                //scrwGrid.refreshGrid();
            }
        });
    }
    ...
    function setScdjForm(record) {
        ...
        ajaxRequest(options.getLastDocUrl, { id: taskid,dataType:1 }, false, function (response) {
            lastDocRecord = response;
            //设置remark/pumpname;
            var remark = response.Remark;
            var pumpName = response.PumpName;
            var pumpMan = response.PumpMan;
            var CustSiloNo = response.CustSiloNo;

            if (response.ProvidedCube == 0) {
                remark = record['Remark'];
                //pumpName = record['PumpName'];
            }
            scrwGrid.setFormFieldValue('ShippingDocumentGH.PumpName', pumpName);
            scrwGrid.setFormFieldValue('ShippingDocumentGH.PumpMan', pumpMan);
            scrwGrid.setFormFieldValue('ShippingDocumentGH.CustSiloNo', CustSiloNo);

            if (scrwGrid.getFormFieldValue('ShippingDocumentGH.Remark').toString().lenth > 0) {
                scrwGrid.setFormFieldValue('ShippingDocumentGH.Remark', remark);
            }
            //设置以供方量、累计车数.
            scrwGrid.setFormFieldValue('ShippingDocumentGH.ProvidedCube', response.ProvidedCube);
            scrwGrid.setFormFieldValue('ShippingDocumentGH.ProvidedTimes', response.ProvidedTimes);
            var plist = response.ProductLines;
            var clist = response.Company;
            //移除生产线下拉，重新填充.
            var pDropDownList = scrwGrid.getFormField('DispatchListGH.ProductLineID');
            pDropDownList.empty();
            var cDropDownList = scrwGrid.getFormField('DispatchListGH.CompanyID');
            cDropDownList.empty();
            var rateDropDownList = scrwGrid.getFormField('DispatchListGH.PCRate');
            rateDropDownList.empty();
            if (isEmpty(plist)) {
                showError('提示', '此任务单未下发配比或配比未审核！');
                return;
            }

            //解除car list disable,绑定事件
            var carField = scrwGrid.getFormField('ShippingDocumentGH.CarID');
            carField[0].disabled = false;
            carField.unbind("change");
            carField.bind('change', carListChange);

            //加载公司列表
            //cDropDownList.append("<option value=\"\"></option>");
            var companyArr = new Array();
            for (i = 0; i < clist.length; i++) {
                var id = clist[i].ID;
                var name = clist[i].CompName;
                //if (clist[i].IsProduction == true) {
                    cDropDownList.append("<option value=\"" + id + "\">" + name + "</option>");
                    companyArr[id] = clist[i];
                //}
            }
            var obj = document.getElementById("DispatchListGH_CompanyID"); //
            obj.selectedIndex = ind;
            var index = obj.selectedIndex; // 选中索引
            var value = obj.options[index].value; // 选中值
            companyChangeT(value);

            cDropDownList.unbind("change");
            cDropDownList.bind('change', companyArr, companyChange);

            //zyx,绑定泵车司机
            var pumpcarField = scrwGrid.getFormField('ShippingDocumentGH.PumpName');
            pumpcarField.unbind("change");
            pumpcarField.bind('change', pumpcarListChange);

            //获取泵名称对应司机
            var carid = pumpcarField.val();
            var driverList = scrwGrid.getFormField('ShippingDocumentGH.PumpDriver');
            driverList.empty();
            if (isEmpty(carid)) {
                return;
            }
            ...
            ajaxRequest(options.getCarInfoUrl, { id: carid }, false, function (response) {
                ...
                if (!isEmpty(users)) {
                    ...
                    for (var i = 0; i < users.length; i++) {
                        var user = users[i];
                        //driverList.append("<option value=\"\"></option>");
                        driverList.append("<option value=\"" + user.UserID + "\">" + user.TrueName + "</option>");
                    }
                }
            })
    ...
    function pumpcarListChange(select) {
        var carid = select.target.value;
        if (isEmpty(carid)) {
            return;
        }
    ...
    function carSelectAction(carid) {
        ...
        scrwGrid.setFormFieldReadOnly('DispatchListGH.BetonCount', false);
        var driverList = scrwGrid.getFormField('ShippingDocumentGH.Driver');
        driverList.empty();
        ajaxRequest(options.getCarInfoUrl, { id: carid }, false, function (response) {
            var users = response.Users;
            //设定容重，
            ...
            ajaxRequest(options.getRemaincubeUrl, { carId: carid }, false, function (response) {
                ...
                if (!response.Result)          //存在没有处理的自动转退料记录
                {
                    if (response.Data == null)              //当前没有处理转退料的权限
                    {
                        ...
                        scrwGrid.setFormId('scdj-form');
                        scrwGrid.setFormFieldValue('DispatchListGH.BetonCount', hntfs);
                        return;
                    }

                }

                scrwGrid.setFormFieldValue('DispatchListGH.BetonCount', formatFloat((hntfs - remainCube), 2));
                scrwGrid.setFormFieldValue('ShippingDocumentGH.RemainCube', remainCube);
    ...
    function calculatePrivided() {
        ...
        var current_cc = cc;
        var checksjfs = scrwGrid.getFormFieldValue('DispatchListGH.SlurryCount');
        var checkhntfs = scrwGrid.getFormFieldValue('DispatchListGH.BetonCount');
        var checkTLFL = scrwGrid.getFormFieldValue('ShippingDocumentGH.RemainCube');
        var checkQTFL = scrwGrid.getFormFieldValue('ShippingDocumentGH.OtherCube');
        ...
        scrwGrid.setFormFieldValue('ShippingDocumentGH.ProvidedCube', current_ygsl);
        //        alert(current_cc);
        scrwGrid.setFormFieldValue('ShippingDocumentGH.ProvidedTimes', current_cc);
        //增加超过计划方量提示
        var checkJHFL = scrwGrid.getFormFieldValue('ShippingDocumentGH.PlanCube');
        ...
        if (temp > 0) {
            showMessage('提示', '超过计划量' + temp.toFixed(2) + '方');
        }
    }

    //公司选择
    function companyChange(select) {
        var pid = select.target.value;
        var pDropDownList = scrwGrid.getFormField('DispatchListGH.ProductLineID');
        pDropDownList.empty();

        var carDropDownList = scrwGrid.getFormField('ShippingDocumentGH.CarID');
        carDropDownList.empty();

        if (isEmpty(pid)) return;

        refreshCarStatus();
        ajaxRequest("/ShippingDocumentGH.mvc/GetCarsByCompanyID", { id: pid }, false, function (response) {
            //carDropDownList.append("<option value=\"\"></option>");
            for (i = 0; i < response.length; i++) {
                var id = response[i].ID;
                carDropDownList.append("<option value=\"" + id + "\">" + id + "</option>");
            }
            if (response.length > 0) {
                carSelectAction(response[0].ID);
            }
        });

        ajaxRequest("/ShippingDocumentGH.mvc/GetProductLinesByCompanyID", { id: pid, taskid: taskId }, false, function (response) {
            pDropDownList.append("<option value=\"\"></option>");
            var productLineArr = new Array();
            for (i = 0; i < response.length; i++) {
                var id = response[i].ID;
                var name = response[i].ProductLineName;
                pDropDownList.append("<option value=\"" + id + "\">" + name + "</option>");
                productLineArr[id] = response[i];
            }
    ...
    function companyChangeT(select) {
        var pid = select;
        var pDropDownList = scrwGrid.getFormField('DispatchListGH.ProductLineID');
    ...
    function productLineChange(select) {
        ...
        //var driverList = $("#ShippingDocument_PumpDriver");
        //driverList.empty();
        ...
        var date = new Date();
        var MTime = dataTimeFormat(date, 'H:i');
        //ms = dataTimeFormat(time, 'Y-m-d'); alert(ms);
        ...
        var nowtoday = year + '-' + p(month) + "-" + p(date);
        var deliverytime = nowtoday + " " + MTime;
        scrwGrid.setFormFieldValue('ShippingDocumentGH.DeliveryTime', deliverytime);
    }
    ...
    */
    ...
    function DeliveryTimeFieldeProvided() {
        if (!isEmpty(scrwGrid.getFormFieldValue('DeliveryTime')) && !isEmpty(scrwGrid.getFormFieldValue('ShippingDocumentGH.DeliveryTime'))) {
            var y = "";
            var m = "";
            var ss = "";
            y = scrwGrid.getFormFieldValue('ShippingDocumentGH.DeliveryTime').substring(0, 10);
            m = scrwGrid.getFormFieldValue('DeliveryTime');
            ss = y + " " + m;
            scrwGrid.setFormFieldValue('ShippingDocumentGH.DeliveryTime', ss);
    ...
    scrwGrid.addListeners('cellclick', function (id, record, colIndex, cellcontent) {
        ...
        var OtherDemand = record['OtherDemand'];
        var IsLubricatingSlurry = record["IsLubricatingSlurry"];
        scrwGrid.setFormFieldValue('ShippingDocumentGH.ProjectName', gcmc);
        scrwGrid.setFormFieldValue('ShippingDocumentGH.Remark', OtherDemand);
        scrwGrid.setFormFieldValue('IsLubricatingSlurry', IsLubricatingSlurry);

        taskId = record['ID'];
        scrwGrid.setFormFieldValue('ShippingDocumentGH.TaskID', taskId);
        //车辆禁用，待请求结束后再enable。防止操作速度过快，累计量、车数 不对。
        scrwGrid.setFormFieldDisabled('ShippingDocumentGH.CarID', true);
        //scrwGrid.setFormFieldDisabled('ShippingDocument.Remark', true);
        //设置机组、绑定事件等。

        castmode = record['CastMode']; //浇筑方式

        setScdjForm(record);
        //是否禁用砂浆输入框
        var sjbool = record['IsSlurry'];
        if (sjbool) {
            scrwGrid.setFormFieldReadOnly('DispatchListGH.SlurryCount', false);
        } else {
            scrwGrid.setFormFieldReadOnly('DispatchListGH.SlurryCount', true);
        }
        var jhfl = record['PlanCube'];
        scrwGrid.setFormFieldValue('DispatchListGH.SlurryCount', 0);
        scrwGrid.setFormFieldValue('DispatchListGH.BetonCount', 0);

        scrwGrid.setFormFieldReadOnly('DispatchListGH.BetonCount', true);

        scrwGrid.setFormFieldValue('ShippingDocumentGH.RemainCube', 0);
        scrwGrid.setFormFieldValue('ShippingDocumentGH.OtherCube', 0);
        scrwGrid.setFormFieldValue('ShippingDocumentGH.PlanCube', jhfl);
        /*
        *不使用这种方式查看最近发车记录 xyl 2013-06-05
        if (colIndex > 7) {
        scdjGrid2.refreshGrid("TaskID='" + taskId + "' AND IsCompleted = 1", 'ID', 'DESC');
        //scdjGrid2.getJqGrid().jqGrid('setGridParam', { sortname: 'ID', sortorder: 'DESC', page: 1, postData: { condition: "TaskID='" + taskId + "' AND IsCompleted = 1"} });
        }
        */
    ...
    //几个量字段增加blur事件绑定。
    var hntfsField = scrwGrid.getFormField('DispatchListGH.BetonCount');
    var sjfsField = scrwGrid.getFormField('DispatchListGH.SlurryCount');
    var remainField = scrwGrid.getFormField('ShippingDocumentGH.RemainCube');
    var otherField = scrwGrid.getFormField('ShippingDocumentGH.OtherCube');
    ...
    //已选择的生产登记
    var sid = null;
    scdjGrid2.addListeners("rowclick", function (ids) {
        sid = ids;
    });
    scdjGrid2.addListeners("gridComplete", function () {
        if (sid) {
            scdjGrid2.getJqGrid().jqGrid("setSelection", sid);
        }
    });

    function scdjAutoRefresh() {
        scdjGrid2.refreshGrid();
        //scrwGrid.refreshGrid();//生产任务列表自动刷新
    ...
    function showConfirm1(title, msg, okFn, cancleFn) {
        gTimer_Remind2 = window.clearInterval(gTimer_Remind2);
        $('<div title="' + title + '"><p>' + msg + '</p></div>').dialog({
            ...
            buttons: {
                "取消": function (btn) {
                    gTimer_Remind2 = window.setInterval(remindRefresh, gSysConfig.SCDJRefreshInterval * 1000);
                    $(this).dialog("close");
                    if (cancleFn != null) {
    ...
    if (gSysConfig.SCDJRefreshInterval) {
        ...
        if (refreshInterval > 0) {
            if (gTimer_SCDJ2)
                clearInterval(gTimer_SCDJ2);
            gTimer_SCDJ2 = window.setInterval(scdjAutoRefresh, gSysConfig.SCDJRefreshInterval * 1000);
            //是否自动刷新车辆状态
            if (gSysConfig.IsRefreshCarStatus == "true") {
                if (gTimer_Car2) clearInterval(gTimer_Car2);
                gTimer_Car2 = window.setInterval(refreshCarStatus, gSysConfig.SCDJRefreshInterval * 1000);
            }
            if (gTimer_Remind2 != null)
                gTimer_Remind2 = window.clearInterval(gTimer_Remind2);
            gTimer_Remind2 = window.setInterval(remindRefresh, gSysConfig.SCDJRefreshInterval * 1000);
        }
    }
    ...
    if (gSysConfig.EnableCarDrag == "true") {
        $("#AllowShipCarGH").dragsort({ dragSelector: "div", dragBetween: true, placeHolderTemplate: "<li class='placeHolder'><div></div></li>", callBack: saveCarOrder });
    }

    function saveCarOrder(carId) {

        var data = $("#AllowShipCarGH li").map(function () { return $(this).attr("data-itemid"); }).get();
        ...
        ajaxRequest(options.getLastDocAutoByCarId,
                ...
                function (response) {
                    ...
                    if (response.Result) {
                        ...
                        scdjGrid2.setFormFieldValue('Driver', response.Data.Driver);
                        scdjGrid2.setFormFieldValue('SourceShipDocID', response.Data.ID)
                    }
    ...
    //置顶
    window.handleTopShow = function (id, istop) {
        if (istop == 0) {
            showConfirm("确认信息", "您确定要将该任务单置顶吗？", function () {
                ajaxRequest(
                '/ProduceTaskGH.mvc/UpdateTopShow',
                {
                    ids: id
                },
                true,
        ...
        if (istop == 1) {//取消置顶
            showConfirm("确认信息", "您确定要将该任务单【取消】置顶吗？", function () {
                ajaxRequest(
                '/ProduceTaskGH.mvc/CancelTopShow',
                {
                    ids: id
                },
                true,
