...
function dispatchListIndexInit(options) {
    ...
    var scrwGrid = new MyGrid({
        ...
        //, title: '生产任务列表'
		    ...
		    , initArray: [
            { label: '完工', name: 'act', index: 'act', width: 90, sortable: false, align: "left", width: 40, search: false }
        ...
        , { label: '计划车数', name: 'NeedTimes', index: 'NeedTimes', width: 55, align: 'right', search: false, hidden: true }
            ...
            , functions: {
                ...
                , handleAllUnCompleted: function (btn) {
                    scrwGrid.setGridParam({ url: 'ProduceTask.mvc/Find', postData: { condition: "IsCompleted = 0"} });
                }
    ...
    scrwGrid1.addListeners("gridComplete", function () {
        var ids = scrwGrid1.getJqGrid().jqGrid('getDataIDs');
        for (var i = 0; i < ids.length; i++) {
            var cl = ids[i];
            ce = "<div onmouseout=\"jQuery(this).removeClass('ui-state-hover');\" onmouseover=\"jQuery(this).addClass('ui-state-hover');\" onclick=\"handleUpdateTime('" + ids[i] + "');\" class='ui-pg-div ui-inline-del' style='float:left;margin-left:5px;' title='更新时间'><span class='ui-icon ui-icon-clock'></span></div>";
            wc = "<div onmouseout=\"jQuery(this).removeClass('ui-state-hover');\" onmouseover=\"jQuery(this).addClass('ui-state-hover');\" onclick=\"handleOtherCompleted('" + ids[i] + "');\" class='ui-pg-div ui-inline-del' style='float:left;margin-left:5px;' title='置为完成'><span class='ui-icon ui-icon ui-icon-circle-close'></span></div>";

            scrwGrid1.getJqGrid().jqGrid('setRowData', ids[i], { act: ce,act1:wc });
        }
        if (rwid) {
            scrwGrid1.getJqGrid().jqGrid("setSelection", rwid);
        }
    });
    ...
    scrwGrid.addListeners("gridComplete", function () {
        ...
        for (var i = 0; i < ids.length; i++) {
            var cl = ids[i];
            ce = "<div onmouseout=\"jQuery(this).removeClass('ui-state-hover');\" onmouseover=\"jQuery(this).addClass('ui-state-hover');\" onclick=\"handleCompleted('" + ids[i] + "');\" class='ui-pg-div ui-inline-del' style='float:left;margin-left:5px;' title='置为完成'><span class='ui-icon ui-icon ui-icon-circle-close'></span></div>";
        ...
        scrwGrid.handleFormSubmit({
            formId: 'scdj-form'
                ...
                , beforeSubmit: function () {
                    ...
                    var maxCube = scrwGrid.getFormFieldValue('MaxCube');
                    var hntfs = scrwGrid.getFormFieldValue('DispatchList.BetonCount');
                    var sjfs = scrwGrid.getFormFieldValue('DispatchList.SlurryCount');
                    var qtfs = scrwGrid.getFormFieldValue('ShippingDocument.OtherCube');
                    var syfs = scrwGrid.getFormFieldValue('ShippingDocument.RemainCube');
    ...
    function carSelectAction(carid) {
        ...
        ajaxRequest(options.getCarInfoUrl, { id: carid }, false, function (response) {
            ...
            //设定容重，
            var maxCube = response.MaxCube;
            scrwGrid.setFormFieldValue('MaxCube', maxCube);
            if (!isEmpty(users)) {
                //设定司机
                for (var i = 0; i < users.length; i++) {
                    var user = users[i];
    ...
    function productLineChange(select) {
        ...
        scrwGrid.setFormFieldValue('ShippingDocument.DeliveryTime', time);
    }
    ...
    scrwGrid1.addListeners('cellclick', function (id, record, colIndex, cellcontent) {
        $("#border-title").html("<b>生产调度</b>");
        scrwGrid.setFormFieldValue('ShippingDocument.TaskID', "");
        shippDocGrid.refreshGrid("TaskID='" + id + "'", 'ID', 'DESC');
        scrwGrid.refreshGrid(); //为了去除今日任务列表的选中事件
    });
    ...
    scrwGrid1.getJqGrid().jqGrid('filterToolbar', { stringResult: true, defaultSearch: 'cn' });
    ...
    $("#task-grid-toolbar").remove();
    $("#task-grid1-toolbar").remove();
    $("#latestShip-toolbar").remove();

    $("#task-grid-toolbar").remove();
    $("#task-grid1-toolbar").remove();
    $("#latestShip-toolbar").remove();

    $("a[href='#task-grid']").show();
    $("a[href='#task-grid1']").show();
    $("#scdjFormDiv").show();
    $(".carlist").show();
}
var gTimer_SCDJ;
