function taskAndFormulaInit(opts) {
    var TaskGrid = new MyGrid({
        ...
        , { label: '技术交底', name: 'Contract.IsJSJD', index: 'Contract.IsJSJD', width: 60, search: false, formatter: booleanFmt, unformat: booleanUnFmt }
        ...
        , { label: "级配", name: "CarpGrade", index: "CarpGrade", formatter: dicCodeFmt, unformat: dicCodeUnFmt, formatoptions: { parentId: "JP" }, stype: "select", searchoptions: { value: dicToolbarSearchValues("JP") }, width: 100, align: "center" }
        ...
        , { label: "任务附件", name: "Attachments", formatter: attachmentFmt, sortable: false, search: false }
   	]
		    , autoLoad: true
            , functions: {
                ...
                handleRefresh: function (btn) {
                    TaskGrid.refreshGrid('1=1 AND IsCompleted=0  AND DataType=1  ');
                },
                ...
                handleTodayTask: function () {
                    ...
                    var condition = "NeedDate between '" + today + " 00:00:00' and '" + today + " 23:59:59' AND DataType=1 ";
                ...
                , handleAllTask: function () {
                    TaskGrid.refreshGrid('1=1 AND DataType=1 ');
                },
                handleBatchGen: function () {
                    ...
                    var index = records[0].ConStrength.toUpperCase().indexOf("M");
                                    ...
                                    if (csValue) {
                                        bindSelectData($("#BetonFormula1"),
                                                ...
                                                { textField: 'FormulaName',
                                                    valueField: 'ID',
                                                    condition: "ConStrength like '%" + csValue + "%' and FormulaType='FType_CS' AND DataType=1  and Lifecycle > -1"
                                                }, function () { }
                                                );
                                        bindSelectData($("#SlurryFormula1"),
                                                ...
                                                { textField: 'FormulaName',
                                                    valueField: 'ID',
                                                    condition: "((ConStrength like '%" + csValue + "%' and FormulaType='FType_CS') or FormulaType='FType_S') AND DataType=1 and Lifecycle > -1"
                                                }, function () { }
                                                );
                                    }
                                },
    ...
    var tmpGrid = new MyGrid({
        ...
        //, buttons: buttons4
		    ...
		    , initArray: [
                ...
                , { label: ' 筒仓名称', name: 'SiloName', index: 'SiloName', width: 120, sortable: false }
                , { label: ' 材料名称', name: 'StuffName', index: 'StuffName', width: 120, sortable: false }
                , { label: ' 用量', name: 'Amount', index: 'Amount', formatter: 'number', editrules: { required: true, number: true }, formatoptions: { thousandsSeparator: "," }, editable: true, width: 100 }
                , { label: ' 排序', name: 'OrderNum', index: 'Silo.OrderNum', width: 60, sortable: true, hidden: true }
    ...
    $("#BetonFormula").bind('change', function () {
        ...
        for (var i = 0; i < plcheckbox.length; i++) {
            if (plcheckbox[i].checked == true) {
                if ($.inArray(plcheckbox[i].name, plist) == -1) {
                    plist.push(plcheckbox[i].name);
                }
            }
        }
        ...
        ajaxRequest('/Formula.mvc/Get', {
            id: formulaid
        },
						false,
						function (response) {
						    if (response.Result) {
						        $('[name="BetonTag"]').val(response.Data.BetonTag);
						    }
						})
        if (formulaid.length > 0) {
    ...
    var ConsGrid = new MyGrid({
        renderTo: 'ConsGridDiv'
		    ...
		    , initArray: [
                ...
                , { label: ' 搅拌线', name: 'ProductLineName', index: 'ProductLineName', sortable: false, width: 60 }
                , { label: ' 理论配比ID', name: 'FormulaID', index: 'FormulaID', hidden: true }
                ...
                , { label: ' 外加剂比率', name: 'ADMRate', index: 'ADMRate', width: 50, align: 'right', formatter: percentFmt, unformat: percentUnFmt }
            ...
            , functions: {
                ...
                handleBacthEdit: function (btn) {//批量修改
                    var consRecord = ConsGrid.getSelectedRecord();
                    if (isEmpty(consRecord.ID)) {
                        showMessage("提示：", "请选择一条配比记录");
                        return false;
                    }
                    var consid = consRecord.ID;
                    var taskid = consRecord.TaskID;
                    var pid = consRecord.ProductLineID;
                    var pname = consRecord.ProductLineName;
                    handleBacthEdit(consid, taskid, pid, pname);
                }
            }
    });
    ...
    //批量修改配比
    window.handleBacthEdit = function (consid, taskid, pid, pname) {
        var _auditstatus = "";
        var oldMixTime = "";
        var WinTitle = "批量修改配比";
        var keys = ConsGrid.getSelectedKeys();
        ConsGrid.showWindow({
            title: WinTitle,
            width: 400,
            height: 220,
            loadFrom: '/ConsMixprop.mvc/Index #PBEditForm form',
            afterLoaded: function () {
                //$("#ConsMixprop_AuditStatus").val(1);
                $("#ConsMixprop_ProductLineID").val(pid);
                $("#ConsMixprop_ProductLineName").val(pname);
            ...
            buttons:
                { "关闭": function () {
                    ...
                    ajaxRequest(
                            "/ConsMixprop.mvc/BatchEditConsMixpropItems",
                            {
                                consid: consid, taskid: taskid, pid: pid
                            },
                            true,
                            function (response) {
                                if (!response.Result) {
        ...
        renderTo: 'ConItemsGridDiv'
		    ...
		    , initArray: [
                ...
                , { label: ' 用量', name: 'Amount', index: 'Amount', formatter: 'number', editrules: { required: true, number: true }, formatoptions: { thousandsSeparator: "," }, editable: true, width: 60 }
                 , { label: ' 排序', name: 'OrderNum', index: 'Silo.OrderNum', width: 60, sortable: true, hidden: true }
                 , { label: ' 材料分类', name: 'StuffTypeName', index: 'StuffTypeName', width: 120, sortable: false }
		    ]
            , functions: {
                handleReload: function (btn) {
                    ItemsGrid.reloadGrid();
                ...
                , handleCopyModify: function (btn) {
                    ItemsGrid.handleAdd({
                        loadFrom: 'divCopyModifyForm',
                        buttons: {
                            "关闭": function () {
                                $(this).dialog('close');
                            },
                            "保存": function () {
                                ...
                                if (synstatus == 1) {
                                    ...
                                    ajaxRequest(
                                    "/ConsMixprop.mvc/CopyModify",
                                    {
                                        "ID": ID,
                                        "SynStatus": 1,
                                        "DirtyDataArr": dirtyDataArr,
                                        "CopyBeginDateTime": $("#CopyBeginDateTime").val(),
                                        "CopyEndDateTime": $("#CopyEndDateTime").val()
                                    },
                                    true,
                                    function (response) {
                                        if (response.Result) {
                                            ...
                                            ConsGrid.refreshGrid();
                                            ItemsGrid.reloadGrid();
                                            $("#divCopyModifyForm").dialog('close');
                                        }
                                    }
                                );
                                }
                ...
                } //handleCopyModify end

            }
    });

    var FormulaItemsGrid = new MyGrid({
        renderTo: 'TaskFormulaItemsGridDiv'
            , title: '理论配比子表信息列表'
            , autoWidth: true
            , buttons: buttons5
            , height: 370
        ...
        //, cellsubmit: 'clientArray'
		    , initArray: [
                ...
                , { label: '材料类型', name: 'StuffTypeName', index: 'StuffType.StuffTypeName', width: 80 }
                , { label: '材料用量', name: 'StuffAmount', index: 'StuffAmount', width: 80, editable: true }
                , { label: '标准用量', name: 'StandardAmount', index: 'StandardAmount', width: 80, hidden: false }
                , { label: '允许偏差范围', name: 'WarpAmount', index: 'WarpAmount', width: 80, editable: true }
		    ]
            , functions: {
                handleReload: function (btn) {
                    FormulaItemsGrid.reloadGrid();
                },
                handleRefresh: function (btn) {
                    FormulaItemsGrid.refreshGrid();
                }
            }
    });
    ...
    ItemsGrid.addListeners("afterSaveCell", function (rid, record, cellname, value) {
        ...
        if (gSysConfig.IsAutoAuditForConsMixprop == 'false') {
            ConsGrid.getJqGrid().setCell(record.ConsMixpropID, 'AuditStatus', 0);
        }
        ajaxRequest(
                        "/ConsMixprop.mvc/ChangeState",
                        {
                            "id": record.ConsMixpropID, "Status": 0
                        },
                        true,
                        function (response) {
                            if (response.Result) {
    ...
    TaskGrid.addListeners('rowdblclick', function (id, record, selBool) {
        ...
        if (gSysConfig.IsPHBAuditInfo != undefined) {
            console.dir(gSysConfig.IsPHBAuditInfo);
            if ((gSysConfig.IsPHBAuditInfo == 'true') && (record['Contract.IsJSJD'] != 'true')) {
                showError("出错啦！", "该合同未技术交底，不允许下发配比！");
                return false;
            }
        }
        var postUrl = opts.updateProduceTaskUrl;
        TaskGrid.handleEdit({
            ...
            afterFormLoaded: function () {
                ...
                var index = _csValue.toUpperCase().indexOf("M");
                ...
                var carpGrade = TaskGrid.getFormFieldValue("CarpGrade");
                var castMode = TaskGrid.getFormFieldValue("CastMode");

                if (csValue) {
                    var betonFormulaCondition;

                    if (gSysConfig.IsTaskFormulaMatch && gSysConfig.IsTaskFormulaMatch.toUpperCase() == "TRUE") {

                        betonFormulaCondition = "ConStrength like '%" + csValue + "%' and FormulaType='FType_C' AND DataType=1 and Lifecycle > -1 and CastMode ='" + castMode + "'";

                        //如果“级配”不为空，则查询级配
                        if (carpGrade && carpGrade !== '') {

                            betonFormulaCondition += " and CarpGrade = '" + carpGrade + "'";
                        }

                    } else {
                        betonFormulaCondition = "ConStrength like '%" + csValue + "%' and FormulaType='FType_CS' AND DataType=1 and Lifecycle > -1"
                    }
                    bindSelectData(TaskGrid.getFormField("BetonFormula"),
                                        ...
                                        { textField: 'FormulaName',
                                            ...
                                            condition: betonFormulaCondition
                                        }, function () {
                        ...
                        if (ID != null) {
                            ...
                            function (response) {
                                if (response.Result) {
                                    ...
                                    FormulaItemsGrid.refreshGrid("FormulaID='" + formulaid + "'");
                                }
                            }
                        );
                        }
    ...
    TaskGrid.addListeners('rowclick', function (id, record, selBool) {
        ConsGrid.refreshGrid("TaskID='" + id + "'");
        ItemsGrid.getJqGrid().jqGrid('clearGridData');
        FormulaItemsGrid.getJqGrid().jqGrid('clearGridData');
        currentTaskid = id;
    });
    ...
    ConsGrid.addListeners('rowclick', function (id, record, selBool) {
        ...
        if (dirtydatalen > 0) {
            ...
            var _FormulaID = record.FormulaID;
            showConfirm("确认信息",
                        ...
                        function () {//否按钮事件
                            ...
                            FormulaItemsGrid.refreshGrid("FormulaID='" + _FormulaID + "'");
            ...
            FormulaItemsGrid.refreshGrid("FormulaID='" + record.FormulaID + "'");
            var cellEditable = true;
            if (gSysConfig.IsAllowModifyForConsMixprop == "true") {
                cellEditable = true;
            } else {
