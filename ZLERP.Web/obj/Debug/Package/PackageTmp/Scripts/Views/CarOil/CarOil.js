...
function caroilIndexInit(opts) {
        ...
        //, width: '100%'
           ...
           //, groupingView: { groupSummary: [true], groupDataSorted: true }
		    , initArray: [
                ...
                , { label: '卡号', name: 'Car.FacInnerID', align: 'right', index: 'Car.FacInnerID', sortable: false, width: 80 }
                , { label: '车主', name: 'Car.Owner', align: 'right', index: 'Car.Owner', sortable: false, width: 80 }
                ...
                , { label: '加油日期', name: 'AddDate', index: 'AddDate', formatter: 'datetime', align: 'center', searchoptions: { dataInit: function (elem) {
                    $(elem).datetimepicker();
                }, sopt: ['ge']
                }
                }
                ...
                , { label: '上次加油里程', name: 'LastKM', index: 'LastKM', width: 100, align: 'right' }
                , { label: '本次加油里程', name: 'ThisKM', index: 'ThisKM', width: 100, align: 'right' }
                , { label: '车辆行程', name: 'KiloMeter', index: 'KiloMeter', width: 100, align: 'right' }
                ...
                handleAdd: function (btn) {
                    myJqGrid.handleAdd({
                        ...
                        afterFormLoaded: function () {
                            ...
                            $('#SiloID').bind('change', SiloIDChange)
                        }
    ...
    function SiloIDChange(select) {
        var SiloID = select.target.value;
        if (isEmpty(SiloID)) {
            return;
        }
        console.log(SiloID);
        ajaxRequest(opts.getOilPrice, { SiloID: SiloID }, false, function (response) {
            if (response.Result) {
                myJqGrid.getFormField("UnitPrice").val(response.Data);
            };
        });
    } 
