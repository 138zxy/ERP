function M_TranBalanceIndexInit(options) {
    var myJqGrid = new MyGrid({
		...
		, initArray: [
            ...
            , { label: '纳入月份', name: 'InMonth', index: 'InMonth', width: 80 }
            , { label: '审核状态', name: 'AuditStatus', index: 'AuditStatus', formatter: booleanFmt, unformat: booleanUnFmt, width: 80 }
            ...
            , { label: '运输公司编码', name: 'TranID', index: 'TranID', width: 80, hidden: true }
            , { label: '运输公司', name: 'SupplyInfo.SupplyName', index: 'SupplyInfo.SupplyName', width: 150 }

            , { label: '材料编码', name: 'StuffID', index: 'StuffID', width: 80, hidden: true }
            ...
            , { label: '结算方式', name: 'BaleType', index: 'BaleType', width: 80 }
            , { label: '单一价格结算', name: 'IsOnePrice', index: 'IsOnePrice', formatter: booleanFmt, unformat: booleanUnFmt, width: 80 }
            , { label: '单一价格', name: 'OnePrice', index: 'OnePrice', width: 80 }
            , { label: '设定价结算', name: 'IsStockPrice', index: 'IsStockPrice', formatter: booleanFmt, unformat: booleanUnFmt, width: 80 }
            , { label: '累计数量(吨)', name: 'AllCount', index: 'AllCount', formatter: Kg2TFmt, unformat: T2KgFmt, width: 80 }
            , { label: '确认数量(吨)', name: 'AllOkCount', index: 'AllOkCount', formatter: Kg2TFmt, unformat: T2KgFmt, width: 80 }
            , { label: '累计金额', name: 'AllMoney', index: 'AllMoney', width: 80, align: 'right', formatter: 'currency' }
        ...
        , functions: {
            ...
            handleAdd: function (btn) { 
                ...
                myJqGridToAdd.refreshGrid(" StuffInID NOT IN (SELECT StuffInID FROM M_TranBalanceDel) AND Lifecycle=3  AND TransportID='" + TranID + "' and StuffID='" + StuffID + "'");
 ...
 ; 
    ...
    var myJqGridTo = new MyGrid({
        ...
        initArray: [
            ...
            , { label: '运输单位', name: 'StuffIn.TransportName', index: 'StuffIn.TransportName', width: 150 } 
            ...
            , { label: '出厂时间', name: 'StuffIn.OutDate', index: 'StuffIn.OutDate', formatter: 'datetime', width: 120 } 
            , { label: '车号', name: 'StuffIn.CarNo', index: 'StuffIn.CarNo', width: 100 }
            , { label: '结算重量(吨)', name: 'StuffIn.FinalFootNum', index: 'StuffIn.FinalFootNum', formatter: Kg2TFmt, unformat: T2KgFmt, align: 'right', width: 100 }
            , { label: '单价(元/吨)', name: 'Price', index: 'Price', editable: true, width: 80, align: 'right', formatter: 'currency' }
            , { label: '其他费用', name: 'OtherMoney', index: 'OtherMoney', editable: true, width: 80, align: 'right', formatter: 'currency' }
            , { label: '总价', name: 'AllMoney', index: 'AllMoney', editable: true, width: 80, align: 'right', formatter: 'currency' }
    ...
    var myJqGridToAdd = new MyGrid({
        ...
        initArray: [
            ...
            , { label: '过磅时间', name: 'InDate', index: 'InDate', formatter: 'date', width: 80 }
            ...
            , { label: '结算重量', name: 'FinalFootNum', index: 'FinalFootNum', align: 'right', formatter: Kg2TFmt, unformat: T2KgFmt, width: 100 }
            , { label: '方量', name: 'Volume', index: 'Volume', width: 60 }  
		],
        autoLoad: false,
        functions: {
            handleReload: function (btn) {
	...
	$("#myJqGridAddDetial").dialog({
	    modal: true,
	    autoOpen: false,
	    title: "未结算的进料单[合同编码和材料编码与选择的结算单一致]",
	    width: 800,
	    Height: 600,
	    buttons: {
	        '确认': function () {
