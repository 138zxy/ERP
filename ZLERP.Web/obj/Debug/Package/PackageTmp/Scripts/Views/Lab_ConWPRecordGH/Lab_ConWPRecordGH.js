...
function lab_conwprecordIndexInit(storeUrl) {
    ...
    myJqGrid.addListeners("rowclick", function (id, record) {
        ckLab_RecordID = id;
        myJqGridItems.refreshGrid("Lab_ConWPRecordGHId='" + id + "'");
    });
    ...
    var myJqGridItems = new MyGrid({
        ...
        //, width: '100%'
            ...
            , buttons: buttons1
            , height: gGridHeight
		    , storeURL: '/Lab_ConWPRecordItemsGH.mvc/Find'
		    , sortByField: 'ID'
		    , primaryKey: 'ID'
		    , setGridPageSize: 30
            , dialogWidth: 700
		    ...
		    , initArray: [
                ...
                , { label: '试件编号', name: 'Lab_ConWPRecordGHId', index: 'Lab_ConWPRecordGHId', width: 80 }
            ...
            , functions: {
                handleReload: function (btn) {
                    ...
                    myJqGridItems.reloadGrid("Lab_ConWPRecordGHId='" + selectrecord.ID + "'");
                },
                handleRefresh: function (btn) {
                    var selectrecord = myJqGrid.getSelectedRecord();
                    console.log(selectrecord.ID);
                ...
                handleAdd: function (btn) {
                    ...
                    myJqGridItems.handleAdd({
                        ...
                        afterFormLoaded: function () {
                            console.log(selectrecord.ID);
                            $("#Lab_ConWPRecordItemsGH_Lab_ConWPRecordGHId").val(selectrecord.ID);
                            $("#Lab_ConWPRecordItemsGH_ReportType").val('');
                            var conditionstr = " ParentID='LabTemplateType' AND FIELD2=3 ";
                            bindSelectData($("#Lab_ConWPRecordItemsGH_ReportType"),
                                '/Dic.mvc/ListData',
                                { textField: 'DicName',
                                    valueField: 'TreeCode',
                            ...
                            var fieldvalue = $('#Lab_ConWPRecordItemsGH_ReportType option:selected').val();
                ...
                , handleDownload: function (btn) {
                    ...
                    showConfirm("确认", "您已选择<font color=red>" + keys.length + "</font>条记录,确定要<font color=green><b>下载报告</b></font>吗？", function () {
                        ajaxRequest(
                        '/Lab_ConWPRecordItemsGH.mvc/DownFile',
                        {
                            ids: keys
                        },
                        true,
