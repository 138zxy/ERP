...
function percentFmt(cellvalue, options, rowObject) {
    ...
    if (cellvalue && cellvalue != '') {
        txt = (parseFloat(cellvalue) * 100).toFixed(2) + "%";
    }
    else {
        cellvalue =''
    }

    return '<span rel="' + cellvalue + '">' + txt + '</span>'
}

function percentUnFmt(cellvalue, options, cell) {
    return $('span', cell).attr('rel');
}
...
//入库状态（入库单）
function StuffinStateFmt(cellvalue, options, rowObject) {
...
function Kg2TFmt(cellvalue, options, rowObject) {
    if (cellvalue && cellvalue != 0)
        return formatFloat(cellvalue / 1000, 4);
    else
        return 0;
}
...
function BuCubeUnFmt(cellvalue, options, rowObject) {
    var ShipCube = cellvalue * 1;
    if (ShipCube > gSysConfig.SupplementCube * 1) {
        return 0.00;
    } else {
        return gSysConfig.SupplementCube * 1 - ShipCube;
    }
}
