function productrecordIndexInit(storeUrl1, storeUrl2, storeUrl3, storeUrl4, storeUrl5) {
    ...
    var condition = "ProduceDate between '" + today + " 00:00:00' and '" + today + " 23:59:59'  ";
    var shippDocGrid = new MyGrid({
        ...
        // , { label: '启动生产日期', name: 'StartupTime', index: 'StartupTime', width: 120, formatter: 'datetime' }
                ...
                , { label: '混凝土/干混数量', name: 'BetonCount', index: 'BetonCount', width: 50, search: false, editable: true, formatoptions: { defaulValue: "", decimalPlaces: 1} }
                , { label: '砂浆数量', name: 'SlurryCount', index: 'SlurryCount', width: 50, search: false, editable: true, formatoptions: { defaulValue: "", decimalPlaces: 1} }
                , { label: '已生产(方)', name: 'TotalProduceCube', index: 'TotalProduceCube', width: 70, align: 'right', search: false, editable: true, formatoptions: { defaulValue: "", decimalPlaces: 1} }
                ...
                , { label: '累计车数', name: 'ProvidedTimes', index: 'ProvidedTimes', width: 50 }
            ...
            , functions: {
                ...
                , FindCustRecord: function (btn) {
                 ...
                 , reportkpjd: function (btn) {
                     if (!shippDocGrid.isSelectedOnlyOne()) {
                         showMessage('提示', '请选择一条记录进行操作！');
                         return;
                     }
                     var selectedRecord = shippDocGrid.getSelectedRecord();
                     window.open("/Reports/FuHuang/kpjd.aspx?uid=" + selectedRecord.ID);
                 }
                 , reportqdjc: function (btn) {
                     var selectedRecords = shippDocGrid.getSelectedRecords();
                     var str = "";
                     for (var i = 0; i < selectedRecords.length; i++) {
                         str += selectedRecords[i].ID + ",";
                     }
                     //去掉最后一个逗号
                     if (str.length > 0) {
                         str = str.substr(0, str.length - 1);
                     }
                     window.open("/Reports/FuHuang/qdjc.aspx?uid=" + str);
                 }
                 , reportdcmx: function (btn) {
                     if (!shippDocGrid.isSelectedOnlyOne()) {
                         showMessage('提示', '请选择一条记录进行操作！');
                         return;
                     }
                     var selectedRecord = shippDocGrid.getSelectedRecord();
                     window.open("/Reports/FuHuang/dcmx.aspx?uid=" + selectedRecord.ID);
                 }
    ...
    var ProductRecordGrid = new MyGrid({
        ...
        //, title:'生产记录罐次表'
		    ...
		    , initArray: [
                ...
                , { label: ' 是否砂浆', name: 'IsSlurry', index: 'IsSlurry', width: 50, align: 'center', formatter: booleanFmt, unformat: booleanUnFmt }
                , { label: ' 是否手动', name: 'IsManual', index: 'IsManual', width: 50, align: 'center', formatter: booleanFmt, unformat: booleanUnFmt }
    ...
    var ProductRecordItemsGrid = new MyGrid({
		...
		, initArray: [
            ...
            , { label: '实际用量', name: 'ActualAmount', index: 'ActualAmount', width: 70 }
