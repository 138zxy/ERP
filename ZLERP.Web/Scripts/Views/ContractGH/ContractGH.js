...
function contractIndexInit(options) {
    ...
    function beforeContractFormSubmit() {
        ...
        var contractMatCube = ContractGrid.getFormFieldValue("ContractGH.MatCube"); //合同垫资方量
        var contractPrepayCube = ContractGrid.getFormFieldValue("ContractGH.PrepayCube"); //合同预付方量
    ...
    var ContractGrid = new MyGrid({
        ...
        //, singleSelect: true
            , initArray: [
                ...
                , { label: "垫资量", name: "MatCube", index: "MatCube" }
                , { label: "预付量", name: "PrepayCube", index: "PrepayCube" }
            ...
            , functions: {
                    ...
                    ProjectGrid.handleAdd({
                        ...
                        afterFormLoaded: function () {
                            ...
                            linkManField.bind("change", function (event) {
                                ...
                                var selectedLinkMan = $("#ProduceTaskGH_LinkMan", "#" + ProjectGrid.getFormId()).children(":selected").text();
                                var linkManId = $("#ProduceTaskGH_LinkMan", "#" + ProjectGrid.getFormId()).val();
                                ProjectGrid.setFormFieldValue("ProduceTaskGH.Tel", "");
                ...
                handlePrintCotract: function (btn) {
                    ...
                    var tplPath = path + "/ContractGH.aspx?customerid=" + contractrecord.CustomerID + "&contractid=" + projectrecord.ContractID + "&projectid=" + projectrecord.ID;
                    window.open(tplPath);
                ...
                handleCancelAudit: function (btn) {
                    ...
                    if (keys.length > 0) {
                        ...
                        if (record && record.AuditStatus == '0') {
                            showError('该合同未审核，不需要取消！');
                            return;
                        }
                        showConfirm("确认信息", "确认要进行取消审核操作?", function (btn) {
                            var requestURL = options.CancelAuditUrl;
                ...
                handleAudit: function (btn) {
                    ...
                    if (keys.length > 0) {
                        ...
                        if (record && record.AuditStatus == '1') {
                            showError('该合同已审核，不需要审核！');
                            return;
                        }
                        showConfirm("确认信息", "确认要进行审核操作?", function (btn) {
                            var requestURL = options.AuditUrl;
                            var postParams = { ids: keys };
                            ajaxRequest(requestURL, postParams, true, function (response) {
                                ContractGrid.reloadGrid();
                            });
     ...
     var ProjectGrid = new MyGrid({
        renderTo: "ProjectGrid"
            ...
            , initArray: [
                ...
                , { label: "工程名称", name: "ProjectName", index: "ProjectName", editrules: { required: true }, editable: true }
    ...
    function AddUnit() {
        $.ajax({
            ...
            success: function (data) {
                if (data.Result) {
                    showMessage("增加成功");
                    $("#ProduceTaskGH_SupplyUnit").append("<option value='" + data.Data + "'>" + data.Data + "</option>");
                }
                else {
