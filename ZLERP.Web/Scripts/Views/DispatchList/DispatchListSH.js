...
function dispatchListIndexInit(options) {
    ...
    var scrwGrid = new MyGrid({
        ...
        //, title: '生产任务列表'
		    ...
		    , sortByField: ' case when OpenTime is null then 1 else 0 end desc ,OpenTime  '
            , storeCondition: " AND DataType=1  "
            , sortOrder: 'desc '
		    , primaryKey: 'ID'
            , toolbarSearch: true
        ...
        //, groupField: 'IsFormulaSend'
            ...
            , { label: '特性', name: 'IdentityValue', index: 'IdentityValue', search: false, sortable: false, width: 60 }
            , { label: '砼标记', name: 'BetonTag', index: 'BetonTag', search: false, sortable: false, width: 60 }
   		    , { label: '浇筑方式', name: 'CastMode', index: 'CastMode', width: 80 }
   		    , { label: '施工部位', name: 'ConsPos', index: 'ConsPos', width: 100 }
   		    , { label: '项目编号', name: 'TaskNo', index: 'TaskNo', width: 100 }
            ...
            , { label: '含砂浆', name: 'IsSlurry', index: 'IsSlurry', width: 50, align: 'center', formatter: booleanFmt, unformat: booleanUnFmt, search: false }
   		    ...
   		    , { label: '湿拌类别', name: 'CementType', index: 'CementType', width: 80, search: false }
   		    , { label: '湿拌配比', name: 'BetonFormulaName', index: 'BetonFormulaName', search: false }
            ...
            , functions: {
                ...
                , handleLastDay: function (btn) {
                    ...
                    condition2 = "NeedDate between '" + uom + " 00:00:00' and '" + uom + " 23:59:59'  order by NeedDate ";
                ...
                buttons: {
                    "确定": function () {
                        ...
                        $.ajax({
                            ...
                            type: "POST",
                            data: {
                                RentCarName: RentCarNameID, dataType: 1
                            },
    ...
    var scrwGrid1 = new MyGrid({
        ...
        , { label: '施工单位', name: 'ConstructUnit', index: 'ConstructUnit', width: 150, searchoptions: { sopt: ['cn']} }
   		...
   		, { label: '客户名称', name: 'CustName', index: 'CustName', width: 200, searchoptions: { sopt: ['cn']} }
   		, { label: '砼强度', name: 'ConStrength', index: 'ConStrength', width: 50, align: 'center' }
   		, { label: '浇筑方式', name: 'CastMode', index: 'CastMode', width: 80 }
   		, { label: '施工部位', name: 'ConsPos', index: 'ConsPos', width: 100 }
   		, { label: '计划', name: 'PlanCube', index: 'PlanCube', width: 40, align: 'center', search: false, formatter: FixedTwoNumber, summaryType: 'sum', summaryTpl: '{0}' }
    ...
    var shippDocGrid = new MyGrid({
        ...
        //, groupField: 'ProductLineName'
		    , initArray: [
                ...
                , { label: '罐容比', name: 'ProvidedTimes', index: 'ProvidedTimes', width: 40, align: 'center', search: false }
                , { label: '已供量', name: 'ProvidedCube', index: 'ProvidedCube', width: 40, align: 'center', search: false, formatter: "number", formatoptions: { defaulValue: "", decimalPlaces: 1} }
                , { label: '生产线', name: 'ProductLineName', index: 'ProductLineName', width: 50, align: 'center' }
                , { label: '罐容比', name: 'PCRate', index: 'PCRate', width: 50, align: 'center' }
                , { label: '调度量', name: 'SendCube', index: 'SendCube', width: 40, align: 'right', formatter: "number", formatoptions: { defaulValue: "", decimalPlaces: 1} }
                , { label: '出票量', name: 'ParCube', index: 'ParCube', width: 40, align: 'right', search: false, formatter: "number", formatoptions: { defaulValue: "", decimalPlaces: 1} }
                , { label: '剩余量', name: 'RemainCube', index: 'RemainCube', width: 40, align: 'right', search: false, formatter: "number", formatoptions: { defaulValue: "", decimalPlaces: 1} }
                , { label: '报废量', name: 'ScrapCube', index: 'ScrapCube', width: 40, align: 'right', search: false, formatter: "number", formatoptions: { defaulValue: "", decimalPlaces: 1} }
                , { label: '转料量', name: 'TransferCube', index: 'TransferCube', width: 40, align: 'right', search: false, formatter: "number", formatoptions: { defaulValue: "", decimalPlaces: 1} }
                , { label: '其他方量2', name: 'XuCube', index: 'XuCube', width: 40, align: 'right', search: false, formatter: "number", formatoptions: { defaulValue: "", decimalPlaces: 1} }
    ...
    var insteadProductGrid = new MyGrid({
        ...
        //, groupField: 'ProductLineName'
		    , initArray: [
                ...
                , { label: '泵送量', name: 'pumpCount', index: 'pumpCount', width: 80, align: 'center', search: false }
...
//    });
            ...
            formId: 'scdj-form'
                ...
                , postCallBack: function (response) {
                    if (response.Result) {
                        ...
                        scdjGrid1.reloadGrid();
                        if (!isEmpty(printType)) {
                            //console.log(response.Data);
                            //setShipDocTemplate("ShippingDocument.xml");
                            //printShippingDoc(printType, response.Data);

                            var url = "/GridReport/PrintDirect.aspx?report=ShippingDocument&ID=" + response.Data;
                            window.open(url, "_blank");
                        }
                        refreshCarStatus();
                        rwid = TaskID;
    ...
    var scdjGrid1 = new MyGrid({
        ...
        //, autoWidth: true
            ...
            , storeCondition: "IsCompleted = 0  AND DataType=1  "
		    ...
		    , initArray: [
                ...
                , { label: '浇筑方式', name: 'CastMode', index: 'CastMode', editable: true, width: 80, sortable: false }
                ...
                , { label: '湿拌方量', name: 'BetonCount', index: 'BetonCount', editable: true, editrules: { required: true }, sortable: false, width: 50 }
                , { label: '砂浆方量', name: 'SlurryCount', index: 'SlurryCount', editable: true, editrules: { required: true }, sortable: false, width: 50, hidden: true }
            ...
            , functions: {
                handleRefresh: function (btn) {
                    scdjGrid1.refreshGrid();
                },
                handleDelete: function (btn) {
                    if (scdjGrid1.isSelectedOnlyOne('请选择一条记录!')) {
                        var record = scdjGrid1.getSelectedRecord();
                        scdjGrid1.deleteRecord({
                ...
                , handleCompleted: function (btn) {
                    if (scdjGrid1.isSelectedOnlyOne('请选择一条记录!')) {
                        showConfirm("确认操作", "确认要置为完成?", function () {
                            var dispatchid = scdjGrid1.getSelectedKey();
                ...
                , handleSampling: function (btn) {
                    if (scdjGrid1.isSelectedOnlyOne('请选择一个车号!')) {
                        var dispatchid = scdjGrid1.getSelectedKey();
                    ...
                    if (scdjGrid1.isSelectedOnlyOne('请选择一条记录!')) {
                        ...
                        scdjGrid1.showWindow({
                            ...
                            , afterLoaded: function () {
                                ...
                                $("#exception-ProvidedTimes").val(scdjGrid1.getSelectedRecord().ProvidedTimes);
                                $("#exception-ProvidedCube").val(scdjGrid1.getSelectedRecord().ProvidedCube);
                                //修改已供车次，方量
                                $("#exception-providedbtn").bind('click', record, function (btn) {

                                    showConfirm("确认操作", "是否执行操作？", function () {
                                        ajaxRequest(options.changeProvidedUrl, { ID: scdjGrid1.getSelectedKey(),
                    ...
                    function (response) {
                        if (response.Result) {
                            ...
                            showConfirm('提示', '需要打印吗？', function () {
                                ...
                                var url = "/GridReport/PrintDirect.aspx?report=ShippingDocument&ID=" + shipDocID;
                                window.open(url, "_blank");
                            });
                            refreshCarStatus();
                            dispatchrecord = scdjGrid1.getRecordByKeyValue(id);
    ...
    function refreshCarStatus() {
        ...
        if ($('.carlist').length == 0) {
            clearInterval(gTimer_Car1);
            return;
        }

        var companyId = $("#CompanyID").val();
        ajaxRequest(options.getCarStatusUrl, { "IsUsed": "1", "CompanyID": companyId, dataType: 1 }, false, function (response) {
            ...
            var freezeCar = response.FreezeCar;
            var allowShipCarRemainCube = response.AllowShipCarRemainCube;
            if (allowShipCar == null || allowShipCar == 'undefined') {
                ...
                for (var i = 0; i < allowShipCar.length; i++) {
                    ...
                    if (allowShipCarRemainCube[i].RemainCube !== '') {
                        allowShipHtml += '<li data-itemid="' + car.ID + '"><div><span>' + car.ID + '</span>[' + allowShipCarRemainCube[i].RemainCube + ']</div></li>';
                    } else {
                        allowShipHtml += '<li data-itemid="' + car.ID + '"><div><span>' + car.ID + '</span></div></li>';
                    }
                }
                $('#AllowShipCar1').append(allowShipHtml);
                //                //运输车号列表只显示可调度车辆
                //                if (gSysConfig.IsShowDispatchStatusCar == "true") {
                //                    var carDropDownList = scrwGrid.getFormField('ShippingDocument.CarID');
                //                    carDropDownList.empty();

                //                    for (i = 0; i < allowShipCar.length; i++) {
                //                        var id = allowShipCar[i].ID;
                //                        //if (length <= 0) { //不存在列表中添加                
            ...
            if (shippingCar == null || shippingCar == 'undefined') {
                ...
                for (var i = 0; i < shippingCar.length; i++) {
                    ...
                    shippingHtml += '<li data-itemid="' + car.ID + '"><div><span>' + car.ID + '</span></div></li>';
                }
                $('#ShippingCar1').append(shippingHtml);
            }
            ...
            if (restCar == null || restCar == 'undefined') {
                ...
                for (var i = 0; i < restCar.length; i++) {
                    var car = restCar[i];
                    restHtml += '<li data-itemid="' + car.ID + '"><div><span>' + car.ID + '</span></div></li>';
                }
                $('#RestCar1').append(restHtml);
                //scrwGrid.refreshGrid();
            }

            if (freezeCar == null || freezeCar == 'undefined') {
            }
            else {
                //冻结车辆
                $('#FreezeCar1').empty();
                var freezeHtml = '';
                for (var i = 0; i < freezeCar.length; i++) {
                    var car = freezeCar[i];
                    freezeHtml += '<li data-itemid="' + car.ID + '"><div><span>' + car.ID + '</span></div></li>';
                }
                $('#FreezeCar1').append(freezeHtml);
            }
    ...
    function setScdjForm(record) {
        ...
        var companyId = $("#CompanyID").val();
        ajaxRequest(options.getLastDocUrl, { id: taskid, companyId: companyId ,dataType:2 }, false, function (response) {
            ...
            //zyx,绑定泵车司机
            var pumpcarField = scrwGrid.getFormField('ShippingDocument.PumpName');
            ...
            scrwGrid.setFormFieldDisabled('ShippingDocument.CarID', false);
    ...
    function pumpcarListChange(select) {
        ...
        var driverList = scrwGrid.getFormField('ShippingDocument.PumpDriver');
        driverList.empty();
        ajaxRequest(options.getCarInfoUrl, { id: carid }, false, function (response) {
            var users = response.Users;
            if (!isEmpty(users)) {
    ...
    function carSelectAction(carid) {
        ...
        ajaxRequest(options.getCarInfoUrl, { id: carid }, false, function (response) {
            ...
            ajaxRequest(options.getRemaincubeUrl, { carId: carid,dataType:1 }, false, function (response) {
    ...
    function calculatePrivided() {
        ...
        //alert("ygsl:" + ygsl + ",checkhntfs:" + checkhntfs + ",checkTLFL:" + checkTLFL + ",checkQTFL:" + checkQTFL);
    ...
    function productLineChange(select) {
        var TaskRecord = scrwGrid.getSelectedRecord();
        ...
        //重置罐容比列表
        var data = select.data[pid];
        var productLineJson = select.data[pid];
        var LineID = productLineJson.ID;
        var PCRate = "1";
        ...
        //获取罐容比。
        ajaxRequest("/DispatchList.mvc/GetInfoByTaskID", { TaskId: Taskid, LineID: LineID }, false, function (response) {
            if (response.Result)          
            {
                if (response.Data != null) {
                    PCRate = response.Data.toString();
                    var select = document.getElementById("DispatchList_PCRate");
                    for (var i = 0; i < select.options.length; i++) {
                        if (select.options[i].value == PCRate) {
    ...
    scrwGrid.addListeners('cellclick', function (id, record, colIndex, cellcontent) {
        scrwGridCellClick(id, record);
    });


    function scrwGridCellClick(id, record) {
        ...
        var OtherDemand = record['OtherDemand'];
        $("#ProduceTask_OtherDemand").val(OtherDemand);
        $("#ShippingDocument_OtherDemand").val(OtherDemand);
        var taskId = record['ID'];
        ...
        *不使用这种方式查看最近发车记录 xyl 2013-06-05
        if (colIndex > 7) {
        scdjGrid1.refreshGrid("TaskID='" + taskId + "' AND IsCompleted = 1", 'ID', 'DESC');
        //scdjGrid1.getJqGrid().jqGrid('setGridParam', { sortname: 'ID', sortorder: 'DESC', page: 1, postData: { condition: "TaskID='" + taskId + "' AND IsCompleted = 1"} });
        }
        */
    ...
    $('#freeze-car-btn button').click(shiftButtonForCar);
    ...
    function shiftButtonForCar(btn) {
        ...
        if ($('.carlist div').hasClass('clickCarCls') && $('.clickCarCls', "#" + btnGroup).length > 0) {
            carId = $('.clickCarCls > span', "#" + btnGroup)[0].innerHTML;
        }
        //alert(operType + "\n" + btnGroup + "\n" + carstatus + "\n" + carId);
        if (isEmpty(carId)) {
    ...
    //已选择的生产登记
    var sid = null;
    scdjGrid1.addListeners("rowclick", function (ids) {
        sid = ids;
    });
    scdjGrid1.addListeners("gridComplete", function () {
        if (sid) {
            scdjGrid1.getJqGrid().jqGrid("setSelection", sid);
        }
    });
    ...
    function showConfirm1(title, msg, okFn, cancleFn) {
        ...
        gTimer_Remind1 = window.clearInterval(gTimer_Remind1);
        $('<div title="' + title + '"><p>' + msg + '</p></div>').dialog({
            ...
            buttons: {
                "取消": function (btn) {
                    gTimer_Remind1 = window.setInterval(remindRefresh, gSysConfig.SCDJRefreshInterval * 1000);
                    $(this).dialog("close");
                    if (cancleFn != null) {
    ...
    if (gSysConfig.SCDJRefreshInterval) {
        ...
        if (refreshInterval > 0) {
            if (gTimer_SCDJ1)
                clearInterval(gTimer_SCDJ1);
            gTimer_SCDJ1 = window.setInterval(scdjAutoRefresh, gSysConfig.SCDJRefreshInterval * 1000);
            //是否自动刷新车辆状态
            if (gSysConfig.IsRefreshCarStatus == "true") {
                if (gTimer_Car1) clearInterval(gTimer_Car1);
                gTimer_Car1 = window.setInterval(refreshCarStatus, gSysConfig.SCDJRefreshInterval * 1000);
            }
            if (gTimer_Remind1 != null)
                gTimer_Remind1 = window.clearInterval(gTimer_Remind1);
            gTimer_Remind1 = window.setInterval(remindRefresh, gSysConfig.SCDJRefreshInterval * 1000);
        }
    }
    ...
    if (gSysConfig.EnableCarDrag == "true") {
        $("#AllowShipCar1").dragsort({
            dragSelector: "div",
            dragBetween: true,
            placeHolderTemplate: "<li class='placeHolder'><div></div></li>",
            //callBack: saveCarOrder,
            dragEnd: saveCarOrder
         });
    }

    function saveCarOrder(carId) {

        var data = $("#AllowShipCar1 li").map(function () { return $(this).attr("data-itemid"); }).get();
        var params = {
            //"carid": carId,
            "carid": $("span", this).html(),
            "orders": data
        };
        ...
        ajaxRequest(options.getLastDocAutoByCarId,
                ...
                function (response) {
                    ...
                    if (response.Result) {
                        ...
                        scdjGrid1.setFormFieldValue('Driver', response.Data.Driver);
                        scdjGrid1.setFormFieldValue('SourceShipDocID', response.Data.ID)
                    }
    ...
    //公司编号，选择事件
    $("#CompanyID").unbind("change").bind("change", function () {
        console.log("trigger rowclick");
        var id = scrwGrid.getSelectedKey();
        var record = scrwGrid.getSelectedRecord();
...
window.onunload = function () {
    clearInterval(gTimer_SCDJ1);
    clearInterval(gTimer_Car1);
    clearInterval(gTimer_Remind1);
} 

//一键休息
function RestCarAll(carStatus) {
    $.ajax({
        url: "/Car.mvc/AllToRestCar",
        dataType: "json",
        type: "POST",
        data: {
            carStatus: carStatus,dataType:1
        },
        success: function (data) {
            if (data.Result) {
                showMessage("一键休息成功！");            
...
//一键开工
function AllToAllowShipCar(carStatus) {
    $.ajax({
        url: "/Car.mvc/AllToAllowShipCar",
        dataType: "json",
        type: "POST",
        data: {
            carStatus: carStatus, dataType: 1
