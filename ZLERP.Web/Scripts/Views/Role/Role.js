function roleIndexInit(opts) {
    ...
    $('#DivOrganizations').height(gGridHeight + 80);
    ...
    var myJqGrid = new MyGrid({
        renderTo: 'My-JqGrid'
            ...
            , functions: {
                ...
                , handleSaveRoleOrganizations: function (btn, e) {
                    console.log("handleSaveRoleOrganizations called");
                    //保存权限  
                    var roleId = myJqGrid.getSelectedKey();
                    var treeObj = $.fn.zTree.getZTreeObj('DivOrganizations');
                    var nodes = treeObj.getCheckedNodes();
                    if (roleId) {
                        var ids = [];
                        for (var i = 0; i < nodes.length; i++) {
                        ...
                        ajaxRequest(
                            ...
                            function () {
                                //清空当前用户缓存
                                delete (roleOrganizationCache[roleId]);
                            }
                        );
                    }
                    else {
    ...
    myJqGrid.addListeners('rowclick', function (id, record, rowIndex, cellIndex) {
        ...
        if (roleOrganizationCache.hasOwnProperty(id)) {
            bindOrganization(roleFuncCache[id]);
        }
        else {
            loadRoleOrganizations(id);
        }
    ...
    //角色组织
    function bindOrganization(data) {
        var treeObj = $.fn.zTree.getZTreeObj('DivOrganizations');
        treeObj.checkAllNodes(false);
        //绑定角色已有权限        　                                      
        for (var i = 0; i < data.length; i++) {
    ...
    //加载组织机构
    function loadRoleOrganizations(roleId) {
        $.post(
                opts.getRoleOrganizationUrl,
                { roleId: roleId },
                function (data, textStatus) {
                    bindOrganization(data);
                    roleFuncCache[roleId] = data;
                },
                'json'
             );
    ...
    var OrganizationTreeSettings = {
        ...
        async: {
            enable: true,
            url: opts.organizationTreeUrl,
            autoParam: ["id", "name", "level"]
            // otherParam: { "otherParam": "zTreeAsyncTest" }　
        },
        callback: {
            //onRightClick: OnRightClick,
            onClick: OrganizationNodeClick,
            onAsyncError: function (event, treeId, node, XMLHttpRequest, textStatus, errorThrown) {
                handleServerError(XMLHttpRequest, textStatus, errorThrown);
            }
        }
    ...
    var OrganizationZTree = $.fn.zTree.init($('#DivOrganizations'), OrganizationTreeSettings);
    function OrganizationNodeClick(e, treeId, treeNode) {
        OrganizationZTree.expandNode(treeNode);
    }
