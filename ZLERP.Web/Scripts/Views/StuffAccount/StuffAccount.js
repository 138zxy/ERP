...
function IndexInit(opts) {
    ...
    var MyJqGridMain = new MyGrid({
        ...
        //, width: '100%'
            ...
            , buttons: buttons0
            , height: gGridHeight
		    , storeURL: "/StuffAccountMain.mvc/Find"
		    , sortByField: 'ID'
            , sortOrder: "ASC"
		    , primaryKey: 'ID'
            , dialogWidth: 400
    ...
    MyJqGridMain.addListeners('rowclick', function (id, record, selBool) {
        myJqGrid.refreshGrid("StuffAccountMainId='" + id + "'");
    });
    ...
    var myJqGrid = new MyGrid({
        renderTo: 'MyJqGrid'
            ...
            //, groupField: 'SiloId'
             , editSaveUrl: "/StuffAccount.mvc/Update"
		    , initArray: [
                { label: 'ID', name: 'ID', index: 'ID', hidden: true }
                , { label: '主表编号', name: 'StuffAccountMainId', index: 'StuffAccountMainId', editable: true, editrules: { required: true }, hidden: true }               
                , { label: '材料ID', name: 'StuffId', index: 'StuffId', hidden: false, width: 80 }
                , { label: '材料名称', name: 'StuffName', index: 'StuffName', hidden: false }
                , { label: '盘存数量', name: 'CurrentCount', index: 'CurrentCount', editable: true, width: 60 }
                , { label: '盘存单价', name: 'UnitPrice', index: 'UnitPrice', editable: false, width: 60 }
                , { label: '盘存金额', name: 'CurrentMoney', index: 'CurrentMoney', editable: true, width: 80 }
                , { label: '筒仓ID', name: 'SiloId', index: 'SiloId', hidden: true }
                , { label: '筒仓', name: 'SiloName', index: 'SiloName', hidden: false, width: 80 }
            ...
            , functions: {
                ...
                handleAdd: function (btn) {
                    ...
                    myJqGrid.handleAdd({
                        ...
                        afterFormLoaded: function () {
                            MoneyChange();
                            $("#StuffAccountMainId").val(selectrecord.ID);
                        }
                    });
                },
                handleEdit: function (btn) {
    ...
    MoneyChange = function () {
        var unitPriceField = $($("[name='UnitPrice']")[0]);
        unitPriceField.unbind('change');
        unitPriceField.bind('change', function (event) {
            var a = $("#UnitPrice").val();
            if (a == "") {
                return;
            }
            var b = $("#CurrentCount").val();
            if (b == "") {
                return;
            }
            $("#CurrentMoney").val($("#CurrentCount").val() * 1 * $("#UnitPrice").val());
        });
        var amountField = $($("[name='CurrentCount']")[0]);
        amountField.unbind('change');
        amountField.bind('change', function (event) {
            var a = $("#UnitPrice").val();
            if (a == "") {
                return;
            }
        ...
        var moneyField = $($("[name='CurrentMoney']")[0]);
        moneyField.unbind('change');
        moneyField.bind('change', function (event) {
            var a = $("#CurrentCount").val();
            if (a == "") {
                return;
            }
            var b = $("#CurrentMoney").val();
            if (b == "") {
                return;
            }
            var c = $("#CurrentMoney").val() * 1 / $("#CurrentCount").val();
            $("#UnitPrice").val(c.toFixed(2));
        });
