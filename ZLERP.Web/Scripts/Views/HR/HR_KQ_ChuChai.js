function HR_KQ_ChuChaiIndexInit(options) {
    var myJqGrid = new MyGrid({
		...
		, initArray: [
            ...
            , { label: '出差单号', name: 'LeaveNo', index: 'LeaveNo', width: 120 }   
            ...
            , { label: '出差类型', name: 'LeaveType', index: 'LeaveType', width: 80 }
            ...
            , { label: '出差/天', name: 'DayLong', index: 'DayLong', width: 80 }
            , { label: '出差原因', name: 'Reson', index: 'Reson', width: 200 } 
            ...
            , { label: "出差附件", name: "Attachments", formatter: attachmentFmt2, sortable: false, search: false }
		]
		, autoLoad: true
        , functions: {
            ...
            handleAdd: function (btn) {
                ajaxRequest(options.GenerateOrderNoUrl, { prefix: "C" },
				false,
				function (response) {
				    if (response.Result) { 
				        myJqGrid.handleAdd({
        ...
        handleEdit: function (btn) {
            ...
            ToAuditing: function (btn) {
                ...
                showConfirm("确认信息", "您确定要审核当前出差单:" + orderNo + "？",
				function () {
            ...
            ToUnAuditing: function (btn) {
                ...
                showConfirm("确认信息", "您确定要反审当前出差单:" + orderNo + "？",
				function () {
				    var requestURL = options.ChangeConditionUrl;
				    ajaxRequest(requestURL, {
				        type: 2,
    ...
    function attachmentUpload(objectId) {
        ...
        $.ajaxFileUpload({
            url: options.uploadUrl + "?objectType=HR_KQ_ChuChai&objectId=" + objectId,
            secureuri: false,
