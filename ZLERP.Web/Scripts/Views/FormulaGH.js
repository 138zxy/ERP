function formulaIndexInit(opts) {
    ...
    var FormulaGrid = new MyGrid({
        renderTo: 'FormulaGHGridDiv'
		    ...
		    , initArray: [
                ...
                , { label: ' 理论配比名称', name: 'FormulaGHName', index: 'FormulaGHName', width: 80 }
                , { label: '料种', name: 'FormulaGHType', index: 'FormulaGHType', width: 80, formatter: dicCodeFmt, unformat: dicCodeUnFmt, formatoptions: { parentId: 'FType' }, stype: 'select', searchoptions: { value: dicToolbarSearchValues('FType')} }
                , { label: ' 强度', name: 'ConStrength', index: 'ConStrength', width: 80, stype: 'select', searchoptions: { dataUrl: opts.conStrengthSelectUrl} }
                , { label: ' 浇筑方式', name: 'CastMode', index: 'CastMode', width: 80 }
                , { label: ' 水泥等级', name: 'CementBreed', index: 'CementBreed', width: 80, search: false, formatter: dicCodeFmt, unformat: dicCodeUnFmt, formatoptions: { parentId: 'GHBreed'} }
                ...
                , { label: ' 标记', name: 'BetonTag', index: 'BetonTag', search: false }
            ...
            , functions: {
                ...
                handleAdd: function (btn) {
                    FormulaGrid.handleAdd({
                        btn: btn
                        , loadFrom: 'FormulaGHFormDiv'
                        , closeDialog: false
                        , afterFormLoaded: function () {
                            var formulaid = FormulaGrid.getFormField("FormulaGH.ID").val();
                            mysubGrid.refreshGrid("FormulaGHID='" + formulaid + "'");
                            FormulaGrid.setFormFieldReadOnly("FormulaGH.ID", false);
                            //FormulaGrid.setFormFieldReadOnly("FormulaGH.FormulaGHName", true);
                        }
                        , beforeSubmit: function () {
                            var cementType = FormulaGrid.getFormField("FormulaGH.CementType").val();
                            var cType = "";
                            if (cementType == '普通混凝土') {
                                cType = "A";
                            }
                            ...
                            var conStrength = FormulaGrid.getFormField("FormulaGH.ConStrength").val();
                            var slump = FormulaGrid.getFormField("FormulaGH.Slump").val();
                            var carpRadii = FormulaGrid.getFormField("FormulaGH.CarpRadii").val();
                            var cementBreed = FormulaGrid.getFormField("FormulaGH.CementBreed").val();
                            var FormulaGH_BetonTag = cType + " " + conStrength + "-GD" + carpRadii + "-" + cementBreed;
                            
                            return true;
                        }
                        , postCallBack: function (response) {
                            //                            alert(this);
                            if (FormulaGrid.getFormField("FormulaGH.ID").val().length > 0) {
                                $("#FormulaGHFormDiv").dialog('close'); //如果隐藏控件里面有值了就直接关闭，传后台也是如果有值说明是更新而不是新增。
                            }
                            else if (response.Result) {
                                var uid = response.Data; //Data是id
                                FormulaGrid.getFormField("FormulaGH.ID").val(uid);
                                var postUrl = opts.getFormulaUrl;
                                if (uid) {
                                    var postParams = { id: uid }; //调的是Get方法 参数id
                                    setFormValuesWithPre("FormulaGHFormDiv", postUrl, postParams, this, "FormulaGH"); //现在看起来好像没什么用了
                                }
                                showMessage("消息", "请根据表格填写配比用量");
                                mysubGrid.refreshGrid("FormulaGHID='" + uid + "'");
                            }
                        }
                    });
                },
                handleEdit: function (btn) {
                    FormulaGrid.handleEdit({
                        ...
                        afterFormLoaded: function () {
                            ...
                            FormulaGrid.setFormFieldReadOnly("FormulaGH.ID", true);
                        }
                    });
                }
                ...
                , handleShow: function (btn) {
                    ...
                    ajaxRequest(
                        btn.data.Url,
                        {
                            "FormulaGH.ID": selectedRecord.ID,
                            "FormulaGH.Lifecycle": 0
    ...
    var ItemsGrid = new MyGrid({
        ...
        //, singleSelect: true
		    ...
		    , initArray: [
                ...
                , { label: ' 理论配比编号', name: 'FormulaGHID', index: 'FormulaGHID', hidden: true }
                ...
                , { label: ' 标准用量', name: 'StandardAmount', index: 'StandardAmount', width: 80, hidden: false, editable: true }
            ...
            , functions: {
                ...
                handleRefresh: function (btn) {
                    ...
                    ItemsGrid.refreshGrid("FormulaGHID='" + formulaid + "'");
                },
                handleAdd: function (btn) {
                    var formulaid = FormulaGrid.getSelectedKey();
                    if (formulaid.length > 0) {
                        ...
                        , loadFrom: 'ItemsDiv'
                        , afterFormLoaded: function () {
                            ItemsGrid.setFormFieldValue('ForGHItem.FormulaGHID', formulaid);
                            ItemsGrid.setFormFieldReadOnly('ForGHItem.FormulaGHID', true);
                        }
                        , postCallBack: function (response) {
                            ItemsGrid.refreshGrid("FormulaGHID='" + formulaid + "'");
                            FormulaGrid.reloadGrid();
        ...
        $("#ForItem_StuffAmount").bind('change', function () {
            $("#ForItem_StandardAmount").val($("#ForItem_StuffAmount").val()); 
        });
            ...
            , functions: {
                ...
                handleAdd: function (btn) {
                    ...
                    if (formulaid.length > 0) {
                        ...
                        , loadFrom: 'ItemsDiv'
                        , afterFormLoaded: function () {
                            mysubGrid.setFormFieldValue('ForGHItem.FormulaGHID', formulaid);
                            mysubGrid.setFormFieldReadOnly('ForGHItem.FormulaGHID', true);
                        }
                        , postCallBack: function (response) {
                            mysubGrid.refreshGrid("FormulaGHID='" + formulaid + "'");
                            FormulaGrid.reloadGrid();
    ...
    FormulaGrid.addListeners('rowclick', function (id, record, selBool) {
        ItemsGrid.getJqGrid().setCaption("材料用量列表：" + record.FormulaGHName + "&nbsp;<font color='#2E6E9E'>－</font>&nbsp;" + record.ConStrength);
        ItemsGrid.refreshGrid("FormulaGHID='" + id + "'");
    ...
    //单元格-提交后
    mysubGrid.addListeners("afterSubmitCell", function () {
    ...
    $("#FormulaGH_SCRate").bind('blur', function () {
        var scrate = $("#FormulaGH_SCRate").val();

        if (scrate > 0) {
            var weight = $("#FormulaGH_Weight").val();
...
//        );
    ...
    $("#TAmount_S").bind('blur', function () {
        var amount=$("#FormulaGH_Weight").val();
...
window.SaveFormulaGH = function () {
    var FormulaGHName = $("#FormulaGHName").val();
    var FormulaGHType = $("#FormulaGHType").val();
    var ConStrength = $("#ConStrength").val();


    var data = $("#FormulaGHDesignDiv form").serialize();
    //data是FormulaDesignModel序列化对象

    $.post("/FormulaGH.mvc/SaveDeFormulaGH", data, function (response) {
        if (response.Result) {
