function contractpumpIndexInit(options) {
    ...
    var ContractPumpGrid = new MyGrid({
        renderTo: 'ContractPumpGrid'
		    ...
		    , initArray: [
                ...
                , { label: ' 价格', name: 'PumpPrice', index: 'PumpPrice', editable: true, formatter: 'number', editrules: { number: true }, align: 'right', width: 100 }
    ...
    var pumpTypeGrid = new MyGrid({
        renderTo: 'pumpTypeGrid'
		    ...
		    , initArray: [
                { label: ' 编号', name: 'ID', index: 'ID', hidden: true }
                , { label: ' 浇筑方式', name: 'DicName', index: 'DicName', sortable:false}
		    ]
		    , autoLoad: false
            //, show
            , functions: {
