function userIndexInit(opts) {
    $('#projects').height(gGridHeight + 80);
    ...
    var filterCondition = "UserType<>'01'  and UserType='ZWRY'";
    ...
    var myJqGrid = new MyGrid({
        renderTo: 'My-JqGrid'
            ...
            , functions: {
                ...
                , handleSaveUserProjects: function (btn, e) {
                    //保存权限  
                    var userId = myJqGrid.getSelectedKey();
                    var treeObj = $.fn.zTree.getZTreeObj('projects');
                    var nodes = treeObj.getCheckedNodes();
                    console.log(nodes);
                    if (userId) {
                        var ids = [];
                        for (var i = 0; i < nodes.length; i++) {
                            ids.push(nodes[i].dbId);
                        }
                        console.log(ids);
                        ajaxRequest(
                            btn.data.Url,
                            { userIds: userId, projects: ids },
                            true,
                            function () {
                                //清空当前用户缓存
                                delete (userFuncCache[userId]);
    ...
    function bindUserFuncs(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            var func = data[i];
            var node = treeObj.getNodesByParam("dbId", func.ID); 
            //console.dir(node[0]);
    ...
    $.fn.zTree.init($('#projects'), treeSettings);
