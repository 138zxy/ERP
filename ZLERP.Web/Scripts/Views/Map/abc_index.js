...
function addFactoryInMap() {
    $.ajax({
        type: "POST",
        url: "/Company.mvc/GetAllCompanyForMap",
        data: {},
        dataType: "json",
        success: function (response) {
            if (response != null) {
                for (var i = 0; i < response.length; i++) {
                    ...
                    factoryLng = response[i].Longtide;
                    factoryLat = response[i].Latitude;
                    var fctopt = { lng: factoryLng, lat: factoryLat, iurl: iconArr.factIcon, clickEv: "", tipTxt: "", content: "<table class='tb_pj_img'><tr><td><img src='" + iconArr.factIcon + "' /></td></tr><tr><td><span>" + compname + "</span></td></tr></table>", id: "factory_mark_s" + i, title: compname };
...
function initMapContextMenu() {
    ...
    $("<li onclick='updateMapProjectGH()'> > 标定干混工地</li>").appendTo(ct_ul);
    if (homemarksetright) {
...
function bindPrjContextMenu(mkid) {
    ...
    AMap.event.addListener(mk, "rightclick", function (e) {
        ...
        $("<li onclick=\"ShowAllBoundInMapByPid('" + getProjIdByMarkerId(mk.extData) + "')\"> > 显示工地围栏 </li>").appendTo(ct_ul);
        $("<li onclick=\"clearAllBound()\"> > 隐藏工地围栏 </li>").appendTo(ct_ul);
...
function updateMapProjectGH() {
    ...
    if (ptLat > 0 && ptLng > 0) {
        $("#markProjectGHDiv").dialog({
            ...
            autoOpen: false,
            close: function (event, ui) {
                $('#markProjectGHDiv>form')[0].reset();
                $(this).dialog("destroy");

                $("#container").append($("#markProjectGHDiv"));
            },
            buttons: [{ text: '取消', click: CloseProjectGHMarkFn }, { text: '保存', click: MarkProjectGH}]
        });
        $("#markProjectGHDiv").dialog('open');
        $("#markProjectGHDiv [name='Longitude']").val(ptLng);
        $("#markProjectGHDiv [name='Latitude']").val(ptLat);

        $("#markProjectGHDiv input[name='ID']").unbind('change');
        $("#markProjectGHDiv input[name='ID']").bind('change', function () {
            var item = $("input[name='PGHName']").data($(this).val() + "_cache");
            if (!isEmpty(item)) {
                var label = item.label;
                ...
                $("#markProjectGHDiv [name='ProjectID']").val(item.value);
                $("#markProjectGHDiv [name='ProjectName']").val(item.text);
                $("#markProjectGHDiv [name='ProjectAddr']").val(result[1]);
                $("#markProjectGHDiv [name='LinkMan']").val(result[2]);
                $("#markProjectGHDiv [name='Tel']").val(result[3]);
                $("#markProjectGHDiv [name='Remark']").val(result[4]);
            }
            else {
                $("#markProjectGHDiv [name='ProjectID']").val("");
                $("#markProjectGHDiv [name='ProjectName']").val("");
                $("#markProjectGHDiv [name='ProjectAddr']").val("");
                $("#markProjectGHDiv [name='LinkMan']").val("");
                $("#markProjectGHDiv [name='Tel']").val("");
                $("#markProjectGHDiv [name='Remark']").val("");
            }
        });
...
function MarkProjectGH() {
    var Longitude = $("#markProjectGHDiv [name='Longitude']").val();
    var Latitude = $("#markProjectGHDiv [name='Latitude']").val();
    var PlaceRange = $("#markProjectGHDiv [name='PlaceRange']").val();
    if (isEmpty(Longitude) || isEmpty(Latitude) || isEmpty(PlaceRange) || PlaceRange <= 0) {
        showError("经纬度或工地范围格式不正确！");
        return;
    }

    ajaxRequest('/ProjectGH.mvc/Update', $("#markProjectGHDiv>form").serialize(), true, function (response) {
        if (response.Result) {
            var pId = $("#markProjectGHDiv>form").find(":input[name='ID']").val();
            $("#markProjectGHDiv").dialog('close');

            var _zTree = $.fn.zTree.getZTreeObj("treeDemo_Project");
            var _node = _zTree.getNodeByParam("dbId", 'p4_' + pId, null);
            if (!isEmpty(_node) && !_node.checked) {
                _zTree.checkNode(_node, null, true, zTreeOnCheck);
            }
...
function carInfoWindow(data) {
    ...
    var content =
    "更新时间:<bluefont>" + data["Receivetime"]
    ...
    var btnstr = "<a href='javascript:void(0)' onclick='javascript:openCarInfoAccd(\"" + data.CustomNo + "\")'>查询当前状态</a> <a href='javascript:void(0)' onclick='javascript:openTrackAccd(\"" + data.Id + "\", \"" + data.VideoDeviceGuid + "\")'>查询历史轨迹</a></a>";
    btnstr += "&nbsp;<a href='javascript:void(0)' onclick='javascript:openReadTimeCarVideo(\"" + data.VideoDeviceGuid + "\")'>实时视频</a>";

    return "<div class='mapTips'>" + titles + content + btnstr + "</div>";
}
function openTrackAccd(id, videoDeviceGuid) {
    console.log("selected Car.videoDeviceGuid = " + videoDeviceGuid);
    $("#carList").val(id);
    $("#VideoDeviceGuid").val(videoDeviceGuid);
    $("#trackPlayAccd").click();
}
...
function openReadTimeCarVideo(videoDeviceGuid) {
	...
	console.log("videoDeviceGuid = " + videoDeviceGuid);
	var realTimeVideoUrl = gSysConfig.CarRealTimeVideoUrl + "?deviceGuid=" + videoDeviceGuid;
...
function initCarInfoDiv() {
    ...
    $('#carinfojqgrd').jqGrid({
        ...
        colModel: [
                    ...
                    , { label: "最后更新时间", name: 'Receivetime', index: 'Receivetime', width: 120, align: 'right' }
                    ...
                    , { label: '视频设备编码', name: 'VideoDeviceGuid', width: 80 }
                    ],
...
function SetBoundCmp() {
    ...
    var boundType = $("#boundType").val();

    var item_p = $("input[name='PLID']").val();
    var item_c = $("input[name='CLID']").val();
    var vp = "";
    var vc = "";
    if (item_c != "") {
        vc = item_c;
    ...
    var oJson = { isOutAlarm: 0, bounds: boundstrs, boundName: bname, ProjectID: vp, CompanyID: vc, boundType: boundType };
...
function ShowAllBoundInMapByPid(id) {
    removeTempBound();
