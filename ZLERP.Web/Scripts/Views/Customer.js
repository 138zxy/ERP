...
//                                });
                ...
                handleEnableAll: function (btn) {
                    var keys = customersgrid.getSelectedKeys();
                    if (keys.length > 0) {
                        var requestURL = '/Customer.mvc/EnableAll';
                        var postParams = { ids: keys };
                        ajaxRequest(requestURL, postParams, true, function (response) {
                            customersgrid.reloadGrid();
                        });
                    }
                    else {
                ...
                //批量关闭
                handleDisableAll: function (btn) {
                    var keys = customersgrid.getSelectedKeys();
                    if (keys.length > 0) {
                        var requestURL = '/Customer.mvc/DisableAll';
                        var postParams = { ids: keys };
                        ajaxRequest(requestURL, postParams, true, function (response) {
                            customersgrid.reloadGrid();
                        });
