...
function consMixpropIndexInit(opts) {
    var initArray = [
        ...
        , { label: ' 强度', name: 'ConStrength', index: 'ConStrength', width: 60, align: 'center', stype: 'select', searchoptions: { dataUrl: opts.conStrengthSelectUrl }, sortable: false }
    ...
    var options = {
            ...
            , datatype: 'json'
            , mtype: 'POST'
            , postData: { condition: " ProduceTaskGH.IsCompleted=0 and AuditStatus = 1 and ProductLineID = '001' " }
            , jsonReader: {
                root: "rows",
                page: "page",
                total: "total",
           ...
           //, groupingView: { groupField: ['ProductLineID'], groupColumnShow: [false], groupDataSorted: [true], groupOrder: ['ASC'] }
		    , cellEdit: true
            , cellurl: '/ConsMixpropGH.mvc/Update'
        ...
        , handleAudit: function (btn) {
            ...
            ConsGrid.showWindow({
                ...
                afterLoaded: function () {
                    $("[name='ConsMixpropGH.ID']", "#PBAuditForm").val(id);
                    $("[name='ConsMixpropGH.AuditStatus']", "#PBAuditForm").val(1);
                    $("[name='ConsMixpropGH.FormulaName']", "#PBAuditForm").val(record.FormulaName);
                    $("[name='ConsMixpropGH.MixingTime']", "#PBAuditForm").val(record.MixingTime);
    ...
    function getDynamicColAndCreateGrid(pid, plValue) {
        ajaxRequest(
                "/ConsMixpropGH.mvc/GetDynamicCol",
                { ProductLineID: pid },
                false,
                function (response) {
                    ...
                    var plValue = '';
                    if (pid) {
                        plValue = " ProduceTaskGH.IsCompleted=0 and ProductLineID='" + pid + "' AND ";
                    }
