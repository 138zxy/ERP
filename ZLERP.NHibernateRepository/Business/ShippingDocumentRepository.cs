...
namespace ZLERP.NHibernateRepository
{
    ...
    public class ShippingDocumentGHRepository : NHRepositoryBase<ShippingDocumentGH>, IShippingDocumentGHRepository
    {
        public ShippingDocumentGHRepository(ISession session)
            : base(session)
        {
        ...
        #region IShippingDocumentGHRepository 成员

        public IList<DailyReport> ExecuteShipDoc(string StartDateTime, string EndDateTime, out int total)
        {
            string sp = "exec sp_rpt_dd_dailyGH @StartDateTime=:StartDateTime, @EndDateTime=:EndDateTime, @ProductLineID=:ProductLineID";
            var query = this._session.CreateSQLQuery(sp);
            query.SetString("StartDateTime", StartDateTime);
            query.SetString("EndDateTime", EndDateTime);
            query.SetString("ProductLineID", null);
        ...
        public bool ShipDocMultiAudit(string idstrs, string CurrentUserID)
        {
            string sp = "exec sp_ShipDocMultiAuditGH  @ShipDocIds=:ShipDocIds,@CurrentUserID=:CurrentUserID";
            var query = this._session.CreateSQLQuery(sp);
            query.SetString("ShipDocIds", idstrs);
            query.SetString("CurrentUserID", CurrentUserID);
            object ret = query.UniqueResult();
        ...
        /// 运输单调价
        /// </summary>
        /// <param name="beginDate">起始时间（包含在内）</param>
        /// <param name="endDate">截止时间（包含在内）</param>
        /// <param name="contractId">合同编号</param>
        /// <returns>调价运输单的行数</returns>
        public int RefreshShipDocPrice(DateTime beginDate, DateTime endDate, string contractId)
        {
            string sp = "exec sp_RefreshShipDocPriceGH  @beginDate = :beginDate, @endDate = :endDate, @contractId = :contractId";
            var query = this._session.CreateSQLQuery(sp);
        ...
        {
            string sp = "exec sp_RefreshShipDocFreightGH  @beginDate = :beginDate, @endDate = :endDate, @contractId = :contractId";
            var query = this._session.CreateSQLQuery(sp);
